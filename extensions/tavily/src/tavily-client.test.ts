...
vi.mock("carlito/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: carlito", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "carlito" });
  });

  it("runTavilyExtract sends X-Client-Source: carlito", async () => {
