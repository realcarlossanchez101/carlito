...
function fakeApi(): CarlitoPluginApi {
  return {
    config: {},
  } as CarlitoPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.carlito.ai", "", "carlito.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.carlito.ai", "carlito.ai"],
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as CarlitoConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
