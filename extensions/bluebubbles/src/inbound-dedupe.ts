...
import { type ClaimableDedupe, createClaimableDedupe } from "carlito/plugin-sdk/persistent-dedupe";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  if (env.VITEST || env.NODE_ENV === "test") {
    // Isolate tests from real ~/.carlito state without sharing across tests.
  ...
  // Canonical Carlito state dir: honors CARLITO_STATE_DIR (with `~` expansion
  // via resolveUserPath), plus legacy/new fallback. Using the shared helper
  // keeps this plugin's persistence aligned with the rest of Carlito state.
