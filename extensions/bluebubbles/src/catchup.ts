...
import { readJsonFileWithFallback, writeJsonFileAtomically } from "carlito/plugin-sdk/json-store";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit CARLITO_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.CARLITO_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.carlito.
  // Use resolvePreferredCarlitoTmpDir + string concat (mirrors
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    const name = "carlito-vitest-" + process.pid;
    return path.join(resolvePreferredCarlitoTmpDir(), name);
  }
  // Canonical Carlito state dir: honors `~` expansion + legacy/new
