...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): CarlitoConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): CarlitoConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the Ollama experimental search endpoint", async () => {
    ...
    const result = await tool.execute({ query: "carlito docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://carlito.ai/docs" }],
    });
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest carlito release" })).rejects.toThrow(
      "ollama signin",
    );
  });
