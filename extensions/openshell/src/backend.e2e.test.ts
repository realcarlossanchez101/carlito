...
const CARLITO_OPENSHELL_E2E = process.env.CARLITO_E2E_OPENSHELL === "1";
const CARLITO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const CARLITO_OPENSHELL_COMMAND = process.env.CARLITO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "carlito-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && CARLITO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: CARLITO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(CARLITO_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(CARLITO_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-openshell-e2e-"));
      ...
      const gatewayName = `carlito-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `carlito-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/carlito-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: CARLITO_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("carlito-openshell-e2e");
