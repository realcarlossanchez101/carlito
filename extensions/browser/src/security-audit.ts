import type { CarlitoPluginSecurityAuditContext } from "carlito/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "carlito/plugin-sdk/secret-input";
import { formatCliCommand } from "carlito/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "carlito/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: CarlitoPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("carlito security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.CARLITO_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.CARLITO_GATEWAY_PASSWORD) ||
