...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const carlito = ctx.forProfile("carlito");

    const opened = await carlito.openTab("about:blank");
  ...
  it("uses an HTTP /json base for focus and close under strict SSRF", async () => {
    ...
    await carlito.focusTab("T1");
    await carlito.closeTab("T1");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await carlito.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await carlito.focusTab("T2");
    await carlito.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(carlito.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(carlito.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(carlito.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
