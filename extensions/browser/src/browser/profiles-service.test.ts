...
vi.mock("./chrome.js", () => ({
  resolveCarlitoUserDataDir: vi.fn(() => "/tmp/carlito-test/carlito/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "carlito-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveCarlitoUserDataDir).mockReturnValue(userDataDir);
