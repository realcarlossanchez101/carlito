...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCarlitoTmpDir: vi.fn(() => "/tmp/carlito"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-carlito-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-carlito-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredCarlitoTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredCarlitoTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCarlitoTmpDir.mockReturnValue("/tmp/carlito");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".carlito-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCarlitoTmpDir.mockReturnValue("/tmp/carlito-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "carlito-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "carlito-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredCarlitoTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "carlito-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "carlito-preferred", "downloads")}${path.sep}`),
    );
  });
