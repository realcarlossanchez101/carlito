...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post(
    ...
    asyncBrowserRoute(async (req, res) => {
      ...
      if (driver && driver !== "carlito" && driver !== "clawd" && driver !== "existing-session") {
        return jsonError(
          res,
          400,
          `unsupported profile driver "${driver}"; use "carlito", "clawd", or "existing-session"`,
        );
      }
