...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".carlito-profile-decorated");
}
 ...
 */
export function decorateCarlitoProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CARLITO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CARLITO_BROWSER_COLOR).toUpperCase();
