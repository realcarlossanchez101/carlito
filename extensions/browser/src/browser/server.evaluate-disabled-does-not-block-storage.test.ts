  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          carlito: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.CARLITO_GATEWAY_PORT;
    process.env.CARLITO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.CARLITO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.CARLITO_GATEWAY_PASSWORD;
