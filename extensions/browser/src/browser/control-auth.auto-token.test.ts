...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => CarlitoConfig>(),
  writeConfigFile: vi.fn<(cfg: CarlitoConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<CarlitoConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: CarlitoConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): CarlitoConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: CarlitoConfig) {
