...
import {
  buildCarlitoChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchCarlitoChrome,
  resolveCarlitoUserDataDir,
  stopCarlitoChrome,
} from "./chrome.js";
 ...
 * not exercise: launchCarlitoChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopCarlitoChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildCarlitoChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveCarlitoUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveCarlitoUserDataDir();
      expect(dir.endsWith(path.join("carlito", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveCarlitoUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildCarlitoChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildCarlitoChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchCarlitoChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchCarlitoChrome(resolved, profile);
  ...
  describe("launchCarlitoChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "carlito-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchCarlitoChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchCarlitoChrome(makeResolved(), profile);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchCarlitoChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopCarlitoChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopCarlitoChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopCarlitoChrome>[0],
        1,
      );
  ...
  describe("launchCarlitoChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "carlito-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveCarlitoUserDataDir(profileName));
        ...
        const staged = resolveCarlitoUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_CARLITO_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      await expect(launchCarlitoChrome(resolved, profile)).rejects.toThrow(/Chrome stderr:/);
    });
    ...
    it("omits the sandbox hint on non-linux platforms", async () => {
      ...
      try {
        ...
        let caught: unknown;
        try {
          await launchCarlitoChrome(resolved, profile);
        } catch (e) {
          caught = e;
        }
    ...
    it("logs a warning when decorateCarlitoProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateCarlitoProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateCarlitoProfile }, "decorateCarlitoProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".carlito-profile-decorated") || s.endsWith("Preferences")) {
