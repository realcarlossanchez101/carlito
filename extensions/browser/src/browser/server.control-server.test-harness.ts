...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.CARLITO_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "carlito",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/carlito" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    carlito: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchCarlitoChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveCarlitoUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopCarlitoChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "carlito";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.CARLITO_GATEWAY_PORT;
  process.env.CARLITO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.CARLITO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.CARLITO_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CARLITO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.CARLITO_GATEWAY_PASSWORD;
  } else {
    process.env.CARLITO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
