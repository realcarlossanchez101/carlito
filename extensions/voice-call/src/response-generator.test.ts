...
function createAgentRuntime(payloads: Array<Record<string, unknown>>) {
  ...
  const runtime = {
    ...
    resolveAgentDir: () => "/tmp/carlito/agents/main",
    resolveAgentWorkspaceDir: () => "/tmp/carlito/workspace/main",
    ...
    runEmbeddedPiAgent,
    session: {
      resolveStorePath: () => "/tmp/carlito/sessions.json",
      loadSessionStore: () => ({}),
      saveSessionStore: async () => {},
      resolveSessionFilePath: () => "/tmp/carlito/sessions/session.jsonl",
    },
