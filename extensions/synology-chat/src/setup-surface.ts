...
function getChannelConfig(cfg: CarlitoConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: CarlitoConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: CarlitoConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: CarlitoConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Carlito uses to send replies back to Chat.",
      ],
