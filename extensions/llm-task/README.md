...
`carlito.invoke --each`) without adding new Carlito code per workflow.
...
This extension depends on Carlito internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Carlito extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.carlito/extensions` as a standalone plugin directory.
