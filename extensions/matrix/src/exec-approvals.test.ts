...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<CarlitoConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<CarlitoConfig["channels"]>["matrix"]>>,
): CarlitoConfig {
  return {
    channels: {
      matrix: {
