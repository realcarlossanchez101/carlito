...
const CARLITO_PLUGIN_SDK_PACKAGE_NAMES = [
  ["carlito", "plugin-sdk"].join("/"),
  ["@carlito", "plugin-sdk"].join("/"),
];
...
function hasTrustedCarlitoRootIndicator(packageRoot, packageJson) {
  ...
  const hasCarlitoBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("carlito")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.carlito === "string");
  const hasCarlitoEntrypoint = fs.existsSync(path.join(packageRoot, "carlito.mjs"));
  return hasCliEntryExport || hasCarlitoBin || hasCarlitoEntrypoint;
}

function findCarlitoPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "carlito" && hasTrustedCarlitoRootIndicator(cursor, pkg)) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findCarlitoPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    for (const packageName of CARLITO_PLUGIN_SDK_PACKAGE_NAMES) {
  ...
  if (extensionApi) {
    aliasMap["carlito/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findCarlitoPackageRoot(moduleDir);
  if (location) {
