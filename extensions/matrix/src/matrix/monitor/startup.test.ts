...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleCarlitoDevices: [] as MatrixManagedDeviceInfo[],
      currentCarlitoDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      currentDeviceId: null,
      staleCarlitoDevices: [{ deviceId: "DEV123", displayName: "Carlito Device", current: false }],
      currentCarlitoDevices: [],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Carlito devices detected for @bot:example.org: DEV123. Run 'carlito matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'carlito matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
