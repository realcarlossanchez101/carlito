...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'carlito matrix verify status --verbose --account ${accountId}' and 'carlito matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Carlito runtime is using the same account. " +
    `Check 'carlito matrix verify status --verbose --account ${accountId}' and 'carlito matrix devices list --account ${accountId}'.`
  );
}
