...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleCarlitoDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Carlito devices detected for ${params.auth.userId}: ${deviceHealth.staleCarlitoDevices.map((device) => device.deviceId).join(", ")}. Run 'carlito matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
