...
function resolveQianfanPreset(cfg: CarlitoConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: CarlitoConfig) => {
...
export function applyQianfanProviderConfig(cfg: CarlitoConfig): CarlitoConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: CarlitoConfig): CarlitoConfig {
