import type { PluginCommandContext } from "carlito/plugin-sdk/plugin-entry";
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an Carlito session file.";
  }
  ...
  return `Attached this Carlito session to Codex thread ${effectiveThreadId}.`;
}
...
async function startThreadAction(
  ...
  if (!ctx.sessionFile) {
    return `Cannot start Codex ${label} because this command did not include an Carlito session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(ctx.sessionFile);
  if (!binding?.threadId) {
    return `No Codex thread is attached to this Carlito session yet.`;
  }
