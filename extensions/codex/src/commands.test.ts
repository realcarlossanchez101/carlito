...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-codex-command-"));
  });
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this Carlito session to Codex thread thread-123.",
    });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this Carlito session yet.",
    });
  });
