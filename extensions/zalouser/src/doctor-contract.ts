...
type ZalouserChannelsConfig = NonNullable<CarlitoConfig["channels"]>;
...
function normalizeZalouserCompatibilityConfig(cfg: CarlitoConfig): ChannelDoctorConfigMutation {
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.zalouser.groups.<id>.allow is legacy; use channels.zalouser.groups.<id>.enabled instead. Run "carlito doctor --fix".',
    match: hasLegacyZalouserGroupAllowAliases,
  },
  {
    path: ["channels", "zalouser", "accounts"],
    message:
      'channels.zalouser.accounts.<id>.groups.<id>.allow is legacy; use channels.zalouser.accounts.<id>.groups.<id>.enabled instead. Run "carlito doctor --fix".',
