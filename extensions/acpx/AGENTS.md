...
The bundled ACPX extension is a thin Carlito wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `carlito/acpx`, not in this extension.
...
Use this flow when Carlito needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `carlito/acpx` repo first.
2. In Carlito, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Carlito and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Carlito ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Carlito-specific glue, move it to `carlito/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
