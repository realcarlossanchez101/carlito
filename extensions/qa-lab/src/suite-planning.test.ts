...
describe("qa suite planning helpers", () => {
  it("normalizes suite concurrency to a bounded integer", () => {
    const previous = process.env.CARLITO_QA_SUITE_CONCURRENCY;
    delete process.env.CARLITO_QA_SUITE_CONCURRENCY;
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.CARLITO_QA_SUITE_CONCURRENCY = previous;
      }
    }
  });
