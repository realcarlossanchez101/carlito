...
const QA_LIVE_ENV_ALIASES = Object.freeze([
  {
    liveVar: "CARLITO_LIVE_OPENAI_KEY",
    providerVar: "OPENAI_API_KEY",
  },
  {
    liveVar: "CARLITO_LIVE_ANTHROPIC_KEY",
    providerVar: "ANTHROPIC_API_KEY",
  },
  {
    liveVar: "CARLITO_LIVE_GEMINI_KEY",
    providerVar: "GEMINI_API_KEY",
  },
]);

export const QA_LIVE_PROVIDER_CONFIG_PATH_ENV = "CARLITO_QA_LIVE_PROVIDER_CONFIG_PATH";
export const QA_LIVE_CLI_BACKEND_PRESERVE_ENV = "CARLITO_LIVE_CLI_BACKEND_PRESERVE_ENV";
export const QA_LIVE_CLI_BACKEND_AUTH_MODE_ENV = "CARLITO_LIVE_CLI_BACKEND_AUTH_MODE";
...
export const QA_PROVIDER_SECRET_ENV_VARS = Object.freeze([
  ...
  "CARLITO_QA_CONVEX_SECRET_CI",
  "CARLITO_QA_CONVEX_SECRET_MAINTAINER",
  "VOYAGE_API_KEY",
]);
...
export function resolveQaLiveCliAuthEnv(
  ...
  const hasAnthropicKey = Boolean(
    baseEnv.ANTHROPIC_API_KEY?.trim() || baseEnv.CARLITO_LIVE_ANTHROPIC_KEY?.trim(),
  );
  if (opts?.forwardHostHomeForClaudeCli && authMode === "api-key" && !hasAnthropicKey) {
    throw new Error(
      "Claude CLI API-key QA mode requires ANTHROPIC_API_KEY or CARLITO_LIVE_ANTHROPIC_KEY",
    );
  }
...
export function resolveQaLiveProviderConfigPath(env: NodeJS.ProcessEnv = process.env) {
  const explicit = env[QA_LIVE_PROVIDER_CONFIG_PATH_ENV]?.trim() || env.CARLITO_CONFIG_PATH?.trim();
  return explicit
    ? { path: resolveUserPath(explicit, env), explicit: true }
    : { path: path.join(os.homedir(), ".carlito", "carlito.json"), explicit: false };
}
