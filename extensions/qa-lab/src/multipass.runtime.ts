...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/carlito-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/carlito";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/carlito-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `carlito-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "CARLITO_CONFIG_PATH" &&
          key !== "CARLITO_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `CARLITO_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `CARLITO_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm carlito qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredCarlitoTmpDir(), `${plan.vmName}-qa-suite-`),
  );
