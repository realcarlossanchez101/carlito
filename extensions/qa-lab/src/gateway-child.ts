...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    CARLITO_HOME: params.homeDir,
    CARLITO_CONFIG_PATH: params.configPath,
    CARLITO_STATE_DIR: params.stateDir,
    CARLITO_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    CARLITO_TEST_FAST: "1",
    CARLITO_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    // QA uses the fast runtime envelope for speed, but it still exercises
    // normal config-driven pulsechecks and runtime config writes.
    CARLITO_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { CARLITO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { CARLITO_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredCarlitoTmpDir(), "carlito-qa-suite-"),
  );
  ...
  const configPath = path.join(tempRoot, "carlito.json");
  ...
  const keepTemp = process.env.CARLITO_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
