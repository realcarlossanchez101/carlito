...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("carlito.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("carlito.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("carlito.qmdEmbedQueueTail");
...
vi.mock("carlito/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("carlito/plugin-sdk/memory-core-host-engine-foundation")
  >("carlito/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("carlito/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("carlito/plugin-sdk/file-lock")>(
    "carlito/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { CarlitoConfig } from "carlito/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: CarlitoConfig }) {
