...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): CarlitoConfig {
    const defaults: NonNullable<NonNullable<CarlitoConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: CarlitoConfig) {
