...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("CARLITO_TEST_MEMORY_UNSAFE_REINDEX", "1");
    clearRegistry();
  ...
  async function getFtsSessionManager(params: {
    ...
    vi.stubEnv("CARLITO_STATE_DIR", path.join(workspaceDir, params.stateDirName));
    const cfg = createCfg({
