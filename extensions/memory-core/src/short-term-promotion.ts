...
import { formatMemoryDreamingDay } from "carlito/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "carlito/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "carlito-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*carlito-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*carlito-memory-promotion:([^\n]+?)\s*-->/gi);
