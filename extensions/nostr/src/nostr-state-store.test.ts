...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CARLITO_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-nostr-"));
  process.env.CARLITO_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.CARLITO_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".carlito");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CARLITO_STATE_DIR = previous;
    }
