...
import type { CarlitoPluginApi, CarlitoPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<CarlitoPluginApi> = {}): CarlitoPluginApi {
...
function fakeCtx(overrides: Partial<CarlitoPluginToolContext> = {}): CarlitoPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: CarlitoPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
