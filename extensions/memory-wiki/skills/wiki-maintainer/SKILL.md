...
description: Maintain the Carlito memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `carlito wiki ingest`, `carlito wiki compile`, and `carlito wiki lint` as the default maintenance loop.
- In `bridge` mode, run `carlito wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `carlito wiki unsafe-local import` only when the user explicitly opted into private local path access.
