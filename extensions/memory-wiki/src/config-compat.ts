...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.memory-wiki.config.bridge.readMemoryCore is legacy; use plugins.entries.memory-wiki.config.bridge.readMemoryArtifacts. Run "carlito doctor --fix".',
    match: hasLegacyBridgeArtifactToggle,
  },
];

export function migrateMemoryWikiLegacyConfig(config: CarlitoConfig): {
...
export function normalizeCompatibilityConfig({ cfg }: { cfg: CarlitoConfig }): {
  config: CarlitoConfig;
  changes: string[];
} {
