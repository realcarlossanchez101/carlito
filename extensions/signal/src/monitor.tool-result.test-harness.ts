...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/carlito-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("carlito/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("carlito/plugin-sdk/reply-runtime")>(
...
vi.mock("carlito/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("carlito/plugin-sdk/conversation-runtime")>(
    "carlito/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("carlito/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("carlito/plugin-sdk/security-runtime")>(
    "carlito/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("carlito/plugin-sdk/reply-runtime"),
      import("carlito/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
