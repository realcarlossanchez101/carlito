...
export const signalDmPolicy = {
  ...
  resolveConfigKeys: (cfg: CarlitoConfig, accountId?: string) =>
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "Carlito"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("carlito gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
