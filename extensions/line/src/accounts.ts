...
import { resolveAccountEntry } from "carlito/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: CarlitoConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: CarlitoConfig): string {
