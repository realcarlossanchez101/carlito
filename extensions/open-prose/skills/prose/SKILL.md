...
metadata: { "carlito": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Carlito `sessions_spawn`
- **File I/O** == Carlito `read`/`write`
- **Remote fetch** == Carlito `web_fetch` (or `exec` with curl when POST is required)
