...
import { canResolveEnvSecretRefInReadOnlyPath } from "carlito/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "carlito/plugin-sdk/secret-input";
...
function resolveSearchConfig(cfg?: CarlitoConfig): WebSearchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: CarlitoConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: CarlitoConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: CarlitoConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: CarlitoConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: CarlitoConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: CarlitoConfig, override?: number): number {
