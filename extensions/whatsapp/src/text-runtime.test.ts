...
describe("jidToE164", () => {
  it("maps @lid using reverse mapping file", async () => {
    await withTempDir("carlito-state-", async (stateDir) => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("carlito-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("carlito-lid-a-", async (first) => {
      await withTempDir("carlito-lid-b-", (second) => {
