...
import { setLoggerOverride } from "carlito/plugin-sdk/runtime-env";
import { withEnvAsync } from "carlito/plugin-sdk/testing";
...
describe("web auto-reply connection", () => {
    ...
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      try {
        ...
        expect(firstArgs.Body).toMatch(
          new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\[carlito\\] first`),
        );
        expect(firstArgs.Body).not.toContain("second");
        expect(secondArgs.Body).toMatch(
          new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\[carlito\\] second`),
        );
  ...
  it("emits pulsecheck logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/carlito-pulsecheck-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/carlito-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
  ...
  it("marks dispatch idle after replies flush", async () => {
    ...
    const mockConfig: CarlitoConfig = {
