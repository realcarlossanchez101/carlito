...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-group-gating-"));
...
const makeConfig = (overrides: Record<string, unknown>) =>
  ({
    ...
    ...overrides,
  }) as unknown as ReturnType<typeof import("carlito/plugin-sdk/config-runtime").loadConfig>;

async function runGroupGating(params: {
  cfg: ReturnType<typeof import("carlito/plugin-sdk/config-runtime").loadConfig>;
...
function makeInboundCfg(messagePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/carlito" } },
...
describe("applyGroupGating", () => {
  ...
  it("does not treat group mention gating as self-chat under implicit self fallback", async () => {
    ...
    const { result, groupHistories } = await runGroupGating({
      ...
      msg: createGroupMessage({
        id: "g-other-mention",
        body: "@carlito please check this",
