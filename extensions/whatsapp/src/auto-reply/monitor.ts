import { resolveAccountEntry } from "carlito/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "carlito/plugin-sdk/channel-inbound";
import { formatCliCommand } from "carlito/plugin-sdk/cli-runtime";
import { hasControlCommand } from "carlito/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "carlito/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "carlito/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "carlito/plugin-sdk/reply-history";
import { resolveAgentRoute } from "carlito/plugin-sdk/routing";
import { logVerbose } from "carlito/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "carlito/plugin-sdk/runtime-env";
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("carlito channels login --channel web")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("carlito channels login --channel web")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
