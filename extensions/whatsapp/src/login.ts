...
import { defaultRuntime, type RuntimeEnv } from "carlito/plugin-sdk/runtime-env";
import { logInfo } from "carlito/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      console.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("carlito channels login")} and scan the QR again.`,
        ),
      );
