...
function createCfg(): CarlitoConfig {
  return {
    channels: {
      discord: {
        enabled: true,
...
function resolveAccount(cfg: CarlitoConfig, accountId = "default"): ResolvedDiscordAccount {
  return discordPlugin.config.resolveAccount(cfg, accountId);
}

function startDiscordAccount(cfg: CarlitoConfig, accountId = "default") {
