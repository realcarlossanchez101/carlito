...
import type { DiscordGuildEntry, CarlitoConfig } from "carlito/plugin-sdk/config-runtime";
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "carlito/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "carlito/plugin-sdk/setup-runtime";
import { formatDocsLink } from "carlito/plugin-sdk/setup-tools";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: CarlitoConfig; accountId: string }) => {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: CarlitoConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: CarlitoConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: CarlitoConfig; accountId: string }) =>
    ...
    disable: (cfg: CarlitoConfig) => setSetupChannelEnabled(cfg, channel, false),
