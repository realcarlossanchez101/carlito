...
function createThreadBinding(
  overrides?: Partial<import("carlito/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("carlito/plugin-sdk/conversation-runtime").SessionBindingRecord;
}
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("carlito/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("carlito/plugin-sdk/config-runtime").CarlitoConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "hi <@carlito-bot>",
      mentionedUsers: [{ id: "carlito-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
        username: "Relay",
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@carlito-bot> /new incident room",
      mentionedUsers: [{ id: "carlito-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey carlito");
