...
import { AcpRuntimeError } from "carlito/plugin-sdk/acp-runtime";
...
let runtimeEnvModule: typeof import("carlito/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): CarlitoConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("carlito/plugin-sdk/plugin-runtime", async () => {
      const actual = await vi.importActual<typeof import("carlito/plugin-sdk/plugin-runtime")>(
        "carlito/plugin-sdk/plugin-runtime",
      );
      return {
        ...actual,
    ...
    runtimeEnvModule = await import("carlito/plugin-sdk/runtime-env");
  ...
  it("passes Carlito EventQueue defaults to Carbon Client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
