...
import { clearSessionStoreCacheForTest } from "carlito/plugin-sdk/config-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("carlito/plugin-sdk/conversation-binding-runtime");
...
vi.mock("carlito/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("carlito/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("carlito/plugin-sdk/conversation-binding-runtime")>(
        "carlito/plugin-sdk/conversation-binding-runtime",
      ),
  );
});
...
vi.mock("carlito/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(os.tmpdir(), `carlito-discord-think-autocomplete-${process.pid}.json`);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("carlito/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("carlito/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("carlito/plugin-sdk/command-auth");
