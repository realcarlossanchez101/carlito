...
vi.mock("carlito/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("carlito/plugin-sdk/reply-dispatch-runtime")>(
...
vi.mock("carlito/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): CarlitoConfig {
  return {
    commands: {
...
describe("discord native /status", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("carlito/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithDispatcher,
    );
