...
import { sleepWithAbort } from "carlito/plugin-sdk/runtime-env";
import {
  createComputedAccountStatusAdapter,
  createDefaultChannelRuntimeState,
} from "carlito/plugin-sdk/status-helpers";
import { resolveTargetsWithOptionalToken } from "carlito/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: CarlitoConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Carlito adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
