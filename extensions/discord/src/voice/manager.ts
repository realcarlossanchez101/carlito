...
import type { DiscordAccountConfig, TtsConfig } from "carlito/plugin-sdk/config-runtime";
...
import { parseTtsDirectives } from "carlito/plugin-sdk/speech";
...
function resolveVoiceTtsConfig(params: { cfg: CarlitoConfig; override?: TtsConfig }): {
  cfg: CarlitoConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredCarlitoTmpDir(), "discord-voice-"));
