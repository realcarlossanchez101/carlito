...
export function shouldShowTelegramDmAccessWarning(cfg: CarlitoConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`carlito config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`carlito config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
