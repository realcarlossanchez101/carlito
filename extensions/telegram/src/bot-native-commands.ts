import { resolveChannelStreamingBlockEnabled } from "carlito/plugin-sdk/channel-streaming";
import {
...
import { getRuntimeConfigSnapshot } from "carlito/plugin-sdk/runtime-config-snapshot";
...
type TelegramChunkMode = ReturnType<
  typeof import("carlito/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("carlito/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): CarlitoConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: CarlitoConfig): TelegramAccountConfig => {
    try {
