...
import type { GetReplyOptions, MsgContext } from "carlito/plugin-sdk/reply-runtime";
...
type LoadConfigFn = typeof import("carlito/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("carlito/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("carlito/plugin-sdk/config-runtime").resolveStorePath;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/carlito-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: CarlitoConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: CarlitoConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: CarlitoConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: CarlitoConfig) => {
