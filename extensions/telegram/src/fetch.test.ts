...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the Carlito debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("CARLITO_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("CARLITO_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("keeps resolver-scoped transport policy for Carlito proxy fetches", async () => {
