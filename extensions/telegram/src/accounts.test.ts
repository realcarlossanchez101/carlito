...
import * as runtimeEnvModule from "carlito/plugin-sdk/runtime-env";
import { withEnv } from "carlito/plugin-sdk/testing";
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", CARLITO_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): CarlitoConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): CarlitoConfig => ({
    channels: {
      telegram: {
