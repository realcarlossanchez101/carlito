...
function normalizeDenyList(cfg: CarlitoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CarlitoPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: CarlitoPluginApi) {
    ...
    const timerService: CarlitoPluginService = {
