...
```bash
export CARLITO_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.carlito/carlito.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env CARLITO_PARALLELS_DISCORD_TOKEN \
...
- Use the `carlito` wrapper for guest `message send/read`; `node carlito.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/carlito-parallels-smoke.*`
...
- host inbound nonce appears in `carlito message read` output
