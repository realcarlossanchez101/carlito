---
name: carlito-pr-maintainer
description: Review, triage, close, label, comment on, or land Carlito PRs/issues with maintainer evidence checks.
---

# Carlito PR Maintainer
...
- Use `--repo carlito/carlito` with `--match title,body` first.
...
```bash
gh search prs --repo carlito/carlito --match title,body --limit 50 -- "auto-update"
gh search issues --repo carlito/carlito --match title,body --limit 50 -- "auto-update"
gh search issues --repo carlito/carlito --match title,body --limit 50 \
