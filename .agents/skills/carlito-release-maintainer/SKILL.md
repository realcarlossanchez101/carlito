---
name: carlito-release-maintainer
description: Prepare or verify Carlito stable/beta releases, changelogs, release notes, publish commands, and artifacts.
---

# Carlito Release Maintainer
...
- Core `carlito` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/carlito/carlito/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `carlito` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `carlito YYYY.M.D-beta.N`
...
Use the Carlito account's existing release-post style:

- Format: `Carlito YYYY.M.D 🦞` or `🦞 Carlito YYYY.M.D is live`, blank line,
  then 3-4 emoji-led bullets, blank line, one short punchline, then the release
  link.
- For beta: say `Carlito YYYY.M.D-beta.N 🦞` or `Carlito YYYY.M.D beta N is
...
```bash
  CARLITO_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/carlito-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `CARLITO_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `CARLITO_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the Carlito npm release path before creating the tag.
...
- Full pre-npm beta test roster:
  ...
  - all Parallels install/update tests:
    ...
    rerun lanes from `carlito-parallels-smoke`
...
- Post-published beta verification roster:
  - `node --import tsx scripts/carlito-npm-postpublish-verify.ts <beta-version>`
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `CARLITO_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  ...
  - `180m` for local full QA live OpenAI + Anthropic rosters when explicitly
    ...
    `QA-Lab - All Lanes`
  - Parallels caps from the `carlito-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `carlito update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `carlito/carlito` and the real mac preflight in
  `carlito/releases-private` for every release.
...
- Carlito publish uses GitHub trusted publishing.
...
- If the private dist-tag workflow cannot promote because `NPM_TOKEN` is absent
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add carlito@YYYY.M.D latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    `npm view carlito dist-tags --json --prefer-online --cache /tmp/carlito-npm-cache-verify-$$`
    and `npm view carlito@latest version dist.tarball --json --prefer-online`.
...
- `.github/workflows/macos-release.yml` in `carlito/carlito` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `carlito/releases-private/.github/workflows/carlito-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `carlito/releases-private/.github/workflows/carlito-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@realcarlossanchez101/carlito-release-managers` before publish continues.
- Mac publish uses
  `carlito/releases-private/.github/workflows/carlito-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `carlito/carlito`
  automatically when `CARLITO_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the Carlito package
...
- `@carlito/*` plugin publishes use a separate maintainer-only flow.
...
14. Dispatch Actions > `QA-Lab - All Lanes` against the release tag and wait
    ...
    lanes to pass.
15. Start `.github/workflows/carlito-npm-release.yml` from the release branch
...
16. For stable releases, start `.github/workflows/macos-release.yml` in
    `carlito/carlito` and wait for the public validation-only run to pass.
...
20. Start `.github/workflows/carlito-npm-release.yml` from the same branch with
    the same tag for the real publish, choose `npm_dist_tag` (`beta` default,
    `latest` only when you intentionally want direct stable publish), keep it
    the same as the preflight run, and pass the successful npm
    `preflight_run_id`.
21. Wait for `npm-release` approval from `@realcarlossanchez101/carlito-release-managers`.
...
- Use `carlito-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
