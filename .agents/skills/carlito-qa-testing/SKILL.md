---
name: carlito-qa-testing
description: Run, watch, debug, extend, or explain Carlito qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
CARLITO_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
...
5. If the user wants to watch the live UI, find the current `carlito-qa` listen port and report `http://127.0.0.1:<port>`.
...
```bash
pnpm carlito qa character-eval \
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Carlito identity combines with the character.
...
```bash
pnpm carlito qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `CARLITO_HOME` sandboxed.
