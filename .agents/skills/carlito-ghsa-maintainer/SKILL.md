---
name: carlito-ghsa-maintainer
description: Inspect, patch, validate, publish, or confirm Carlito GHSA security advisories and private-fork state.
---

# Carlito GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `carlito-release-maintainer`.
...
```bash
fork=$(gh api /repos/carlito/carlito/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/carlito/carlito/security-advisories/<GHSA> \
