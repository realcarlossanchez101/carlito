...
function createAssistantCanvasBlock(params: {
  ...
  const url = params.url ?? `/__carlito__/canvas/documents/${viewId}/index.html`;
...
function clearDeleteConfirmSkip() {
  try {
    getSafeLocalStorage()?.removeItem("carlito:skipDeleteConfirm");
  } catch {
    /* noop */
  }
}
...
describe("grouped chat rendering", () => {
  it("falls back to the logo while authenticated avatar routes are loading", () => {
    ...
    expect(img?.getAttribute("src")).toBe("/carlito-logo.svg");
  });
  ...
  it("falls back to the local logo when the assistant avatar is a remote URL", () => {
    ...
    expect(avatar?.getAttribute("src")).toBe("/carlito-logo.svg");
  });
  ...
  it("renders allowed transcript images and skips blocked/non-image media", () => {
    const renderUserMedia = (message: unknown) => {
      ...
      renderGroupedMessage(container, message, "user", {
        ...
        basePath: "/carlito",
        assistantAttachmentAuthToken: "session-token",
        localMediaPreviewRoots: ["/tmp/carlito"],
      });
      return container;
    };
    ...
    let container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/carlito/user-upload.png",
      timestamp: Date.now(),
    });
    expect(
      container.querySelector<HTMLImageElement>(".chat-message-image")?.getAttribute("src"),
    ).toBe(
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Fuser-upload.png&token=session-token",
    );
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPaths: ["/tmp/carlito/first.png", "/tmp/carlito/second.jpg"],
    ...
    expect(
      ...
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Ffirst.png&token=session-token",
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Fsecond.jpg&token=session-token",
    ]);
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/carlito/user-upload.pdf",
  ...
  it("fetches managed chat images with auth and renders blob previews", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      ...
      expect(headers.get("x-carlito-requester-session-key")).toBe("agent:main:main");
      return {
        ok: true,
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/carlito/test image.png\nMEDIA:/tmp/carlito/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Ftest+image.png&token=session-token&meta=1",
    ...
    expect(image?.getAttribute("src")).toBe(
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Ftest+image.png&token=session-token",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Ftest-doc.pdf&token=session-token",
    );
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Ftest+image.png&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/carlito/__carlito__/assistant-media?source=%2Ftmp%2Fcarlito%2Ftest+image.png&token=fresh-token&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(container.querySelector(".chat-message-image")).not.toBeNull();
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__carlito__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__carlito__/media/test-doc.pdf");
    expect(container.textContent).not.toContain("Unavailable");
  });
    ...
    const cases = [
      renderCase({
        ...
        message: {
          ...
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/carlito/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/carlito/__carlito__/assistant-media?source=%2FC%3A%2Ftmp%2Fcarlito%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/carlito/__carlito__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/carlito/__carlito__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__carlito__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasHostUrl: "http://127.0.0.1:19003/__carlito__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__carlito__/cap/cap_123/__carlito__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__carlito__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
            rawText: JSON.stringify({
              kind: "canvas",
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
    ...
    expect(iframe?.getAttribute("src")).toBe(
      "/__carlito__/canvas/documents/cv_inline_default/index.html",
    );
        ...
        createMessageGroup(
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__carlito__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
