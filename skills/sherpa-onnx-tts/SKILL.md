...
1. Download the runtime for your OS (extracts into `$CARLITO_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.carlito/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$CARLITO_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.carlito/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Carlito config file (`$CARLITO_CONFIG_PATH`, default `~/.carlito/carlito.json`):
