...
**⚠️ CRITICAL: Never review PRs in Carlito's own project folder!**
...
8. **NEVER start Codex inside your Carlito state directory** (`$CARLITO_STATE_DIR`, default `~/.carlito`) - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/carlito/** - that's the LIVE Carlito instance!
...
For long-running background tasks, append a wake trigger to your prompt so Carlito gets notified immediately when the agent finishes (instead of waiting for the next pulsecheck):

```
... your task here.

When completely finished, run this command to notify me:
carlito system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: carlito system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
