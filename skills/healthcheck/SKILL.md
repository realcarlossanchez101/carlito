---
name: healthcheck
description: Audit and harden hosts running Carlito for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# Carlito Host Hardening

## Overview

Assess and harden the host running Carlito, then align it to a user-defined risk tolerance without breaking access. Use Carlito security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Carlito changes the host firewall, SSH, or OS updates; it does not.
...
5. Carlito gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Carlito can access sensitive data.
...
### 2) Run Carlito security audits (read-only)

As part of the default read-only checks, run `carlito security audit --deep`. Only offer alternatives if the user requests them:

1. `carlito security audit` (faster, non-probing)
2. `carlito security audit --json` (structured output)

Offer to apply Carlito safe defaults (numbered):

1. `carlito security audit --fix`

Be explicit that `--fix` only tightens Carlito defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Carlito version/update status (read-only)

As part of the default read-only checks, run `carlito update status`.
...
- Credential hygiene notes (location of Carlito creds, prefer disk encryption)
...
After Carlito install or first hardening pass, run at least one baseline audit and version check:

- `carlito security audit`
- `carlito security audit --deep`
- `carlito update status`

Ongoing monitoring is recommended. Use the Carlito cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `carlito cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `carlito update status`
...
Before creating, `carlito cron list` and match on exact `name`. If found, `carlito cron edit <id> ...`.
If not found, `carlito cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `carlito update status` (preferred for source checkouts and channels)
2. `npm view carlito version` (published npm version)

## Carlito command accuracy

Use only supported commands and flags:

- `carlito security audit [--deep] [--fix] [--json]`
- `carlito status` / `carlito status --deep`
- `carlito health --json`
- `carlito update status`
- `carlito cron add|list|runs|run`

Do not invent CLI flags or imply Carlito enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Carlito compaction:
