...
`carlito doctor` is the repair + migration tool for Carlito. It fixes stale
...
- Extra workspace dir detection (`~/carlito`).
...
doctor-style RPC methods, but they are **not** part of `carlito doctor` CLI
repair/migration.
...
you to run `carlito doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.carlito/carlito.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `carlito doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.carlito/sessions/` to `~/.carlito/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.carlito/agent/` to `~/.carlito/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.carlito/credentials/*.json` (except `oauth.json`)
  - to `~/.carlito/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `carlito doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.carlito/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.carlito` folders exist across
  home directories or when `CARLITO_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.carlito/carlito.json` is
...
that re-auth is required and prints the exact `carlito models auth login --provider ...`
command to run.
...
`carlito doctor --fix` / `carlito doctor --repair` mode. External plugins still
use `carlito plugins install` / `carlito plugins update`; doctor does not
install dependencies for arbitrary plugin paths.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the Carlito service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Carlito gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`carlito doctor` without `--fix`) this check
is skipped entirely.
...
- inspect pending requests with `carlito devices list`
- approve the exact request with `carlito devices approve <requestId>`
- rotate a fresh token with `carlito devices rotate --device <deviceId> --role <role>`
- remove and re-approve a stale record with `carlito devices remove <deviceId>`
...
- **Legacy workspace dirs**: warns when `~/carlito` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(carlito completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `carlito completion --write-state` to regenerate the cache manually.
...
- `carlito doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `carlito doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `carlito memory status --deep` to verify embedding readiness at runtime.
...
- `carlito doctor` prompts before rewriting supervisor config.
- `carlito doctor --yes` accepts the default repair prompts.
- `carlito doctor --repair` applies recommended fixes without prompts.
- `carlito doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `carlito gateway install --force`.
