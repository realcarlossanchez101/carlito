...
Carlito can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Carlito requires a shared password.
...
- `token` (default when `CARLITO_GATEWAY_TOKEN` is set)
- `password` (shared secret via `CARLITO_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Carlito verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Carlito only treats a request as Serve when it arrives from loopback with
...
Prefer `CARLITO_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
carlito gateway --tailscale serve
carlito gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Carlito to undo `tailscale serve`
