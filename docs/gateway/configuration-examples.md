---
summary: "Schema-accurate configuration examples for common Carlito setups"
read_when:
  - Learning how to configure Carlito
...
```json5
{
  agent: { workspace: "~/.carlito/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.carlito/carlito.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.carlito/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/carlito" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.carlito/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
