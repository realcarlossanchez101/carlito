...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Carlito runs”).
...
- `carlito health` and `carlito status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `carlito gateway status`, `carlito gateway health`, `carlito gateway probe`, and `carlito gateway call` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`CARLITO_GATEWAY_PORT`).
...
- URL override safety:
  ...
  - Env URL overrides (`CARLITO_GATEWAY_URL`) may use env credentials only (`CARLITO_GATEWAY_TOKEN` / `CARLITO_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `CARLITO_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `CARLITO_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `CARLITO_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `CARLITO_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `CARLITO_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `CARLITO_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
```bash
carlito config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.carlito.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.carlito.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.carlito.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

Note: if you have a leftover `com.carlito.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
...
```bash
launchctl kickstart -k gui/$UID/ai.carlito.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.carlito.ssh-tunnel
```
