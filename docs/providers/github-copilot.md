---
summary: "Sign in to GitHub Copilot from Carlito using the device flow"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `carlito models auth login-github-copilot` flow
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. Carlito can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in Carlito

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Carlito runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        carlito models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        carlito models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Carlito talks to
...
```bash
# Skip confirmation
carlito models auth login-github-copilot --yes

# Login and set the default model in one step
carlito models auth login --provider github-copilot --method device --set-default
```
...
<AccordionGroup>
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Carlito
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Environment variable resolution order">
    Carlito resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Carlito uses the highest-priority one.
    The device-login flow (`carlito models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Carlito runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Carlito can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Carlito discovers available
...
1. Carlito resolves your GitHub token (from env vars or auth profile).
...
available, Carlito skips Copilot and tries the next provider.
