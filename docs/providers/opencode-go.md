...
Carlito sources the Go catalog from the bundled pi model registry. Run
`carlito models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    Carlito handles per-model routing automatically when the model ref uses
