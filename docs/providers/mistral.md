---
summary: "Use Mistral models and Voxtral transcription with Carlito"
read_when:
  - You want to use Mistral models in Carlito
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

Carlito supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    carlito onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    carlito onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    carlito models list --provider mistral
    ```
  </Step>
</Steps>
...
Carlito currently ships this bundled Mistral catalog:
...
<Note>
Carlito defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    Carlito maps the session **thinking** level to Mistral's API:

    | Carlito thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
