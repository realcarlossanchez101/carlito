---
summary: "Use Synthetic's Anthropic-compatible API in Carlito"
read_when:
...
Carlito registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    carlito onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Carlito's Anthropic client appends `/v1` to the base URL automatically, so use
...
`carlito models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Carlito appends `/v1` automatically.
