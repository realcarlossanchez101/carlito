...
title: "Groq"
read_when:
  - You want to use Groq with Carlito
  - You need the API key env var or CLI auth choice
---

[Groq](https://groq.com) provides ultra-fast inference on open-source models
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Carlito connects
...
Groq's model catalog changes frequently. Run `carlito models list | grep groq`
...
<Tip>
Use `carlito models list --provider groq` for the most up-to-date list of
...
media-understanding provider, Carlito uses Groq's `whisper-large-v3-turbo`
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    gateway processes. Use `~/.carlito/.env` or `env.shellEnv` config for
