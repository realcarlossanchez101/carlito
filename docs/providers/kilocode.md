---
summary: "Use Kilo Gateway's unified API to access many models in Carlito"
title: "Kilocode"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Carlito
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    carlito onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    carlito models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Carlito treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Carlito.
</Note>

## Built-in catalog

Carlito dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Carlito queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Carlito keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Carlito falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.carlito/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Carlito configuration reference.
  </Card>
