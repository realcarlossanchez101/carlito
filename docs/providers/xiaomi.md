---
summary: "Use Xiaomi MiMo models with Carlito"
read_when:
  - You want Xiaomi MiMo models in Carlito
  - You need XIAOMI_API_KEY setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. Carlito uses the Xiaomi
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    carlito onboard --auth-choice xiaomi-api-key
    ```

    Or pass the key directly:

    ```bash
    carlito onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    carlito models list --provider xiaomi
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.carlito/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.carlito/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
