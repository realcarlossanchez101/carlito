---
summary: "Host Carlito on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Carlito on Oracle Cloud
  - Looking for free VPS hosting for Carlito
  - Want 24/7 Carlito on a small server
title: "Oracle Cloud"
---

Run a persistent Carlito Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@carlito`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=carlito --reset` to re-authenticate.
