---
summary: "Optional Docker-based setup and onboarding for Carlito"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Carlito on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export CARLITO_IMAGE="ghcr.io/carlito/carlito:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/realcarlossanchez101/carlito/pkgs/container/carlito).
  ...
  <Step title="Complete onboarding">
    ...
    `carlito-gateway` directly. `carlito-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm carlito-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm carlito-cli channels login

    # Telegram
    docker compose run --rm carlito-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm carlito-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t carlito:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node carlito-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `CARLITO_EXTRA_MOUNTS`
or `CARLITO_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `carlito-cli` shares `carlito-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d carlito-gateway`, run onboarding
and setup-time config writes through `carlito-gateway` with
...
| Variable                      | Purpose                                                         |
| ----------------------------- | --------------------------------------------------------------- |
| `CARLITO_IMAGE`               | Use a remote image instead of building locally                  |
| `CARLITO_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)       |
| `CARLITO_EXTENSIONS`          | Pre-install plugin deps at build time (space-separated names)   |
| `CARLITO_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`) |
| `CARLITO_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                   |
| `CARLITO_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)          |
| `CARLITO_DOCKER_SOCKET`       | Override Docker socket path                                     |
...
```bash
docker compose exec carlito-gateway node dist/index.js health --token "$CARLITO_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `CARLITO_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `CARLITO_CONFIG_DIR` to `/home/node/.carlito` and
`CARLITO_WORKSPACE_DIR` to `/home/node/.carlito/workspace`, so those paths
survive container replacement.

That mounted config directory is where Carlito keeps:

- `carlito.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `CARLITO_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/carlito/`.
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export CARLITO_SANDBOX=1
    export CARLITO_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm carlito-cli gateway probe
    docker compose run -T --rm carlito-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `carlito-cli` uses `network_mode: "service:carlito-gateway"` so CLI
    ...
    `no-new-privileges` on `carlito-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.carlito`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/carlito-config /path/to/carlito-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export CARLITO_HOME_VOLUME="carlito_home"`
    2. **Bake system deps**: `export CARLITO_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `CARLITO_HOME_VOLUME` or `CARLITO_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/realcarlossanchez101/carlito/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Carlito runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm carlito-cli dashboard --no-open
    docker compose run --rm carlito-cli devices list
    docker compose run --rm carlito-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm carlito-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm carlito-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Carlito up to date
