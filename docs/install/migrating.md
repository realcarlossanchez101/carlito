---
summary: "Move (migrate) an Carlito install from one machine to another"
read_when:
  - You are moving Carlito to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration guide"
---

# Migrating Carlito to a New Machine

This guide moves an Carlito gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.carlito/` by default) and your **workspace**, you preserve:

- **Config** -- `carlito.json` and all gateway settings
...
<Tip>
Run `carlito status` on the old machine to confirm your state directory path.
Custom profiles use `~/.carlito-<profile>/` or a path set via `CARLITO_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf carlito-state.tgz .carlito
    ```

    If you use multiple profiles (e.g. `~/.carlito-work`), archive each separately.

  </Step>

  <Step title="Install Carlito on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.carlito/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `CARLITO_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `carlito doctor`.
  </Accordion>

  <Accordion title="Copying only carlito.json">
...
- [ ] `carlito status` shows the gateway running
