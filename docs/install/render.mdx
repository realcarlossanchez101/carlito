---
summary: "Deploy Carlito on Render with Infrastructure-as-Code"
read_when:
  - Deploying Carlito to Render
...
Deploy Carlito on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/realcarlossanchez101/carlito)
...
repository configures everything needed to run Carlito:
...
services:
  - type: web
    ...
    envVars:
      - key: CARLITO_GATEWAY_PORT
        value: "8080"
      - key: CARLITO_STATE_DIR
        value: /data/.carlito
      - key: CARLITO_WORKSPACE_DIR
        value: /data/workspace
      - key: CARLITO_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Carlito state
...
`CARLITO_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Carlito repository, Render will not auto-deploy your Carlito. To update it, run a manual Blueprint sync from the dashboard.
...
For Carlito, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Carlito state plus any configured
...
- Missing `CARLITO_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `CARLITO_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `carlito backup create` in the Render shell.
