---
summary: "Install Carlito — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Carlito, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://carlito.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Carlito and Node kept under a local prefix such as
`~/.carlito`, without depending on a system-wide Node install:

```bash
curl -fsSL https://carlito.ai/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g carlito@latest
```
...
Or skip the link and use `pnpm carlito ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:carlito/carlito#main
```
...
```bash
carlito --version      # confirm the CLI is available
carlito doctor         # check for config issues
carlito gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `carlito onboard --install-daemon` or `carlito gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Carlito on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Carlito completely.
  </Card>
</CardGroup>

## Troubleshooting: `carlito` not found

If the install succeeded but `carlito` is not found in your terminal:
