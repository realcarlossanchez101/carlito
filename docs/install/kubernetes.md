---
summary: "Deploy Carlito Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Carlito on a Kubernetes cluster
  - You want to test Carlito in a Kubernetes environment
title: "Kubernetes"
---

# Carlito on Kubernetes

A minimal starting point for running Carlito on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Carlito is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/carlito 18789:18789 -n carlito
...
```bash
kubectl get secret carlito-secrets -n carlito -o jsonpath='{.data.CARLITO_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: carlito (configurable via CARLITO_NAMESPACE)
├── Deployment/carlito        # Single pod, init container + gateway
├── Service/carlito           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/carlito-config  # carlito.json + AGENTS.md
└── Secret/carlito-secrets    # Gateway token + API keys
```
...
Edit `carlito.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret carlito-secrets -n carlito \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/carlito -n carlito
```

### Custom namespace

```bash
CARLITO_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/carlito/carlito:latest # or pin to a specific version from https://github.com/realcarlossanchez101/carlito/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # carlito.json + AGENTS.md
