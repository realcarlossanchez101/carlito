---
summary: "OpenProse: .prose workflows, slash commands, and state in Carlito"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In Carlito it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
carlito plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `carlito plugins install ./path/to/local/open-prose-plugin`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses Carlito tools under the hood.
...
## Carlito runtime mapping

OpenProse programs map to Carlito primitives:

| OpenProse concept         | Carlito tool     |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use Carlito tool allowlists and approval gates to control side effects.
