...
Carlito supports **camera capture** for agent workflows:
...
```bash
carlito nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
carlito nodes camera snap --node <id> --facing front
carlito nodes camera clip --node <id> --duration 3000
carlito nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`carlito.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `carlito` CLI to invoke camera commands on the macOS node.

Examples:

```bash
carlito nodes camera list --node <id>            # list camera ids
carlito nodes camera snap --node <id>            # prints MEDIA:<path>
carlito nodes camera snap --node <id> --max-width 1280
carlito nodes camera snap --node <id> --delay-ms 2000
carlito nodes camera snap --node <id> --device-id <id>
carlito nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
carlito nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
carlito nodes camera clip --node <id> --device-id <id>
carlito nodes camera clip --node <id> --no-audio
```

Notes:

- `carlito nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
carlito nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
