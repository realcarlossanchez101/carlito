...
- On completion, Carlito best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.
- For manual spawns, delivery is resilient:
  - Carlito tries direct `agent` delivery first with a stable idempotency key.
...
- Completion delivery keeps the resolved requester route:
  ...
  - if the completion origin only provides a channel, Carlito fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, Carlito uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. Carlito creates or binds a thread to that session target in the active channel.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, Carlito falls back to that session's requester when available
...
- Sub-agent context only injects `AGENTS.md` + `TOOLS.md` (no `SOUL.md`, `IDENTITY.md`, `USER.md`, `PULSECHECK.md`, or `BOOTSTRAP.md`).
