...
`carlito agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    carlito agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    carlito agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    carlito agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    carlito agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    carlito agent --agent ops --message "Generate report" \
...
carlito agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
