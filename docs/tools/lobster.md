---
summary: "Typed workflow runtime for Carlito with resumable approval gates."
title: Lobster
read_when:
...
Lobster is a workflow shell that lets Carlito run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.

Lobster is one authoring layer above detached background work. For flow orchestration above individual tasks, see [Task Flow](/automation/taskflow) (`carlito tasks flow`). For the task activity ledger, see [`carlito tasks`](/automation/tasks).
...
- **One call instead of many**: Carlito runs one Lobster tool call and gets a structured result.
...
Carlito runs Lobster workflows **in-process** using an embedded runner. No external CLI subprocess is spawned; the workflow engine executes inside the gateway process and returns a JSON envelope directly.
...
gog.gmail.search --query 'newer_than:1d' \
  | carlito.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
carlito.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Carlito tool calls, set `pipeline` to the file path.
...
If you need the standalone Lobster CLI for development or external pipelines, install it from the [Lobster repo](https://github.com/carlito/lobster) and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Carlito keeps core tools enabled. To restrict core
...
→ carlito calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ carlito calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Carlito tools that do.
