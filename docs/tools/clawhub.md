---
summary: "ClawHub guide: public registry, native Carlito install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Carlito skills and plugins**.

- Use native `carlito` commands to search/install/update skills and install
...
```bash
carlito plugins install clawhub:<package>
...
Native `carlito` commands install into your active workspace and persist source
...
`carlito plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, Carlito stops and points you at
`carlito skills install <slug>` instead.

## What ClawHub is

- A public registry for Carlito skills and plugins.
...
4. Users browse, download, and install skills in Carlito.
...
If you want to add new capabilities to your Carlito agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `carlito skills install <skill-slug>`
3. Start a new Carlito session so it picks up the new skill.
...
## How it fits into Carlito

Native `carlito skills install` installs into the active workspace `skills/`
directory. `carlito plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but Carlito warns
...
current working directory. If an Carlito workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Carlito loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.carlito/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Carlito how to perform a
...
Interested in becoming a moderator? Ask in the Carlito Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Carlito workspace).
...
Code plugins must include the required Carlito metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/carlito/skills` and `~/.carlito/skills`). This is designed to find older skill installs without extra flags.
