...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.carlito/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Carlito picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    carlito agent --message "give me a greeting"
    ```
...
| `metadata.carlito.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.carlito.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.carlito.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `carlito agent --message "..."` to test before sharing
...
| `~/.carlito/skills/`            | Medium     | Shared (all agents)   |
| Bundled (shipped with Carlito)  | Low        | Global                |
