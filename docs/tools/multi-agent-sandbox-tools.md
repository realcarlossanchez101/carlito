...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `carlito sandbox explain`.
...
Legacy `agent.*` configs are migrated by `carlito doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=carlito-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CARLITO_STATE_DIR:-$HOME/.carlito}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
