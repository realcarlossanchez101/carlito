...
Carlito supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Carlito also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `carlito configure --section web`. It stores the key in
`~/.carlito/carlito.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.carlito/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
