...
- `gateway`/`node` approvals are controlled by `~/.carlito/exec-approvals.json`.
...
- Carlito sets `CARLITO_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.carlito/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#no-approval-yolo-mode).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Carlito prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`carlito security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `carlito doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`carlito security audit` and `carlito doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
