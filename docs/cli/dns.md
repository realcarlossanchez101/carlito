---
summary: "CLI reference for `carlito dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: carlito.internal)
title: "DNS"
---
...
```bash
carlito dns setup
carlito dns setup --domain carlito.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `carlito.internal`)
...
- zone file path
- current tailnet IPs
- recommended `carlito.json` discovery config
...
- If `--domain` is omitted, Carlito uses `discovery.wideArea.domain` from config.
