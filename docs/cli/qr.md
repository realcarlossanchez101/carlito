---
summary: "CLI reference for `carlito qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
carlito qr
carlito qr --setup-code-only
carlito qr --json
carlito qr --remote
carlito qr --url wss://gateway.example/ws
```
...
- With `--remote`, Carlito requires either `gateway.remote.url` or
