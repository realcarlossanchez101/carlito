---
summary: "CLI reference for `carlito onboard` (interactive onboarding)"
read_when:
...
```bash
carlito onboard
carlito onboard --flow quickstart
carlito onboard --flow manual
carlito onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`CARLITO_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `carlito gateway run`.
...
- Fastest first chat: `carlito dashboard` (Control UI, no channel setup).
