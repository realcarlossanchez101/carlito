---
summary: "CLI reference for `carlito health` (gateway health snapshot via RPC)"
read_when:
...
carlito health
carlito health --json
carlito health --timeout 2500
carlito health --verbose
carlito health --debug
```

Notes:

- Default `carlito health` asks the running gateway for its health snapshot. When the
