...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Carlito Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`carlito acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Carlito channel
conversations instead of hosting an ACP harness session, use
[`carlito mcp serve`](/cli/mcp) instead.
...
- Carlito acts as an ACP server
- an IDE or ACP client connects to Carlito
- Carlito forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Carlito runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Carlito: use `carlito acp`
- Carlito should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Carlito gateway or agent instead.                                                                                                                                      |
...
# Remote Gateway
carlito acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
carlito acp --url wss://gateway-host:18789 --token-file ~/.carlito/gateway.token

# Attach to an existing session key
carlito acp --session agent:main:main

# Attach by label (must already exist)
carlito acp --session-label "support inbox"

# Reset the session key before the first prompt
carlito acp --session agent:main:main --reset-session
```
...
carlito acp client --server-args --url wss://gateway-host:18789 --token-file ~/.carlito/gateway.token

# Override the server command (default: carlito)
carlito acp client --server "node" --server-args carlito.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Carlito through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Carlito Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `carlito acp` over stdio.

Example config (persisted):

```bash
carlito config set gateway.remote.url wss://gateway-host:18789
carlito config set gateway.remote.token <token>
```
...
```bash
carlito acp --session agent:main:main
carlito acp --session agent:design:main
carlito acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Carlito plugin tools or selected
...
[Carlito tools MCP bridge](/tools/acp-agents#carlito-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Carlito bot over ACP, use `acpx` with its built-in `carlito` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx carlito` at `carlito acp`.
3. Target the Carlito session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Carlito ACP session
acpx carlito exec "Summarize the active Carlito session state."

# Persistent named session for follow-up turns
acpx carlito sessions ensure --name codex-bridge
acpx carlito -s codex-bridge --cwd /path/to/repo \
  "Ask my Carlito work agent for recent context relevant to this repo."
```

If you want `acpx carlito` to target a specific Gateway and session key every
time, override the `carlito` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "carlito": {
      "command": "env CARLITO_HIDE_BANNER=1 CARLITO_SUPPRESS_NOTES=1 carlito acp --url ws://127.0.0.1:18789 --token-file ~/.carlito/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Carlito checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env CARLITO_HIDE_BANNER=1 CARLITO_SUPPRESS_NOTES=1 node carlito.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Carlito agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Carlito ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`CARLITO_GATEWAY_TOKEN`, `CARLITO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`CARLITO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `CARLITO_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `carlito acp client` sets `CARLITO_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `carlito`).
