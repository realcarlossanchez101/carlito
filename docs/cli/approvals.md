---
summary: "CLI reference for `carlito approvals` and `carlito exec-policy`"
read_when:
...
`carlito exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.carlito/exec-approvals.json`

Examples:

```bash
carlito exec-policy show
carlito exec-policy show --json

carlito exec-policy preset yolo
carlito exec-policy preset cautious --json

carlito exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `carlito exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `carlito approvals set --gateway`
or `carlito approvals set --node <id|name|ip>`.

## Common commands

```bash
carlito approvals get
carlito approvals get --node <id|name|ip>
carlito approvals get --gateway
```

`carlito approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
carlito approvals set --file ./exec-approvals.json
carlito approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
carlito approvals set --node <id|name|ip> --file ./exec-approvals.json
carlito approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Carlito policy aligned, also set:
...
```bash
carlito approvals allowlist add "~/Projects/**/bin/rg"
carlito approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
carlito approvals allowlist add --agent "*" "/usr/bin/uname"

carlito approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `carlito nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.carlito/exec-approvals.json`.
