---
summary: "CLI reference for `carlito plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
carlito plugins list --verbose
carlito plugins list --json
carlito plugins install <path-or-spec>
carlito plugins inspect <id>
carlito plugins inspect <id> --json
carlito plugins inspect --all
carlito plugins info <id>
carlito plugins enable <id>
carlito plugins disable <id>
carlito plugins uninstall <id>
carlito plugins doctor
carlito plugins update <id-or-npm-spec>
carlito plugins update --all
carlito plugins marketplace list <marketplace>
carlito plugins marketplace list <marketplace> --json
```

Bundled plugins ship with Carlito. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native Carlito plugins must ship `carlito.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: carlito` or `Format: bundle`. Verbose list/info
...
carlito plugins install clawhub:<package>              # ClawHub only
carlito plugins install <package> --force              # overwrite existing install
carlito plugins install <package> --pin                # pin version
carlito plugins install <package> --dangerously-force-unsafe-install
carlito plugins install <path>                         # local path
carlito plugins install <plugin>@<marketplace>         # marketplace
carlito plugins install <plugin> --marketplace <name>  # marketplace (explicit)
...
If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `carlito.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

If config is invalid, `plugins install` normally fails closed and tells you to
run `carlito doctor --fix` first. The only documented exception is a narrow
...
If you run `plugins install` for a plugin id that is already installed, Carlito
...
override, while `carlito skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`carlito.hooks` in `package.json`. Use `carlito hooks` for filtered hook
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Carlito
...
```bash
carlito plugins install clawhub:carlito-codex-app-server
carlito plugins install clawhub:carlito-codex-app-server@1.2.3
```

Carlito now also prefers ClawHub for bare npm-safe plugin specs. It only falls
back to npm if ClawHub does not have that package or version:

```bash
carlito plugins install carlito-codex-app-server
```

Carlito downloads the package archive from ClawHub, checks the advertised
...
```bash
carlito plugins install <plugin-name> --marketplace <marketplace-name>
carlito plugins install <plugin-name> --marketplace <owner/repo>
carlito plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
carlito plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Carlito accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, Carlito auto-detects:

- native Carlito plugins (`carlito.plugin.json`)
...
```bash
carlito plugins install -l ./my-plugin
```
...
```bash
carlito plugins uninstall <id>
carlito plugins uninstall <id> --dry-run
...
carlito plugins update <id-or-npm-spec>
carlito plugins update --all
carlito plugins update <id-or-npm-spec> --dry-run
carlito plugins update @realcarlossanchez101/voice-call@beta
carlito plugins update carlito-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Carlito reuses the recorded install spec for that
...
or exact version. Carlito resolves that package name back to the tracked plugin
...
Before a live npm update, Carlito checks the installed package version against
...
downloading, reinstalling, or rewriting `carlito.json`.

When a stored integrity hash exists and the fetched artifact hash changes,
Carlito treats that as npm artifact drift. The interactive
`carlito plugins update` command prints the expected and actual hashes and asks
...
with `CARLITO_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in
