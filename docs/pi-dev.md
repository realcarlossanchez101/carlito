...
This guide summarizes a sane workflow for working on the pi integration in Carlito.
...
```bash
CARLITO_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm carlito agent --message "Hello" --thinking low`
...
State lives under the Carlito state directory. Default is `~/.carlito`. If `CARLITO_STATE_DIR` is set, use that directory instead.

To reset everything:

- `carlito.json` for config
