...
read_when:
  ...
  - Reviewing Carlito security posture
title: "Pairing"
---

“Pairing” is Carlito’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `carlito-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.carlito/credentials/`:
...
3. On your phone, open the Carlito iOS app → Settings → Gateway.
...
reconnects asking for more scopes or a broader role, Carlito keeps the
existing approval as-is and creates a fresh pending upgrade request. Use
`carlito devices list` to compare the currently approved access with the newly
requested access before you approve.

### Node pairing state storage

Stored under `~/.carlito/devices/`:
...
- The legacy `node.pair.*` API (CLI: `carlito nodes pending|approve|reject|rename`) is a
