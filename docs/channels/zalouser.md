...
read_when:
  - Setting up Zalo Personal for Carlito
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Carlito.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current Carlito releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `carlito plugins install @realcarlossanchez101/zalouser`
- Or from a source checkout: `carlito plugins install ./path/to/local/zalouser-plugin`
...
```bash
carlito directory self --channel zalouser
carlito directory peers list --channel zalouser --query "name"
carlito directory groups list --channel zalouser --query "work"
```
...
- `carlito pairing list zalouser`
- `carlito pairing approve zalouser <code>`
...
- On startup, Carlito resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Carlito stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Carlito state. Example:
...
- Carlito sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Carlito sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `carlito channels logout --channel zalouser && carlito channels login --channel zalouser`
...
- The channel now runs fully in Carlito without external CLI binaries.
