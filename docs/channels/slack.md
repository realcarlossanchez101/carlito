...
```json
{
  "display_information": {
    "name": "Carlito",
    "description": "Slack connector for Carlito"
  },
  "features": {
    "bot_user": { "display_name": "Carlito", "always_online": true },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/carlito",
        "description": "Send a message to Carlito",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `carlito pairing approve slack <code>`.
...
- If streaming fails mid-reply, Carlito falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Carlito is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are Carlito-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Carlito falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `carlito channels status --probe --json` shows `botTokenStatus` or
