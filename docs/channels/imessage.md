...
```bash
carlito pairing list imessage
carlito pairing approve imessage <CODE>
```
  ...
  <Tab title="Remote Mac over SSH">
    Carlito only requires a stdio-compatible `cliPath`, so you can point `cliPath` at a wrapper script that SSHes to a remote Mac and runs `imsg`.
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.carlito/scripts/imsg-ssh",
...
```
    ...
    If `remoteHost` is not set, Carlito attempts to auto-detect it by parsing the SSH wrapper script.
    `remoteHost` must be `host` or `user@host` (no spaces or SSH options).
    Carlito uses strict host-key checking for SCP, so the relay host key must already exist in `~/.ssh/known_hosts`.
...
- Full Disk Access is required for the process context running Carlito/`imsg` (Messages DB access).
...
<Tabs>
  ...
  <Tab title="Sessions and deterministic replies">
    ...
    If that `chat_id` is explicitly configured under `channels.imessage.groups`, Carlito treats it as group traffic (group gating + group session isolation).
...
<AccordionGroup>
  <Accordion title="Dedicated bot macOS user (separate iMessage identity)">
    ...
    4. Create SSH wrapper so Carlito can run `imsg` in that user context.
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`carlito pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs Carlito/`imsg`.
