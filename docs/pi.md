---
summary: "Architecture of Carlito's embedded Pi agent integration and session lifecycle"
title: "Pi integration architecture"
read_when:
  - Understanding Pi SDK integration design in Carlito
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

This document describes how Carlito integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Carlito uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Carlito directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Carlito's local TUI mode)                                              |
...
├── pi-tools.ts                    # createCarlitoCodingTools()
...
├── carlito-tools.ts              # Carlito-specific tools
...
Image injection is prompt-local: Carlito loads image refs from the current prompt and
...
2. **Custom Replacements**: Carlito replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Carlito Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Carlito's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Carlito CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Pulsechecks, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Carlito wraps this with `guardSessionManager()` for tool result safety.
...
Carlito maintains an auth profile store with multiple API keys per provider:
...
Carlito loads custom pi extensions for specialized behavior:
...
Carlito also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Carlito Embedded                                                                             |
| --------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                               |
| Tools           | Default coding tools    | Custom Carlito tool suite                                                                    |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                  |
| Session storage | `~/.pi/agent/sessions/` | `~/.carlito/agents/<agentId>/sessions/` (or `$CARLITO_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `CARLITO_LIVE_TEST=1`)
