...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Carlito
...
Running Carlito via Node with `tsx` fails at startup with:

```
[carlito] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `carlito status` and `gateway:watch` worked.
