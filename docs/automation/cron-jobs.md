...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into Carlito
...
# See run history
carlito cron runs --id <job-id>
```

## How cron works

- Cron runs **inside the Gateway** process (not inside the model).
- Job definitions persist at `~/.carlito/cron/jobs.json` so restarts do not lose schedules.
- Runtime execution state persists next to it in `~/.carlito/cron/jobs-state.json`. If you track cron definitions in git, track `jobs.json` and gitignore `jobs-state.json`.
- After the split, older Carlito versions can read `jobs.json` but may treat jobs as fresh because runtime fields now live in `jobs-state.json`.
...
together`, and similar hints) and no descendant subagent run is still
  responsible for the final answer, Carlito re-prompts once for the actual
...
This fires ~5–6 times per month instead of 0–1 times per month. Carlito uses Croner's default OR behavior here. To require both conditions, use Croner's `+` day-of-week modifier (`0 9 15 * +1`) or schedule on one field and guard the other in your job's prompt or command.
...
running, Carlito suppresses that partial parent update instead of announcing it.
...
agent sends to the configured/current target, Carlito skips the fallback
...
Wire Gmail inbox triggers to Carlito via Google PubSub.

**Prerequisites**: `gcloud` CLI, `gog` (gogcli), Carlito hooks enabled, Tailscale for the public HTTPS endpoint.

### Wizard setup (recommended)

```bash
carlito webhooks gmail setup --account carlito@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `CARLITO_SKIP_GMAIL_WATCHER=1` to opt out.
...
carlito cron show <jobId>

# Edit a job
carlito cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
carlito cron run <jobId>

# Run only if due
carlito cron run <jobId> --due

# View run history
carlito cron runs --id <jobId> --limit 50

# Delete a job
carlito cron remove <jobId>

# Agent selection (multi-agent setups)
carlito cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops
carlito cron edit <jobId> --clear-agent
```

Model override note:

- `carlito cron add|edit --model ...` changes the job's selected model.
...
Disable cron: `cron.enabled: false` or `CARLITO_SKIP_CRON=1`.
...
- Check `cron.enabled` and `CARLITO_SKIP_CRON` env var.
- Confirm the Gateway is running continuously.
- For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
- `reason: not-due` in run output means manual run was checked with `carlito cron run <jobId> --due` and the job was not due yet.
...
- If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`),
  Carlito suppresses direct outbound delivery and also suppresses the fallback
