...
Hooks are small scripts that run when something happens inside the Gateway. They can be discovered from directories and inspected with `carlito hooks`. The Gateway loads internal hooks only after you enable hooks or configure at least one hook entry, hook pack, legacy handler, or extra hook directory.

There are two kinds of hooks in Carlito:

- **Internal hooks** (this page): run inside the Gateway when agent events fire, like `/new`, `/reset`, `/stop`, or lifecycle events.
- **Webhooks**: external HTTP endpoints that let other systems trigger work in Carlito. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `carlito hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "carlito": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
**Metadata fields** (`metadata.carlito`):
...
1. **Bundled hooks**: shipped with Carlito
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.carlito/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
The Gateway skips internal hook discovery on startup until internal hooks are configured. Enable a bundled or managed hook with `carlito hooks enable <name>`, install a hook pack, or set `hooks.internal.enabled=true` to opt in. When you enable one named hook, the Gateway loads only that hook's handler; `hooks.internal.enabled=true`, extra hook directories, and legacy handlers opt into broad discovery.

### Hook packs

Hook packs are npm packages that export hooks via `carlito.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.carlito/logs/commands.log`   |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
carlito hooks enable <hook-name>
```
...
Logs every slash command to `~/.carlito/logs/commands.log`.
...
# Enable/disable
carlito hooks enable <hook-name>
carlito hooks disable <hook-name>
```
...
1. Verify the hook is enabled: `carlito hooks list`
