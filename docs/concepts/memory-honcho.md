...
[Honcho](https://honcho.dev) adds AI-native memory to Carlito. It persists
...
```bash
carlito plugins install @honcho-ai/carlito-honcho
carlito honcho setup
...
Settings live under `plugins.entries["carlito-honcho"].config`:
    ...
    entries: {
      "carlito-honcho": {
        config: {
          ...
          workspaceId: "carlito", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `carlito honcho setup` detects and
...
```bash
carlito honcho setup                        # Configure API key and migrate files
carlito honcho status                       # Check connection status
carlito honcho ask <question>               # Query Honcho about the user
carlito honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/carlito-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Carlito integration guide](https://docs.honcho.dev/v3/guides/integrations/carlito)
- [Memory](/concepts/memory) -- Carlito memory overview
