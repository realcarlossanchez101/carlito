...
alongside Carlito. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Carlito falls back to the
...
Carlito creates a self-contained QMD home under
`~/.carlito/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Carlito creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, Carlito retries with `qmd query`.
- If QMD fails entirely, Carlito falls back to the builtin SQLite engine.
...
process, so you can tune QMD globally without adding new Carlito config:
...
collection under `~/.carlito/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Carlito logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Carlito
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Carlito uses.
...
the next startup reconciliation. Carlito recreates stale managed collections
...
Carlito's builtin symlink rules. Keep temporary monorepo checkouts under
