---
summary: "How Carlito summarizes long conversations to stay within model limits"
read_when:
...
When a conversation approaches that limit, Carlito **compacts** older messages
...
When Carlito splits history into compaction chunks, it keeps assistant tool
calls paired with their matching `toolResult` entries. If a split point lands
inside a tool block, Carlito moves the boundary so the pair stays together and
...
Carlito compacts and retries). Typical overflow signatures include
...
<Info>
Before compacting, Carlito automatically reminds the agent to save important
notes to [memory](/concepts/memory) files. This prevents context loss.
</Info>

Use the `agents.defaults.compaction` setting in your `carlito.json` to configure compaction behavior (mode, target tokens, etc.).
...
Plugins can register a custom compaction provider via `registerCompactionProvider()` on the plugin API. When a provider is registered and configured, Carlito delegates summarization to it instead of the built-in LLM pipeline.
...
Setting a `provider` automatically forces `mode: "safeguard"`. Providers receive the same compaction instructions and identifier-preservation policy as the built-in path, and Carlito still preserves recent-turn and split-turn suffix context after provider output. If the provider fails or returns an empty result, Carlito falls back to built-in LLM summarization.

## Auto-compaction (default on)

When a session nears or exceeds the model’s context window, Carlito triggers auto-compaction and may retry the original request using the compacted context.
...
Before compaction, Carlito can run a **silent memory flush** turn to store
