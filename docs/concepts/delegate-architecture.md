---
summary: "Delegate architecture: running Carlito as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Carlito as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Carlito agent that:
...
Carlito's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Carlito's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.carlito/workspace-delegate",
  tools: {
...
- Cron run history: `~/.carlito/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.carlito/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Carlito's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
carlito agents add delegate
```

This creates:

- Workspace: `~/.carlito/workspace-delegate`
- State: `~/.carlito/agents/delegate/agent`
- Sessions: `~/.carlito/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.carlito/workspace" },
      {
        id: "delegate",
        workspace: "~/.carlito/workspace-delegate",
        tools: {
...
~/.carlito/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.carlito/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.carlito/workspace-org",
        agentDir: "~/.carlito/agents/org-assistant/agent",
...
recall view. Carlito redacts credential/token-like text, truncates long
