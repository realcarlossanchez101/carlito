...
The private QA stack is meant to exercise Carlito in a more realistic,
...
```bash
pnpm carlito qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`CARLITO_RUN_NODE_OUTPUT_LOG=<path>` to a repo-local log file.
...
disposable server. It requires `CARLITO_QA_TELEGRAM_GROUP_ID`,
`CARLITO_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
`CARLITO_QA_TELEGRAM_SUT_BOT_TOKEN`, plus two distinct bots in the same
...
```bash
pnpm carlito qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Carlito
...
- `mock-openai` is the scenario-aware Carlito mock. It remains the default
