...
- `/usage cost` in chats: local cost summary aggregated from Carlito session logs.
- CLI: `carlito status --usage` prints a full per-provider breakdown.
- CLI: `carlito channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Carlito treats
  ...
  quota, so Carlito inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Carlito prefers the
...
can supply plugin-specific usage auth logic; otherwise Carlito falls back to
