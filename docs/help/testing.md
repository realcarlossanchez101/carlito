...
Carlito has three Vitest suites (unit/integration, e2e, live) and a small set
...
- Linux VM-backed QA lane: `pnpm carlito qa suite --runner multipass --scenario channel-chat-baseline`
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `CARLITO_LIVE_MODEL_FILE_PROBE=0` or
    `CARLITO_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Carlito Scheduled Live And E2E Checks` and manual
    `Carlito Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes separate Docker live model
    matrix jobs sharded by provider.
  - For focused CI reruns, dispatch `Carlito Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/carlito-live-and-e2e-checks-reusable.yml` and its
    scheduled/release callers.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `carlito models list --provider moonshot --json`, then run an isolated
  `carlito agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
Convex-managed live Telegram lane as parallel jobs. `Carlito Release Checks`
...
- `pnpm carlito qa suite`
  ...
  - Supports provider modes `live-frontier`, `mock-openai`, and `aimock`.
    ...
    `mock-openai` lane.
- `pnpm carlito qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  - Builds an npm tarball from the current checkout, installs it globally in
    ...
    endpoint.
  - Use `CARLITO_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:bundled-channel-deps`
  - Packs and installs the current Carlito build in Docker, starts the Gateway
...
- `pnpm carlito qa matrix`
  ...
  - Uses the pinned stable Tuwunel image `ghcr.io/matrix-construct/tuwunel:v1.5.1` by default. Override with `CARLITO_QA_MATRIX_TUWUNEL_IMAGE` when you need to test a different image.
...
- `pnpm carlito qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `CARLITO_QA_TELEGRAM_GROUP_ID`, `CARLITO_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `CARLITO_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `CARLITO_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
  - Exits non-zero when any scenario fails. Use `--allow-failures` when you
    want artifacts without a failing exit code.
...
When `--credential-source convex` (or `CARLITO_QA_CREDENTIAL_SOURCE=convex`) is enabled for
`carlito qa telegram`, QA lab acquires an exclusive lease from a Convex-backed pool, pulsechecks
...
- `CARLITO_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `CARLITO_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `CARLITO_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `CARLITO_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `CARLITO_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `CARLITO_QA_CREDENTIAL_PULSECHECK_INTERVAL_MS` (default `30000`)
- `CARLITO_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `CARLITO_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `CARLITO_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `CARLITO_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `CARLITO_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`CARLITO_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`CARLITO_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm carlito qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm carlito qa credentials list --kind telegram
pnpm carlito qa credentials remove --credential-id <credential-id>
```

Use `--json` for machine-readable output in scripts and CI utilities.

Default endpoint contract (`CARLITO_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
- how `carlito qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `carlito qa <runner>` instead of registering a competing root command.
   Runner plugins should declare `qaRunners` in `carlito.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`.
    ...
    <Accordion title="Projects, shards, and scoped lanes"> - Untargeted `pnpm test` runs twelve smaller shard configs (`core-unit-fast`, `core-unit-src`, `core-unit-security`, `core-unit-ui`, `core-unit-support`, `core-support-boundary`, `core-contracts`, `core-bundled`, `core-runtime`, `agentic`, `auto-reply`, `extensions`) instead of one giant native root-project process. This cuts peak RSS on loaded machines and avoids auto-reply/extension work starving unrelated suites. - `pnpm test --watch` still uses the native root `vitest.config.ts` project graph, because a multi-shard watch loop is not practical. - `pnpm test`, `pnpm test:watch`, and `pnpm test:perf:imports` route explicit file/directory targets through scoped lanes first, so `pnpm test extensions/discord/src/monitor/message-handler.preflight.test.ts` avoids paying the full root project startup tax. - `pnpm test:changed` expands changed git paths into the same scoped lanes when the diff only touches routable source/test files; config/setup edits still fall back to the broad root-project rerun. - `pnpm check:changed` is the normal smart local gate for narrow work. It classifies the diff into core, core tests, extensions, extension tests, apps, docs, release metadata, and tooling, then runs the matching typecheck/lint/test lanes. Public Plugin SDK and plugin-contract changes include one extension validation pass because extensions depend on those core contracts. Release metadata-only version bumps run targeted version/config/root-dependency checks instead of the full suite, with a guard that rejects package changes outside the top-level version field. - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-carlito-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes. - Selected `plugin-sdk` and `commands` helper source files also map changed-mode runs to explicit sibling tests in those light lanes, so helper edits avoid rerunning the full heavy suite for that directory. - `auto-reply` has three dedicated buckets: top-level core helpers, top-level `reply.*` integration tests, and the `src/auto-reply/reply/**` subtree. This keeps the heaviest reply harness work off the cheap status/chunk/token tests.
      ...
      <Accordion title="Vitest pool and isolation defaults">
        ...
        - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
          ...
          Set `CARLITO_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
          behavior.
      </Accordion>
      ...
      <Accordion title="Fast local iteration">
        ...
        - The base Vitest config marks the projects/config files as
          ...
          wiring changes.
        - The config keeps `CARLITO_VITEST_FS_MODULE_CACHE` enabled on supported
          hosts; set `CARLITO_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Useful overrides:
  - `CARLITO_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `CARLITO_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Carlito's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `CARLITO_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `CARLITO_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `CARLITO_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.carlito`.
- Set `CARLITO_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `CARLITO_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `CARLITO_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/pulsecheck output:
  ...
  - Tune direct-model pulsechecks with `CARLITO_LIVE_PULSECHECK_MS`.
  - Tune gateway/probe pulsechecks with `CARLITO_LIVE_GATEWAY_PULSECHECK_MS`.
...
- Optional target overrides:
  - `CARLITO_ANDROID_NODE_ID` or `CARLITO_ANDROID_NODE_NAME`.
  - `CARLITO_ANDROID_GATEWAY_URL` / `CARLITO_ANDROID_GATEWAY_TOKEN` / `CARLITO_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `CARLITO_LIVE_TEST=1` if invoking Vitest directly)
- Set `CARLITO_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `CARLITO_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `CARLITO_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `CARLITO_LIVE_MODELS="openai/gpt-5.4,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Modern/all sweeps default to a curated high-signal cap; set `CARLITO_LIVE_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers:
  - `CARLITO_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `CARLITO_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@carlito" actually does)
...
- How to select models:
  ...
  - `CARLITO_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `CARLITO_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all gateway sweeps default to a curated high-signal cap; set `CARLITO_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers (avoid “OpenRouter everything”):
  - `CARLITO_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `CARLITO_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.5"`
  - `CARLITO_LIVE_CLI_BACKEND_COMMAND="/full/path/to/codex"`
  - `CARLITO_LIVE_CLI_BACKEND_ARGS='["exec","--json","--color","never","--sandbox","read-only","--skip-git-repo-check"]'`
  - `CARLITO_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `CARLITO_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `CARLITO_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `CARLITO_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `CARLITO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=0` to disable the default Claude Sonnet -> Opus same-session continuity probe (set to `1` to force it on when the selected model supports a switch target).
...
- It resolves CLI smoke metadata from the owning extension, then installs the matching Linux CLI package (`@anthropic-ai/claude-code`, `@openai/codex`, or `@google/gemini-cli`) into a cached writable prefix at `CARLITO_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/carlito/docker-cli-tools`).
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `CARLITO_LIVE_ACP_BIND_AGENT=claude`
  - `CARLITO_LIVE_ACP_BIND_AGENT=codex`
  - `CARLITO_LIVE_ACP_BIND_AGENT=gemini`
  - `CARLITO_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `CARLITO_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
  - `CARLITO_LIVE_ACP_BIND_CODEX_MODEL=gpt-5.5`
  - `CARLITO_LIVE_ACP_BIND_PARENT_MODEL=openai/gpt-5.4`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `CARLITO_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
...
- Use `CARLITO_LIVE_ACP_BIND_AGENTS=claude`, `CARLITO_LIVE_ACP_BIND_AGENTS=codex`, or `CARLITO_LIVE_ACP_BIND_AGENTS=gemini` to narrow the matrix.
...
- Inside Docker, the runner sets `CARLITO_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx` so acpx keeps provider env vars from the sourced profile available to the child harness CLI.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - select `CARLITO_AGENT_RUNTIME=codex`
  - send a first gateway agent turn to `openai/gpt-5.4` with the Codex harness forced
  - send a second turn to the same Carlito session and verify the app-server
    thread can resume
...
- Enable: `CARLITO_LIVE_CODEX_HARNESS=1`
- Default model: `openai/gpt-5.4`
- Optional image probe: `CARLITO_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `CARLITO_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- Optional Guardian probe: `CARLITO_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`
- The smoke sets `CARLITO_AGENT_HARNESS_FALLBACK=none` so a broken Codex
...
```bash
source ~/.profile
CARLITO_LIVE_CODEX_HARNESS=1 \
  CARLITO_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  CARLITO_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  CARLITO_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1 \
  CARLITO_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.4 \
...
- Docker enables the image, MCP/tool, and Guardian probes by default. Set
  `CARLITO_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `CARLITO_LIVE_CODEX_HARNESS_MCP_PROBE=0` or
  `CARLITO_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0` when you need a narrower debug
  run.
- Docker also exports `CARLITO_AGENT_HARNESS_FALLBACK=none`, matching the live
...
- Single model, direct (no gateway):
  - `CARLITO_LIVE_MODELS="openai/gpt-5.4" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `CARLITO_LIVE_GATEWAY_MODELS="openai/gpt-5.4" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `CARLITO_LIVE_GATEWAY_MODELS="openai/gpt-5.4,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `CARLITO_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `CARLITO_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API vs Gemini CLI:
  - API: Carlito calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: Carlito shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
`CARLITO_LIVE_GATEWAY_MODELS="openai/gpt-5.4,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `CARLITO_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `carlito models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `carlito models list` / model selection.

- Per-agent auth profiles: `~/.carlito/agents/<agentId>/agent/auth-profiles.json` (this is what “profile keys” means in the live tests)
- Config: `~/.carlito/carlito.json` (or `CARLITO_CONFIG_PATH`)
- Legacy state dir: `~/.carlito/credentials/` (copied into the staged live home when present, but not the main profile-key store)
...
- Enable: `CARLITO_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `xai`
- Optional narrowing:
  - `CARLITO_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google,openrouter,xai"`
  - `CARLITO_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-2,google/gemini-3.1-flash-image-preview,openrouter/google/gemini-3.1-flash-image-preview,xai/grok-imagine-image"`
  - `CARLITO_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit,openrouter:generate,xai:default-generate,xai:default-edit"`
- Optional auth behavior:
  - `CARLITO_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

## Music generation live

- Test: `extensions/music-generation-providers.live.test.ts`
- Enable: `CARLITO_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `CARLITO_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `CARLITO_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.5+"`
- Optional auth behavior:
  - `CARLITO_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `CARLITO_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
- Harness: `pnpm test:live:media video`
- Scope:
  - Exercises the shared bundled video-generation provider path
  - Defaults to the release-safe smoke path: non-FAL providers, one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `CARLITO_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `--video-providers fal` or `CARLITO_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` to run it explicitly
  - Loads provider env vars from your login shell (`~/.profile`) before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `CARLITO_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `CARLITO_LIVE_TEST=1 CARLITO_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `CARLITO_LIVE_VIDEO_GENERATION_PROVIDERS="google,openai,runway"`
  - `CARLITO_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `CARLITO_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `CARLITO_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `CARLITO_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `CARLITO_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `CARLITO_LIVE_GATEWAY_SMOKE=1`,
  `CARLITO_LIVE_GATEWAY_MAX_MODELS=8`,
  `CARLITO_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `CARLITO_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Carlito tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, verifies enabling the plugin installs its runtime deps on demand, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `CARLITO_NPM_ONBOARD_PACKAGE_TGZ=/path/to/carlito-*.tgz`, skip the host rebuild with `CARLITO_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `CARLITO_NPM_ONBOARD_CHANNEL=discord`.
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `carlito infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `CARLITO_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/carlito-*.tgz`, skip the host build with `CARLITO_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `CARLITO_BUN_GLOBAL_SMOKE_DIST_IMAGE=carlito-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `CARLITO_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `CARLITO_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
...
- Bundled plugin runtime deps: `pnpm test:docker:bundled-channel-deps` builds a small Docker runner image by default, builds and packs Carlito once on the host, then mounts that tarball into each Linux install scenario. Reuse the image with `CARLITO_SKIP_DOCKER_BUILD=1`, skip the host rebuild after a fresh local build with `CARLITO_BUNDLED_CHANNEL_HOST_BUILD=0`, or point at an existing tarball with `CARLITO_BUNDLED_CHANNEL_PACKAGE_TGZ=/path/to/carlito-*.tgz`.
- Narrow bundled plugin runtime deps while iterating by disabling unrelated scenarios, for example:
  `CARLITO_BUNDLED_CHANNEL_SCENARIOS=0 CARLITO_BUNDLED_CHANNEL_UPDATE_SCENARIO=0 CARLITO_BUNDLED_CHANNEL_ROOT_OWNED_SCENARIO=0 CARLITO_BUNDLED_CHANNEL_SETUP_ENTRY_SCENARIO=0 pnpm test:docker:bundled-channel-deps`.

To prebuild and reuse the shared built-app image manually:

```bash
CARLITO_DOCKER_E2E_IMAGE=carlito-docker-e2e:local pnpm test:docker:e2e-build
CARLITO_DOCKER_E2E_IMAGE=carlito-docker-e2e:local CARLITO_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `CARLITO_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `CARLITO_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
`.pnpm-store`, `.worktrees`, `__carlito_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `CARLITO_SKIP_CHANNELS=1` so gateway live probes do not start
...
`CARLITO_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Carlito gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `carlito/default`, then sends a
...
This lane expects a usable live model key, and `CARLITO_PROFILE_FILE`
...
container, starts a second container that spawns `carlito mcp serve`, then
...
- `CARLITO_CONFIG_DIR=...` (default: `~/.carlito`) mounted to `/home/node/.carlito`
- `CARLITO_WORKSPACE_DIR=...` (default: `~/.carlito/workspace`) mounted to `/home/node/.carlito/workspace`
- `CARLITO_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `CARLITO_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `CARLITO_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `CARLITO_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/carlito/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `CARLITO_LIVE_PROVIDERS` / `CARLITO_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `CARLITO_DOCKER_AUTH_DIRS=all`, `CARLITO_DOCKER_AUTH_DIRS=none`, or a comma list like `CARLITO_DOCKER_AUTH_DIRS=.claude,.codex`
- `CARLITO_LIVE_GATEWAY_MODELS=...` / `CARLITO_LIVE_MODELS=...` to narrow the run
- `CARLITO_LIVE_GATEWAY_PROVIDERS=...` / `CARLITO_LIVE_PROVIDERS=...` to filter providers in-container
- `CARLITO_SKIP_DOCKER_BUILD=1` to reuse an existing `carlito:local-live` image for reruns that do not need a rebuild
- `CARLITO_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `CARLITO_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `CARLITO_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
