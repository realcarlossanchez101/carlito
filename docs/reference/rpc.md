...
Carlito integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Carlito owns lifecycle when `channels.signal.autoStart=true`.
...
- Carlito spawns `imsg rpc` as a child process (legacy iMessage integration).
