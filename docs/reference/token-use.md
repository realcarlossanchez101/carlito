---
summary: "How Carlito builds prompt context and reports token usage + costs"
read_when:
...
Carlito tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Carlito assembles its own system prompt on every run. It includes:
...
For images, Carlito downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Carlito session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `carlito status --usage` and `carlito channels list` show
...
available; otherwise Carlito falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, Carlito shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Carlito can
...
Anthropic's 1M context window is currently beta-gated. Carlito can inject the
...
Carlito skips the `context-1m-*` beta header because Anthropic currently
