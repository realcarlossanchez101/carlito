...
This document explains how Carlito manages sessions end-to-end:
...
Carlito is designed around a single **Gateway process** that owns session state.
...
Carlito persists sessions in two layers:
...
Carlito resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Carlito reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.carlito/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Carlito intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Carlito splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Carlito
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Carlito preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Carlito consumes the events, but Pi decides when to compact).
...
Carlito also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Carlito bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Carlito leaves it alone.
...
- If the provider fails or returns an empty result, Carlito falls back to built-in LLM summarization automatically.
...
- `carlito status` (CLI)
- `carlito sessions` / `sessions --json`
...
Carlito supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Carlito strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Carlito also suppresses **draft/typing streaming** when a
...
Carlito uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Carlito’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `carlito status`.
