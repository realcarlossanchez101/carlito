...
CLI onboarding is the **recommended** way to set up Carlito on macOS,
...
`carlito dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`carlito configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.carlito/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `carlito doctor` first.
</Note>
...
- CLI command reference: [`carlito onboard`](/cli/onboard)
