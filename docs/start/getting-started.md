---
summary: "Get Carlito installed and run your first chat in minutes."
read_when:
...
Install Carlito, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.carlito/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.carlito/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Carlito as a service account or want custom paths:

- `CARLITO_HOME` — home directory for internal path resolution
- `CARLITO_STATE_DIR` — override the state directory
- `CARLITO_CONFIG_PATH` — override the config file path
