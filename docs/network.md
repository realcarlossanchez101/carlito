...
This hub links the core docs for how Carlito connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`carlito gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__carlito__/canvas/`, `/__carlito__/a2ui/`), protected by Gateway auth when bound beyond loopback.
