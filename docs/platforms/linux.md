...
2. `npm i -g carlito@latest`
3. `carlito onboard --install-daemon`
...
Carlito installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `carlito gateway install` and
`carlito onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/carlito-gateway[-<profile>].service`:

```
[Unit]
Description=Carlito Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Carlito therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Carlito starts the child through a short
...
- Carlito-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `CARLITO_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
