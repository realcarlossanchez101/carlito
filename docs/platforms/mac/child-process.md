...
service via the external `carlito` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.carlito.gateway`
  (or `ai.carlito.<profile>` when using `--profile`/`CARLITO_PROFILE`; legacy `com.carlito.*` is supported).
