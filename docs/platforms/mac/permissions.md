...
- Same path: run the app from a fixed location (for Carlito, `dist/Carlito.app`).
...
```bash
sudo tccutil reset Accessibility ai.carlito.mac
sudo tccutil reset ScreenCapture ai.carlito.mac
...
Workaround: move files into the Carlito workspace (`~/.carlito/workspace`) if you want to avoid per-folder grants.
