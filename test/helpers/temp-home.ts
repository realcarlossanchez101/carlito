...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked CARLITO_HOME.
  delete process.env.CARLITO_HOME;
  process.env.CARLITO_STATE_DIR = path.join(base, ".carlito");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "carlito-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".carlito", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
