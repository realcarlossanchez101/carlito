...
const piEmbeddedMocks = getSharedMocks("carlito.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("carlito.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("carlito.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("carlito.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    carlitoHome: process.env.CARLITO_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("CARLITO_HOME", snapshot.carlitoHome);
  restoreKey("CARLITO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.CARLITO_STATE_DIR = join(home, ".carlito");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "carlito-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".carlito", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): CarlitoConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "carlito"),
