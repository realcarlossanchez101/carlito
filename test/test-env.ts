...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.CARLITO_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "CARLITO_TEST_FAST", value: process.env.CARLITO_TEST_FAST },
    {
      key: "CARLITO_STRICT_FAST_REPLY_CONFIG",
      value: process.env.CARLITO_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "CARLITO_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.CARLITO_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "CARLITO_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.CARLITO_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "CARLITO_STATE_DIR", value: process.env.CARLITO_STATE_DIR },
    { key: "CARLITO_CONFIG_PATH", value: process.env.CARLITO_CONFIG_PATH },
    { key: "CARLITO_GATEWAY_PORT", value: process.env.CARLITO_GATEWAY_PORT },
    { key: "CARLITO_BRIDGE_ENABLED", value: process.env.CARLITO_BRIDGE_ENABLED },
    { key: "CARLITO_BRIDGE_HOST", value: process.env.CARLITO_BRIDGE_HOST },
    { key: "CARLITO_BRIDGE_PORT", value: process.env.CARLITO_BRIDGE_PORT },
    { key: "CARLITO_CANVAS_HOST_PORT", value: process.env.CARLITO_CANVAS_HOST_PORT },
    { key: "CARLITO_TEST_HOME", value: process.env.CARLITO_TEST_HOME },
    { key: "CARLITO_AGENT_DIR", value: process.env.CARLITO_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CARLITO_TEST_HOME = tempHome;
  process.env.CARLITO_TEST_FAST = "1";
  process.env.CARLITO_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.CARLITO_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.CARLITO_BRIDGE_ENABLED;
  delete process.env.CARLITO_BRIDGE_HOST;
  delete process.env.CARLITO_BRIDGE_PORT;
  delete process.env.CARLITO_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.CARLITO_STATE_DIR = path.join(tempHome, ".carlito");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.CARLITO_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.CARLITO_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".carlito");
  const priorIsolatedHome = params.env.CARLITO_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".carlito")
  ) {
    realStateDir = path.join(params.realHome, ".carlito");
  }
  const tempStateDir = path.join(params.tempHome, ".carlito");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.CARLITO_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.CARLITO_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "carlito.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "carlito.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.CARLITO_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.CARLITO_LIVE_USE_REAL_HOME);
