...
describe("local-heavy-check-runtime", () => {
  ...
  it("skips declaration transforms for no-emit tsgo checks", () => {
    const { args } = applyLocalTsgoPolicy([], makeEnv({ CARLITO_LOCAL_CHECK: "0" }), ROOMY_HOST);
  ...
  it("keeps explicit tsgo flags and Go env overrides intact when throttled", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        CARLITO_TSGO_PPROF_DIR: "/tmp/profile",
      }),
      CONSTRAINED_HOST,
    );
  ...
  it("keeps explicit tsgo declaration flags intact", () => {
    const env = makeEnv({ CARLITO_LOCAL_CHECK_MODE: "full" });
  ...
  it("uses the configured local tsgo build info file", () => {
    const { args } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        CARLITO_LOCAL_CHECK_MODE: "full",
        CARLITO_TSGO_BUILD_INFO_FILE: ".artifacts/custom/tsgo.tsbuildinfo",
      }),
      ROOMY_HOST,
    );
  ...
  it("avoids incremental cache reuse for ad hoc tsgo runs", () => {
    const { args } = applyLocalTsgoPolicy(
      ["--extendedDiagnostics"],
      makeEnv({ CARLITO_LOCAL_CHECK_MODE: "full" }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the throttled tsgo policy on roomy hosts", () => {
    const { args, env } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        CARLITO_LOCAL_CHECK_MODE: "throttled",
      }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the tsgo lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLockForTsgo(
        ["--help"],
        makeEnv({ CARLITO_TSGO_FORCE_LOCK: "1" }),
      ),
    ).toBe(true);
  });
  ...
  it("skips the heavy-check lock for explicit oxlint file targets", () => {
    const cwd = createTempDir("carlito-oxlint-lock-skip-");
  ...
  it("keeps the heavy-check lock for directory targets and broad oxlint runs", () => {
    const cwd = createTempDir("carlito-oxlint-lock-keep-");
  ...
  it("allows forcing the oxlint lock back on", () => {
    const cwd = createTempDir("carlito-oxlint-lock-force-");
    ...
    expect(
      shouldAcquireLocalHeavyCheckLockForOxlint(["--type-aware", "--", "sample.ts"], {
        cwd,
        env: makeEnv({ CARLITO_OXLINT_FORCE_LOCK: "1" }),
      }),
    ).toBe(true);
  });

  it("reclaims stale local heavy-check locks from dead pids", () => {
    const cwd = createTempDir("carlito-local-heavy-check-");
    const commonDir = path.join(cwd, ".git");
    const lockDir = path.join(commonDir, "carlito-local-checks", "heavy-check.lock");
  ...
  it("cleans up stale legacy test locks when acquiring the shared heavy-check lock", () => {
    const cwd = createTempDir("carlito-local-heavy-check-legacy-");
    const commonDir = path.join(cwd, ".git");
    const locksDir = path.join(commonDir, "carlito-local-checks");
