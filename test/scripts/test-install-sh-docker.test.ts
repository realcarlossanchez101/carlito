...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/carlito-release-checks.yml";
...
describe("test-install-sh-docker", () => {
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(workflow).toContain("CARLITO_INSTALL_SMOKE_UPDATE_BASELINE: latest");
  });

  it("can reuse dist from the already-built root Docker smoke image", () => {
    const script = readFileSync(SCRIPT_PATH, "utf8");

    expect(script).toContain('UPDATE_DIST_IMAGE="${CARLITO_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with pulsecheck and install-size audits", () => {
    ...
    expect(script).toContain("carlito update --tag");
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${CARLITO_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${CARLITO_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("CARLITO_INSTALL_SMOKE_MODE=npm-global");
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("CARLITO_BUN_GLOBAL_SMOKE_DIST_IMAGE");
  });
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "CARLITO_BUN_GLOBAL_SMOKE_DIST_IMAGE: carlito-dockerfile-smoke:local",
    );
    expect(workflow).toContain("format('{0}-manual-{1}', github.workflow, github.run_id)");
    expect(workflow).toContain("CARLITO_CI_FORCE_FULL_INSTALL_SMOKE");
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("CARLITO_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain('CARLITO_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
