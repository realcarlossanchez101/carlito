...
} from "../../scripts/carlito-cross-os-release-checks.ts";

describe("scripts/carlito-cross-os-release-checks", () => {
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'carlito.cmd')");
    expect(script).toContain("$cmd = Get-Command carlito -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        CARLITO_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        CARLITO_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        CARLITO_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("carlito-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://carlito.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://carlito.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://carlito.ai/install.ps1");
  });
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/carlito", "darwin"),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("carlito@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("carlito@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "carlito-cross-os-installed-version-"));
    try {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "carlito-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
