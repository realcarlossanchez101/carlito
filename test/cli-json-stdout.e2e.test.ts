  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.CARLITO_HOME;
        delete env.CARLITO_STATE_DIR;
        delete env.CARLITO_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "carlito.mjs");
