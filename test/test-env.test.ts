...
function createTempHome(): string {
  return makeTempDir(tempDirs, "carlito-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-carlito.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.carlito/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.carlito/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".carlito", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".carlito", "external-plugins", "glueclaw", "carlito.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".carlito", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.CARLITO_LIVE_TEST_QUIET = "1";
    process.env.CARLITO_CONFIG_PATH = "~/custom-carlito.json5";
    process.env.CARLITO_TEST_HOME = priorIsolatedHome;
    process.env.CARLITO_STATE_DIR = path.join(priorIsolatedHome, ".carlito");
    ...
    expect(process.env.CARLITO_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".carlito", "carlito.json");
    ...
    expect(fs.existsSync(path.join(testEnv.tempHome, ".carlito", "credentials", "token.txt"))).toBe(
      true,
    );
    expect(
      fs.existsSync(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".carlito", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.CARLITO_LIVE_TEST = "1";
    process.env.CARLITO_LIVE_USE_REAL_HOME = "1";
    process.env.CARLITO_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.CARLITO_LIVE_TEST;
    delete process.env.CARLITO_LIVE_GATEWAY;
    delete process.env.CARLITO_LIVE_USE_REAL_HOME;
    delete process.env.CARLITO_LIVE_TEST_QUIET;
