...
If you believe you've found a security issue in Carlito, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [carlito/carlito](https://github.com/realcarlossanchez101/carlito)
- **macOS desktop app** — [carlito/carlito](https://github.com/realcarlossanchez101/carlito) (apps/macos)
- **iOS app** — [carlito/carlito](https://github.com/realcarlossanchez101/carlito) (apps/ios)
- **Android app** — [carlito/carlito](https://github.com/realcarlossanchez101/carlito) (apps/android)
- **ClawHub** — [carlito/clawhub](https://github.com/carlito/clawhub)
- **Trust and threat model** — [carlito/trust](https://github.com/carlito/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@carlito.ai](mailto:security@carlito.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.carlito.ai).
...
- Tested version details (Carlito version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through Carlito. Showing that Carlito can reach a native parser is not enough by itself.
- Demonstrated impact tied to Carlito's documented trust boundaries.
- For exposed-secret reports: proof the credential is Carlito-owned (or grants access to Carlito-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example Sharp/libvips/libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through Carlito. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Carlito. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Carlito is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Carlito does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-carlito-scopes` headers are ignored for that shared-secret path
...
- Carlito can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Carlito, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Carlito's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Carlito's trusted computing base for a gateway.
...
- Using Carlito in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Carlito production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.carlito`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Carlito-owned and not granting access to Carlito-operated infrastructure/services) without demonstrated Carlito impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Carlito security guidance assumes:

- The host where Carlito runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.carlito` state/config (including `carlito.json`) is effectively a trusted operator.
...
Carlito's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Carlito separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Carlito can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Carlito process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Carlito uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/carlito` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/carlito` (or `carlito-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Carlito-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Carlito temp helpers (`resolvePreferredCarlitoTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-carlito-dir.ts`
...
For threat model + hardening guidance (including `carlito security audit --deep` and `--fix`), see:
...
Carlito's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `carlito gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Carlito keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `carlito security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__carlito__/canvas/`, `/__carlito__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__carlito__/canvas/`, `/__carlito__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Carlito requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running Carlito in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v carlito-data:/app/data \
  carlito/carlito:latest
```
