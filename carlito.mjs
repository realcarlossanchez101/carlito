...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `carlito: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
// Bidirectional alias for the rebrand. Pre-rebrand users had OPENCLAW_* env
// vars in their shells/CI; the swept source reads CARLITO_*. Copy each set
// name to the other so both readers see the value. Drop in the next major.
for (const key of Object.keys(process.env)) {
  if (key.startsWith("OPENCLAW_")) {
    const mirror = "CARLITO_" + key.slice("OPENCLAW_".length);
    if (process.env[mirror] === undefined) process.env[mirror] = process.env[key];
  } else if (key.startsWith("CARLITO_")) {
    const mirror = "OPENCLAW_" + key.slice("CARLITO_".length);
    if (process.env[mirror] === undefined) process.env[mirror] = process.env[key];
  }
}
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["carlito: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:carlito/carlito#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g carlito@latest`.");
  return lines.join("\n");
};
