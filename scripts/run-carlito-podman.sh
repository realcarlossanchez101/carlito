#!/usr/bin/env bash
# Rootless Carlito in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-carlito-podman.sh launch        # Start gateway
#   ./scripts/run-carlito-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   carlito --container carlito dashboard --no-open
#   carlito --container carlito channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      CARLITO_GATEWAY_TOKEN|CARLITO_PODMAN_CONTAINER|CARLITO_PODMAN_IMAGE|CARLITO_IMAGE|CARLITO_PODMAN_PULL|CARLITO_PODMAN_GATEWAY_HOST_PORT|CARLITO_GATEWAY_PORT|CARLITO_PODMAN_BRIDGE_HOST_PORT|CARLITO_BRIDGE_PORT|CARLITO_GATEWAY_BIND|CARLITO_PODMAN_USERNS|CARLITO_BIND_MOUNT_OPTIONS|CARLITO_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-carlito-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${CARLITO_CONFIG_DIR:-$EFFECTIVE_HOME/.carlito}"
ENV_FILE="${CARLITO_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${CARLITO_CONFIG_DIR:-$EFFECTIVE_HOME/.carlito}"
ENV_FILE="${CARLITO_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${CARLITO_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${CARLITO_PODMAN_CONTAINER:-carlito}"
CARLITO_IMAGE="${CARLITO_PODMAN_IMAGE:-${CARLITO_IMAGE:-carlito:local}}"
PODMAN_PULL="${CARLITO_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${CARLITO_PODMAN_GATEWAY_HOST_PORT:-${CARLITO_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${CARLITO_PODMAN_BRIDGE_HOST_PORT:-${CARLITO_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${CARLITO_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$CARLITO_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v carlito >/dev/null 2>&1; then
    return 0
  fi
  CARLITO_CONTAINER="" CARLITO_CONFIG_DIR="$config_dir" \
    carlito config get gateway.bind 2>/dev/null || true
}
...
# CARLITO_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${CARLITO_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'CARLITO_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v carlito >/dev/null 2>&1; then
    echo "Warning: carlito not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    CARLITO_CONTAINER="" CARLITO_CONFIG_DIR="$config_dir" \
      carlito config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    CARLITO_CONTAINER="" CARLITO_CONFIG_DIR="$config_dir" \
      carlito config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  CARLITO_CONTAINER="" CARLITO_CONFIG_DIR="$config_dir" \
    carlito config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${CARLITO_GATEWAY_TOKEN:-}" ]]; then
  export CARLITO_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "CARLITO_GATEWAY_TOKEN" "$CARLITO_GATEWAY_TOKEN"
  echo "Generated CARLITO_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/carlito.json"
...
PODMAN_USERNS="${CARLITO_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported CARLITO_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (CARLITO_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${CARLITO_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${CARLITO_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$CARLITO_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.carlito/.npm \
    -e CARLITO_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.carlito:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.carlito/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$CARLITO_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME carlito dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME carlito devices approve --latest  # if pairing required"
