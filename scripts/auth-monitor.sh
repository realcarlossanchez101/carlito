...
#
# Suggested cron: */30 * * * * /home/admin/carlito/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Carlito notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., carlito-alerts)
...
STATE_FILE="$HOME/.carlito/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Carlito if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use carlito
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Carlito to $NOTIFY_PHONE..."
            carlito send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Carlito is down. Run: ssh l36 '~/carlito/scripts/mobile-reauth.sh'" "urgent"
    exit 1
