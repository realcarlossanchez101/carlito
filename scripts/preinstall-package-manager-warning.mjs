...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[carlito] warning: detected ${packageManager} for install lifecycle.`,
    "[carlito] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[carlito] prefer: corepack pnpm install",
  ].join("\n");
}
