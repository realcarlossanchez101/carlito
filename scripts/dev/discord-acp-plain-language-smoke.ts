...
type DriverMode = "token" | "webhook" | "carlito";
...
function resolveStateDir(): string {
  const override = process.env.CARLITO_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.CARLITO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".carlito");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver carlito] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Carlito spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|carlito> Driver transport mode (default: token)\n" +
    ...
    "  --carlito-bin <path>        Carlito CLI binary for driver=carlito (default: carlito)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  CARLITO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  CARLITO_DISCORD_SMOKE_DRIVER\n" +
    "  CARLITO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  CARLITO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  CARLITO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  CARLITO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  CARLITO_DISCORD_SMOKE_AGENT\n" +
    "  CARLITO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  CARLITO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  CARLITO_DISCORD_SMOKE_POLL_MS\n" +
    "  CARLITO_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  CARLITO_DISCORD_SMOKE_CARLITO_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.CARLITO_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.CARLITO_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "carlito"
        ? "carlito"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.CARLITO_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.CARLITO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.CARLITO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.CARLITO_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.CARLITO_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.CARLITO_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.CARLITO_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.CARLITO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.CARLITO_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.CARLITO_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const carlitoBin =
    resolveArg("--carlito-bin") || process.env.CARLITO_DISCORD_SMOKE_CARLITO_BIN || "carlito";
...
async function carlitoCliJson<T>(params: { carlitoBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.carlitoBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`carlito ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithCarlito(params: {
  carlitoBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await carlitoCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    carlitoBin: params.carlitoBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "carlito") {
    return await readMessagesWithCarlito({
      carlitoBin: params.args.carlitoBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `carlito-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await carlitoCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("carlito message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "carlito"
            ? await readMessagesWithCarlito({
                carlitoBin: args.carlitoBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Carlito.`,
        diagnostics: {
          bindingCandidates: [
            {
