...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/carlito/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Carlito Re-Auth" -c "After getting key, run: ssh $SERVER '~/carlito/scripts/mobile-reauth.sh'" --id carlito-auth
        ;;
    *)
