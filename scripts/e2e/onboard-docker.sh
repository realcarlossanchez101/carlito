...
IMAGE_NAME="$(docker_e2e_resolve_image "carlito-onboard-e2e" CARLITO_ONBOARD_E2E_IMAGE)"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/carlito-bin:$PATH"
  mkdir -p /tmp/carlito-bin
  cat > /tmp/carlito-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$CARLITO_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_carlito_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/carlito-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/carlito-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$CARLITO_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/carlito-e2e-$1.XXXXXX"
  }

  set_isolated_carlito_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export CARLITO_HOME="$home_dir"
    export CARLITO_STATE_DIR="$home_dir/.carlito"
  ...
  run_case_logged() {
    local label="$1"
    shift
    local log_path="/tmp/carlito-onboard-${label}.log"
  ...
  run_case_local_basic() {
    ...
    set_isolated_carlito_env "$home_dir"
    run_case_logged local-basic node "$CARLITO_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$CARLITO_STATE_DIR/workspace"
    config_path="$CARLITO_CONFIG_PATH"
    sessions_dir="$CARLITO_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_carlito_env "$home_dir"
	    ...
	    run_case_logged remote-non-interactive node "$CARLITO_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
JSON
	    ...
	    run_case_logged reset-config node "$CARLITO_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$CARLITO_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$CARLITO_ENTRY\" configure --section skills" send_skills_flow
