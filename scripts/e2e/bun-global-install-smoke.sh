...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${CARLITO_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${CARLITO_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${CARLITO_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
COMMAND_TIMEOUT_MS="${CARLITO_BUN_GLOBAL_SMOKE_TIMEOUT_MS:-180000}"
SMOKE_DIR=""
PACK_DIR=""
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "CARLITO_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (CARLITO_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set CARLITO_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/carlito-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack Carlito tarball"
...
main() {
  ...
  echo "==> Bun global install packed Carlito"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  carlito_bin="$BUN_INSTALL/bin/carlito"
  if [ ! -x "$carlito_bin" ]; then
    carlito_bin="$(command -v carlito || true)"
  fi
  if [ -z "$carlito_bin" ] || [ ! -x "$carlito_bin" ]; then
    echo "Bun global install did not create an executable carlito binary" >&2
    exit 1
  fi

  echo "==> Carlito version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$carlito_bin" --version

  echo "==> Carlito image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$carlito_bin" infer image providers --json)"
  CARLITO_IMAGE_PROVIDERS_JSON="$providers_json" node - <<'NODE'
const raw = process.env.CARLITO_IMAGE_PROVIDERS_JSON ?? "";
let parsed;
try {
