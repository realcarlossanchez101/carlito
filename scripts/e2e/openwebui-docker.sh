...
IMAGE_NAME="$(docker_e2e_resolve_image "carlito-openwebui-e2e" CARLITO_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${CARLITO_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${CARLITO_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${CARLITO_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${CARLITO_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${CARLITO_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${CARLITO_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="carlito-openwebui-e2e-$$"
GW_NAME="carlito-openwebui-gateway-$$"
OW_NAME="carlito-openwebui-$$"
...
docker run -d \
  ...
  -e "CARLITO_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/carlito-openwebui-config.XXXXXX.json)"
    CARLITO_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.CARLITO_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.CARLITO_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.CARLITO_OPENWEBUI_MODEL },
];
...
EOF
    cat > "$workspace/.carlito/workspace-state.json" <<'"'"'EOF'"'"'
{
  "version": 1,
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Carlito E2E" \
  -e WEBUI_SECRET_KEY="carlito-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Carlito E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="carlito/default" \
...
echo "Running Open WebUI -> Carlito smoke..."
if ! docker exec \
