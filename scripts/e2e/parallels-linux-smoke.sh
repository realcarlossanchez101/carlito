...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/carlito-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/carlito-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/carlito-install.sh
  if [[ -n "$INSTALL_VERSION" ]]; then
    guest_exec /usr/bin/env CARLITO_NO_ONBOARD=1 bash /tmp/carlito-install.sh --version "$INSTALL_VERSION" --no-onboard
  else
    guest_exec /usr/bin/env CARLITO_NO_ONBOARD=1 bash /tmp/carlito-install.sh --no-onboard
  fi
  guest_exec carlito --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec carlito --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" carlito onboard \
    --non-interactive \
    --mode local \
...
set -euo pipefail
plugin_dir=/root/.carlito/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{
  "name": "@realcarlossanchez101/test-bad-plugin",
  "version": "1.0.0",
  "carlito": {
...
config_path = Path("/root/.carlito/carlito.json")
config = {}
...
verify_bad_plugin_diagnostic() {
  guest_bash_script <<'EOF'
grep -F "failed to load setup entry" /tmp/carlito-parallels-linux-gateway.log
EOF
}
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "carlito gateway run" >/dev/null 2>&1 || true
rm -f /tmp/carlito-parallels-linux-gateway.log
setsid sh -lc 'exec env CARLITO_HOME=/root CARLITO_STATE_DIR=/root/.carlito CARLITO_CONFIG_PATH=/root/.carlito/carlito.json ${API_KEY_ENV}=${api_key_value_q} carlito gateway run --bind loopback --port 18789 --force >/tmp/carlito-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec carlito gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec carlito gateway status --deep --require-rpc
    return
  fi
  guest_exec carlito gateway status --deep
}

verify_local_turn() {
  guest_exec carlito models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" carlito agent \
    --local \
    --agent main \
    --message ping \
    --json
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "carlito-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "carlito-main-upgrade.tgz"
