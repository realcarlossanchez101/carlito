...
IMAGE_NAME="$(docker_e2e_resolve_image "carlito-cron-mcp-cleanup-e2e" CARLITO_IMAGE)"
PORT="18789"
TOKEN="cron-mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="carlito-cron-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t carlito-cron-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "CARLITO_STATE_DIR=/tmp/carlito-state" \
  -e "CARLITO_CONFIG_PATH=/tmp/carlito-state/carlito.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "CARLITO_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
