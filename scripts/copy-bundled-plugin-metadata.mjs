...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.CARLITO_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@carlito/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "carlito.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "carlito.plugin.json");
    ...
    if (packageJson.carlito && "extensions" in packageJson.carlito) {
      packageJson.carlito = {
        ...packageJson.carlito,
        extensions: rewritePackageExtensions(packageJson.carlito.extensions),
        ...(typeof packageJson.carlito.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.carlito.setupEntry) }
          : {}),
      };
    }
