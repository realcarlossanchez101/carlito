...
import { parseReleaseVersion } from "../carlito-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.carlito?.extensions ?? [];

  if (!packageName.startsWith("@carlito/")) {
    errors.push(`package name must start with "@carlito/"; found "${packageName || "<missing>"}".`);
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("carlito.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("carlito.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.carlito?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.carlito?.install?.npmSpec),
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "carlito-plugin-npm-view-"));
