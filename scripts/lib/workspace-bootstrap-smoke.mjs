...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "carlito-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "carlito.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".carlito", "workspace");
