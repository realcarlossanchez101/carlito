...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.carlito?.extensions)
    ? packageJson.carlito.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.carlito?.setupEntry === "string" &&
    packageJson.carlito.setupEntry.trim().length > 0
      ? packageJson.carlito.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "carlito.plugin.json"));
    }
    if (hasPackageJson) {
