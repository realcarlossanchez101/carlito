...
elif [[ -n "${CARLITO_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(carlito_live_collect_auth_dirs_from_csv "${CARLITO_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(carlito_live_collect_auth_files_from_csv "${CARLITO_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e CARLITO_LIVE_GATEWAY_MODELS="${CARLITO_LIVE_GATEWAY_MODELS:-modern}" \
  -e CARLITO_LIVE_GATEWAY_PROVIDERS="${CARLITO_LIVE_GATEWAY_PROVIDERS:-}" \
  -e CARLITO_LIVE_GATEWAY_SMOKE="${CARLITO_LIVE_GATEWAY_SMOKE:-1}" \
  -e CARLITO_LIVE_GATEWAY_MAX_MODELS="${CARLITO_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e CARLITO_LIVE_GATEWAY_STEP_TIMEOUT_MS="${CARLITO_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e CARLITO_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${CARLITO_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}")
