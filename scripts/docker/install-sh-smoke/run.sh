...
SMOKE_MODE="${CARLITO_INSTALL_SMOKE_MODE:-install}"
SMOKE_PREVIOUS_VERSION="${CARLITO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CARLITO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="carlito"
PACKAGE_NAME="${CARLITO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
FRESH_VERSION="${CARLITO_INSTALL_FRESH_VERSION:-}"
FRESH_TAG_URL="${CARLITO_INSTALL_FRESH_TAG_URL:-}"
UPDATE_BASELINE_VERSION="${CARLITO_INSTALL_UPDATE_BASELINE:-latest}"
UPDATE_BASELINE_TAG_URL="${CARLITO_INSTALL_UPDATE_BASELINE_TAG_URL:-}"
UPDATE_EXPECT_VERSION="${CARLITO_INSTALL_UPDATE_EXPECT_VERSION:-}"
UPDATE_TAG_URL="${CARLITO_INSTALL_UPDATE_TAG_URL:-}"
PULSECHECK_INTERVAL="${CARLITO_INSTALL_SMOKE_PULSECHECK_INTERVAL:-60}"
INSTALL_COMMAND_TIMEOUT="${CARLITO_INSTALL_SMOKE_COMMAND_TIMEOUT:-900}"
...
run_install_smoke() {
  if [[ -n "$FRESH_VERSION" && -n "$FRESH_TAG_URL" ]]; then
    ...
    if [[ -n "${CARLITO_INSTALL_LATEST_OUT:-}" ]]; then
      ...
      if [[ -n "$latest_npm_version" ]]; then
        printf "%s" "$latest_npm_version" > "${CARLITO_INSTALL_LATEST_OUT:-}"
      else
        printf "%s" "$FRESH_VERSION" > "${CARLITO_INSTALL_LATEST_OUT:-}"
      fi
    fi
...
NODE
  ...
  if [[ "$SKIP_PREVIOUS" == "1" ]]; then
    echo "==> Skip preinstall previous (CARLITO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
  else
  ...
  echo "==> Verify installed version"
  if [[ -n "${CARLITO_INSTALL_LATEST_OUT:-}" ]]; then
    printf "%s" "$LATEST_VERSION" > "${CARLITO_INSTALL_LATEST_OUT:-}"
  fi
...
run_update_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: CARLITO_INSTALL_UPDATE_EXPECT_VERSION is required for update mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: CARLITO_INSTALL_UPDATE_TAG_URL is required for update mode" >&2
    return 1
  fi
  ...
  echo "==> Run carlito update from host-served tgz"
  local update_status
  ...
  set +e
  UPDATE_JSON="$(
    run_with_pulsecheck "carlito update" \
      env npm_config_omit=optional NPM_CONFIG_OMIT=optional \
      carlito update --tag "$UPDATE_TAG_URL" --yes --json 2>"$update_stderr_file"
  )"
  update_status=$?
  set -e
  ...
  if [[ "$update_status" -ne 0 ]]; then
    echo "ERROR: carlito update failed with exit code $update_status" >&2
...
run_npm_global_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: CARLITO_INSTALL_UPDATE_EXPECT_VERSION is required for npm-global mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: CARLITO_INSTALL_UPDATE_TAG_URL is required for npm-global mode" >&2
    return 1
  fi
...
case "$SMOKE_MODE" in
  ...
  npm-global)
    ...
    ;;
  *)
    echo "ERROR: unsupported CARLITO_INSTALL_SMOKE_MODE=$SMOKE_MODE" >&2
    exit 1
    ;;
esac
