...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("carlito/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced carlito/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced carlito/plugin-sdk/<subpath> must exist in the public package exports.",
  );
