#!/bin/bash
# Setup Carlito Auth Management System
...
echo "=== Carlito Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'carlito-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Carlito message: Send warning via Carlito itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/carlito-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/carlito-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/carlito-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now carlito-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status carlito-auth-monitor.timer"
echo ""
echo "Quick commands:"
