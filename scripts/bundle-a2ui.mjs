...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "CarlitoKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "CarlitoKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.CARLITO_SPARSE_PROFILE || process.env.CARLITO_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because CARLITO_A2UI_SKIP_MISSING=1 or CARLITO_SPARSE_PROFILE is set.",
      );
      return;
    }
