...
const PUBLISHED_INSTALLER_BASE_URL = "https://carlito.ai";
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.CARLITO_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.CARLITO_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.CARLITO_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `carlito@${previousVersion}` : "carlito@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.CARLITO_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.CARLITO_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        CARLITO_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `carlito-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".carlito");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    CARLITO_HOME: lane.homeDir,
    CARLITO_STATE_DIR: lane.stateDir,
    CARLITO_CONFIG_PATH: join(lane.stateDir, "carlito.json"),
    CARLITO_DISABLE_BONJOUR: "1",
    CARLITO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  const updateEnv = { ...env };
  delete updateEnv.CARLITO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  return updateEnv;
}

export function resolveExplicitBaselineVersion(baselineSpec) {
  const trimmed = baselineSpec.trim();
  if (!trimmed || trimmed === "carlito@latest") {
    return "";
  }
  if (trimmed.startsWith("carlito@")) {
    return trimmed.slice("carlito@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "carlito@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view carlito@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'carlito.cmd'),
      (Join-Path $npmPrefix 'carlito.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command carlito -ErrorAction Stop
...
Write-Output "__CARLITO_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__CARLITO_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed carlito path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v carlito >/dev/null 2>&1",
    'printf "__CARLITO_PATH__=%s\\n" "$(command -v carlito)"',
    "carlito --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__CARLITO_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed carlito path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `carlito update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.carlito/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.CARLITO_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.CARLITO_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.CARLITO_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.CARLITO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Carlito Control</title>") &&
          html.includes("<carlito-app></carlito-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Carlito Control</title>")} app=${html.includes("<carlito-app></carlito-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runCarlito(params) {
...
function installedPackageRoot(prefixDir) {
  return process.platform === "win32"
    ? join(prefixDir, "node_modules", "carlito")
    : join(prefixDir, "lib", "node_modules", "carlito");
}

function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "carlito.mjs");
}
