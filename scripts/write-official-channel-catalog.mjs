...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.carlito) ? packageJson.carlito : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.carlito?.channel?.id) || left.name;
    const rightId = trimString(right.carlito?.channel?.id) || right.name;
