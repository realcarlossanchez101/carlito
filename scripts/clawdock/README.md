...
Inspired by Simon Willison's [Running Carlito in Docker](https://til.simonwillison.net/llms/carlito-docker).
...
Canonical docs page: https://docs.carlito.ai/install/clawdock
...
On first command, ClawDock auto-detects your Carlito directory:

- Checks common paths (`~/carlito`, `~/workspace/carlito`, etc.)
...
| `clawdock-cli <command>`  | Run Carlito CLI commands                       |
...
| `clawdock-config`      | Open the Carlito config directory         |
...
| File                       | Purpose                                                                  |
| -------------------------- | ------------------------------------------------------------------------ |
| `Dockerfile`               | Builds the `carlito:local` image (Node 22, pnpm, non-root `node` user)   |
| `docker-compose.yml`       | Defines `carlito-gateway` and `carlito-cli` services, bind-mounts, ports |
...
| ------------------------- | ------------------------------------------------ | ---------------------------------------------------------------- |
| `<project>/.env`          | **Docker infra** — image, ports, gateway token   | `CARLITO_GATEWAY_TOKEN`, `CARLITO_IMAGE`, `CARLITO_GATEWAY_PORT` |
| `~/.carlito/.env`         | **Secrets** — API keys and bot tokens            | `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `TELEGRAM_BOT_TOKEN`      |
| `~/.carlito/carlito.json` | **Behavior config** — models, channels, policies | Model selection, WhatsApp allowlists, agent settings             |

**Do NOT** put API keys or bot tokens in `carlito.json`. Use `~/.carlito/.env` for all secrets.

### Initial Setup

`./docker-setup.sh` (in the project root) handles first-time Docker configuration:

- Builds the `carlito:local` image from `Dockerfile`
- Creates `<project>/.env` from `.env.example` with a generated gateway token
- Sets up `~/.carlito` directories if they don't exist
...
- `CARLITO_DOCKER_APT_PACKAGES` — extra apt packages to install (e.g. `ffmpeg`)
- `CARLITO_INSTALL_BROWSER=1` — pre-install Chromium for browser automation (adds ~300MB, but skips the 60-90s Playwright install on each container start)
...
- `~/.carlito/.env` is available inside the container at `/home/node/.carlito/.env` — Carlito loads it automatically as the global env fallback
- `~/.carlito/carlito.json` is available at `/home/node/.carlito/carlito.json` — the gateway watches it and hot-reloads most changes
...
The project `.env` feeds Docker Compose directly (gateway token, image name, ports). The `~/.carlito/.env` feeds the Carlito process inside the container.
...
```bash
CARLITO_CONFIG_DIR=/Users/you/.carlito
CARLITO_WORKSPACE_DIR=/Users/you/.carlito/workspace
CARLITO_GATEWAY_PORT=18789
CARLITO_BRIDGE_PORT=18790
CARLITO_GATEWAY_BIND=lan
CARLITO_GATEWAY_TOKEN=<generated-by-docker-setup>
CARLITO_IMAGE=carlito:local
```

### Env Precedence

Carlito loads env vars in this order (highest wins, never overrides existing):
...
3. **`~/.carlito/.env`** — global secrets (API keys, bot tokens)
4. **`carlito.json` `env` block** — inline vars, applied only if still missing
5. **Shell env import** — optional login-shell scrape (`CARLITO_LOAD_SHELL_ENV=1`)

## Common Workflows

### Update Carlito

> **Important:** `carlito update` does not work inside Docker.
...
```bash
carlito channels login --channel whatsapp --verbose
```
...
2. Configure it in the Carlito config
...
- Bash or Zsh shell
- Carlito project (run `scripts/docker/setup.sh`)
