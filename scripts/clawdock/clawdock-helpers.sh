#!/usr/bin/env bash
# ClawDock - Docker helpers for Carlito
# Inspired by Simon Willison's "Running Carlito in Docker"
# https://til.simonwillison.net/llms/carlito-docker
#
# Installation:
...
# Common paths to check for Carlito
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/carlito"
  "${HOME}/workspace/carlito"
  "${HOME}/projects/carlito"
  "${HOME}/dev/carlito"
  "${HOME}/code/carlito"
  "${HOME}/src/carlito"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Carlito at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/carlito"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Carlito not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/realcarlossanchez101/carlito.git ~/carlito"
    echo "  cd ~/carlito && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^CARLITO_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d carlito-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart carlito-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f carlito-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.carlito"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show carlito.json
  if [[ -f "${config_dir}/carlito.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/carlito.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/carlito.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No carlito.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec carlito-gateway \
    bash -c 'echo "alias carlito=\"./carlito.mjs\"" > /tmp/.bashrc_carlito && bash --rcfile /tmp/.bashrc_carlito'
}
...
clawdock-cli() {
  _clawdock_compose run --rm carlito-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build carlito-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d carlito-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build carlito-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "CARLITO_GATEWAY_TOKEN=$token" carlito-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" carlito-gateway \
    bash -c './carlito.mjs config set gateway.remote.token "$TOKEN" && ./carlito.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec carlito-gateway \
    bash -c "./carlito.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart carlito-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm carlito-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec carlito-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'carlito config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Carlito${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (carlito alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to carlito project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.carlito)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'carlito channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'carlito status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.carlito.ai${_CLR_RESET}"
  echo ""
}
