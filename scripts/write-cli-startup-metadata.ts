...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.carlito?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.carlito?.channel?.order;
      const labelRaw = parsed.carlito?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".carlito-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "carlito-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "carlito-build",
    ...
    CARLITO_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
    CARLITO_PLUGIN_DISCOVERY_CACHE_MS: "0",
    CARLITO_PLUGIN_MANIFEST_CACHE_MS: "0",
