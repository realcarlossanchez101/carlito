...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  CARLITO_SKIP_ACPX_RUNTIME: "1",
  CARLITO_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$CARLITO_WATCH_PID_FILE"',
    'mkdir -p "$CARLITO_HOME/.carlito"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$CARLITO_HOME/.carlito/carlito.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    CARLITO_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    CARLITO_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-gateway-watch-"));
