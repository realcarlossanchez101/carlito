...
# Carlito Auth Widget for Termux
...
SERVER="${CARLITO_SERVER:-l36}"

# Check auth status
termux-toast "Checking Carlito auth..."

STATUS=$(ssh "$SERVER" '$HOME/carlito/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/carlito/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CARLITO_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/carlito/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CARLITO_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Carlito won't work until re-auth"
                ;;
        esac
        ;;
