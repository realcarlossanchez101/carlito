...
ACP_AGENT_LIST_RAW="${CARLITO_LIVE_ACP_BIND_AGENTS:-${CARLITO_LIVE_ACP_BIND_AGENT:-claude,codex,gemini}}"
TEMP_DIRS=()
DOCKER_USER="${CARLITO_DOCKER_USER:-node}"
DOCKER_HOME_MOUNT=()
DOCKER_AUTH_PRESTAGED=0

carlito_live_acp_bind_append_build_extension() {
...
carlito_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    claude) printf '%s\n' "claude-cli" ;;
    codex) printf '%s\n' "codex-cli" ;;
    gemini) printf '%s\n' "google-gemini-cli" ;;
    *)
      echo "Unsupported CARLITO_LIVE_ACP_BIND agent: ${1:-} (expected claude, codex, or gemini)" >&2
      return 1
      ;;
  esac
}

carlito_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
    claude) printf '%s' "${CARLITO_LIVE_ACP_BIND_AGENT_COMMAND_CLAUDE:-${CARLITO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    codex) printf '%s' "${CARLITO_LIVE_ACP_BIND_AGENT_COMMAND_CODEX:-${CARLITO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    gemini) printf '%s' "${CARLITO_LIVE_ACP_BIND_AGENT_COMMAND_GEMINI:-${CARLITO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    *) return 1 ;;
  esac
}
...
agent="${CARLITO_LIVE_ACP_BIND_AGENT:-claude}"
case "$agent" in
  claude)
...
if [ -n "\${CARLITO_LIVE_ACP_BIND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${CARLITO_LIVE_ACP_BIND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${CARLITO_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${CARLITO_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  gemini)
    ...
    ;;
  *)
    echo "Unsupported CARLITO_LIVE_ACP_BIND_AGENT: $agent" >&2
    exit 1
    ;;
esac
tmp_dir="$(mktemp -d)"
...
cd "$tmp_dir"
export CARLITO_LIVE_ACP_BIND_AGENT_COMMAND="${CARLITO_LIVE_ACP_BIND_AGENT_COMMAND:-}"
pnpm test:live src/gateway/gateway-acp-bind.live.test.ts
EOF

carlito_live_acp_bind_append_build_extension acpx
...
for token in "${ACP_AGENT_TOKENS[@]}"; do
  agent="$(carlito_live_trim "$token")"
  [[ -n "$agent" ]] || continue
  carlito_live_acp_bind_resolve_auth_provider "$agent" >/dev/null
  ACP_AGENTS+=("$agent")
done

if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use CARLITO_LIVE_ACP_BIND_AGENTS=claude,codex,gemini." >&2
  exit 1
fi

for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  AUTH_PROVIDER="$(carlito_live_acp_bind_resolve_auth_provider "$ACP_AGENT")"
  AGENT_COMMAND="$(carlito_live_acp_bind_resolve_agent_command "$ACP_AGENT")"
  ...
  if [[ -n "${CARLITO_DOCKER_AUTH_DIRS:-}" ]]; then
    ...
    while IFS= read -r auth_dir; do
      [[ -n "$auth_dir" ]] || continue
      AUTH_DIRS+=("$auth_dir")
    done < <(carlito_live_collect_auth_dirs_from_csv "$AUTH_PROVIDER")
    while IFS= read -r auth_file; do
      [[ -n "$auth_file" ]] || continue
      AUTH_FILES+=("$auth_file")
    done < <(carlito_live_collect_auth_files_from_csv "$AUTH_PROVIDER")
  fi
  ...
  DOCKER_RUN_ARGS=(docker run --rm -t \
    ...
    -e CARLITO_LIVE_ACP_BIND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" \
    -e CARLITO_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}" \
    -e GEMINI_API_KEY \
    -e GOOGLE_API_KEY \
    -e OPENAI_API_KEY \
    ...
    -e CARLITO_LIVE_ACP_BIND=1 \
    -e CARLITO_LIVE_ACP_BIND_AGENT="$ACP_AGENT" \
    -e CARLITO_LIVE_ACP_BIND_AGENT_COMMAND="$AGENT_COMMAND")
