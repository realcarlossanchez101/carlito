...
const EXPECTED_REPOSITORY_URL = "https://github.com/realcarlossanchez101/carlito";
...
const skipPackValidationEnv = "CARLITO_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "carlito") {
    errors.push(`package.json name must be "carlito"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.carlito !== "carlito.mjs") {
    errors.push(
      `package.json bin.carlito must be "carlito.mjs"; found "${pkg.bin?.carlito ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`carlito-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `carlito-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
