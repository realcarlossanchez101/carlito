 ...
 * `carlito update`. Use it to verify the completion installation flow
...
\`carlito update\`, \`carlito doctor\`, and \`carlito onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Carlito Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`carlito completion --install\` later to enable.`));
    return;
  }
