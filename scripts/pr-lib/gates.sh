...
prepare_gates() {
  ...
  if [ "$reuse_gates" = "true" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="full"
      if [ -n "${CARLITO_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with CARLITO_VITEST_MAX_WORKERS=$CARLITO_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env CARLITO_VITEST_MAX_WORKERS="$CARLITO_VITEST_MAX_WORKERS" pnpm test
      else
