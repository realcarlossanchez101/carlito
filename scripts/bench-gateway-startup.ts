...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "carlito.json");
...
function sanitizedEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LOGNAME: process.env.LOGNAME ?? "carlito-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "carlito-bench",
    npm_config_update_notifier: "false",
    CARLITO_CONFIG: configPath,
    CARLITO_CONFIG_PATH: configPath,
    CARLITO_GATEWAY_STARTUP_TRACE: "1",
    CARLITO_HOME: root,
    CARLITO_LOCAL_CHECK: "0",
    CARLITO_STATE_DIR: path.join(root, "state"),
    CARLITO_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
  return env;
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "carlito-gateway-bench-"));
