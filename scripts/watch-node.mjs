...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[carlito] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.CARLITO_WATCH_MODE = "1";
  childEnv.CARLITO_WATCH_SESSION = watchSession;
  ...
  childEnv.CARLITO_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.CARLITO_WATCH_COMMAND = deps.args.join(" ");
  }
