...
BASE_IMAGE="${BASE_IMAGE:-carlito-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-carlito-sandbox-common:bookworm-slim}"
...
CARLITO_DOCKER_BUILD_USE_BUILDX="${CARLITO_DOCKER_BUILD_USE_BUILDX:-0}"
CARLITO_DOCKER_BUILD_CACHE_FROM="${CARLITO_DOCKER_BUILD_CACHE_FROM:-}"
CARLITO_DOCKER_BUILD_CACHE_TO="${CARLITO_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${CARLITO_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${CARLITO_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${CARLITO_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${CARLITO_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${CARLITO_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=carlito.sandbox=1)
NOTE
