#!/usr/bin/env bash
# Reset Carlito like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CARLITO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Carlito.app/Contents/MacOS/Carlito"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Carlito"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Carlito"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Carlito"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.carlito.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/carlito-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CARLITO_RESTART_LOG:-/tmp/carlito-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CARLITO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.carlito/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  CARLITO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node carlito.mjs daemon install --force --runtime node"
      log "  node carlito.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.carlito/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.carlito.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Carlito instances"
kill_all_carlito
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Carlito"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Carlito.app" ]]; then
    APP_BUNDLE="/Applications/Carlito.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Carlito.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Carlito.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Carlito.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CARLITO_APP_BUNDLE to your installed Carlito.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node carlito.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node carlito.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".carlito", "carlito.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Carlito is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.carlito.gateway.plist' | head -n 40 || true"
fi
