...
- regenerates apps/ios/Carlito.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${CARLITO_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid CARLITO_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid CARLITO_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid CARLITO_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid CARLITO_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing CARLITO_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(CARLITO_URL_SLASH)$(CARLITO_URL_SLASH)#g'
)"
...
CARLITO_CODE_SIGN_STYLE = Automatic
CARLITO_DEVELOPMENT_TEAM = ${TEAM_ID}
CARLITO_IOS_SELECTED_TEAM = ${TEAM_ID}
CARLITO_APP_BUNDLE_ID = ai.carlito.client
CARLITO_SHARE_BUNDLE_ID = ai.carlito.client.share
CARLITO_ACTIVITY_WIDGET_BUNDLE_ID = ai.carlito.client.activitywidget
CARLITO_WATCH_APP_BUNDLE_ID = ai.carlito.client.watchkitapp
CARLITO_WATCH_EXTENSION_BUNDLE_ID = ai.carlito.client.watchkitapp.extension
CARLITO_APP_PROFILE =
CARLITO_SHARE_PROFILE =
CARLITO_PUSH_TRANSPORT = relay
CARLITO_PUSH_DISTRIBUTION = official
CARLITO_URL_SLASH = /
CARLITO_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
CARLITO_PUSH_APNS_ENVIRONMENT = production
EOF
