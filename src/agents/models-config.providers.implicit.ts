...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.CARLITO_LIVE_TEST === "1" || env.CARLITO_LIVE_GATEWAY === "1" || env.LIVE === "1";
  if (!live) {
    return null;
  }
  const raw = env.CARLITO_LIVE_PROVIDER_DISCOVERY_TIMEOUT_MS?.trim();
  if (!raw) {
    return 15_000;
  }
...
function resolveProviderDiscoveryFilter(params: {
  ...
  const testRaw = env.CARLITO_TEST_ONLY_PROVIDER_PLUGIN_IDS?.trim();
  if (testRaw) {
    const ids = testRaw
      .split(",")
  ...
  const rawValues = [
    env.CARLITO_LIVE_PROVIDERS?.trim(),
    env.CARLITO_LIVE_GATEWAY_PROVIDERS?.trim(),
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): CarlitoConfig {
...
): Promise<NonNullable<CarlitoConfig["models"]>["providers"]> {
