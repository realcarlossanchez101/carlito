...
describe("CarlitoStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    ...
    const { CarlitoStdioClientTransport } = await import("./mcp-stdio-transport.js");

    const transport = new CarlitoStdioClientTransport({
      command: "npx",
      args: ["-y", "example-mcp"],
      env: { EXAMPLE: "1" },
      cwd: "/tmp/example",
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new CarlitoStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
