...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousCarlitoAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousCarlitoAgentDir = process.env.CARLITO_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousCarlitoAgentDir === undefined) {
      delete process.env.CARLITO_AGENT_DIR;
    } else {
      process.env.CARLITO_AGENT_DIR = previousCarlitoAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "CARLITO_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.CARLITO_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.CARLITO_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.CARLITO_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: CarlitoConfig = {
  models: {
    providers: {
      "custom-proxy": {
