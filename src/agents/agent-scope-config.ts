...
export function listAgentEntries(cfg: CarlitoConfig): AgentEntry[] {
...
export function listAgentIds(cfg: CarlitoConfig): string[] {
...
export function resolveDefaultAgentId(cfg: CarlitoConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
