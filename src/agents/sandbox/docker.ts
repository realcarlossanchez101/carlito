...
import { markCarlitoExecEnv } from "../../infra/carlito-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "carlito.sandbox=1");
  args.push("--label", `carlito.sessionKey=${params.scopeKey}`);
  args.push("--label", `carlito.createdAtMs=${createdAtMs}`);
  args.push("--label", `carlito.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `carlito.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markCarlitoExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "carlito.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`carlito sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`carlito sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("carlito sandbox recreate --all");
}
