...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<CarlitoConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
    id: "config-model",
    name: "Config model",
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-agent-"));
    try {
      const providers: NonNullable<NonNullable<CarlitoConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("replaces resolved env var value with env var name to prevent plaintext persistence", async () => {
    ...
    const env = {
      ...
      CARLITO_SKIP_PROVIDERS: undefined,
      CARLITO_TEST_MINIMAL_GATEWAY: undefined,
    };
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<CarlitoConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<CarlitoConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
