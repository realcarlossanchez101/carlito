...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("carlito-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("carlito-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("carlito-tilde-test-mkdir-");
  ...
  it("ignores CARLITO_HOME for write operations", async () => {
    const carlitoHome = await createTempDir("carlito-home-override-", os.tmpdir());
    const dir = await createTempDir("carlito-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("CARLITO_HOME", carlitoHome);

    createHostWorkspaceWriteTool(carlitoHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(carlitoHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores CARLITO_HOME for mkdir operations", async () => {
    const carlitoHome = await createTempDir("carlito-home-override-", os.tmpdir());
    const dir = await createTempDir("carlito-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(carlitoHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores CARLITO_HOME for readFile operations", async () => {
    const carlitoHome = await createTempDir("carlito-home-override-", os.tmpdir());
    const dir = await createTempDir("carlito-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(carlitoHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores CARLITO_HOME for access operations", async () => {
    const carlitoHome = await createTempDir("carlito-home-override-", os.tmpdir());
    const dir = await createTempDir("carlito-tilde-test-edit-");
