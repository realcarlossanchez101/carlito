...
import { planCarlitoModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: CarlitoConfig): {
  config: CarlitoConfig;
  sourceConfigForSecrets: CarlitoConfig;
} {
...
export async function ensureCarlitoModelsJson(
  config?: CarlitoConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveCarlitoAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planCarlitoModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
