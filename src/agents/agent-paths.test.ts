...
describe("resolveCarlitoAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          CARLITO_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveCarlitoAgentDir();
  ...
  it("honors CARLITO_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          CARLITO_STATE_DIR: undefined,
          CARLITO_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveCarlitoAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when CARLITO_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers CARLITO_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          CARLITO_STATE_DIR: undefined,
          CARLITO_AGENT_DIR: primaryOverride,
