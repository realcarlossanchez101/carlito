...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".carlito", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-exec-approvals-"));
  });

  beforeEach(async () => {
    previousHome = process.env.HOME;
    previousUserProfile = process.env.USERPROFILE;
    previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.CARLITO_DISABLE_BUNDLED_PLUGINS;
  ...
  afterEach(() => {
    ...
    if (previousDisableBundledPlugins === undefined) {
      ...
      process.env.CARLITO_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-test-bin-"));
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".carlito", "skills", "gog");
