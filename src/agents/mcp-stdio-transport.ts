...
export type CarlitoStdioServerParameters = {
  command: string;
  args?: string[];
...
export class CarlitoStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: CarlitoStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "CarlitoStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
