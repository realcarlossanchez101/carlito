...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseCarlitoManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyCarlitoManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): CarlitoSkillMetadata | undefined {
