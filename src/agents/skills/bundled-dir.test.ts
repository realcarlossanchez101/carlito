...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["CARLITO_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns CARLITO_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-bundled-override-"));
    process.env.CARLITO_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.CARLITO_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "carlito" }));
