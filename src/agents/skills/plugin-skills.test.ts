    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "carlito.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "carlito.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "carlito.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("carlito-");
  const acpxRoot = await tempDirs.make("carlito-acpx-plugin-");
  const helperRoot = await tempDirs.make("carlito-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("carlito-");
  const pluginRoot = await tempDirs.make("carlito-plugin-");
  const outsideDir = await tempDirs.make("carlito-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("carlito-");
    const pluginRoot = await tempDirs.make("carlito-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("carlito-");
    const pluginRoot = await tempDirs.make("carlito-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
