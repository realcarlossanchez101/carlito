...
function resolveAttachmentLimits(config: CarlitoConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".carlito", "attachments");
  const relDir = path.posix.join(".carlito", "attachments", attachmentId);
