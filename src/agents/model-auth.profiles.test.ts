...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "carlito-adc-"));
...
function buildDemoLocalProviderCfg(apiKey: string): CarlitoConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-oauth-"));
    ...
    try {
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.CARLITO_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-auth-"));
