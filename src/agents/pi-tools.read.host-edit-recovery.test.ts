...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when CARLITO_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const carlitoHome = path.join(tmpDir, "carlito-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(carlitoHome, { recursive: true });
    ...
    process.env.CARLITO_HOME = carlitoHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(carlitoHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
