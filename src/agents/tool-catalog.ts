...
function buildCoreToolGroupMap() {
  ...
  const carlitoTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInCarlitoGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:carlito": carlitoTools,
