...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CARLITO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      CARLITO_HOME: "/srv/carlito-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/carlito-home"), ".carlito", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".carlito", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("carlito-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("carlito-workspace-");
    await fs.mkdir(path.join(tempDir, ".carlito"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-workspace-hardlink-"));
    try {
