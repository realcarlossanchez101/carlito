...
const GOOGLE_LIVE_SINGLE_KEY = "CARLITO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "CARLITO_LIVE_ANTHROPIC_KEY",
    listVar: "CARLITO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "CARLITO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `CARLITO_LIVE_${base}_KEY`;
