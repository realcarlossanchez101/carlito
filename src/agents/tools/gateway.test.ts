...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    carlito: process.env.CARLITO_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.carlito === undefined) {
      delete process.env.CARLITO_GATEWAY_TOKEN;
    } else {
      process.env.CARLITO_GATEWAY_TOKEN = envSnapshot.carlito;
    }
  });
  ...
  it("uses CARLITO_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.CARLITO_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
