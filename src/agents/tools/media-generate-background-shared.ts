...
function buildMediaGenerationReplyInstruction(params: {
  ...
  if (params.status === "ok") {
    return [
      ...
      `If you cannot use the message tool, reply in your normal assistant voice and include the exact MEDIA: lines from the result so Carlito attaches the finished ${params.completionLabel}.`,
...
function isAsyncMediaDirectSendEnabled(config: CarlitoConfig | undefined): boolean {
