...
const DIRECT_ENABLED = Boolean(process.env.CARLITO_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const LIVE_PULSECHECK_MS = Math.max(1_000, toInt(process.env.CARLITO_LIVE_PULSECHECK_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.CARLITO_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
...
function resolveLiveModelsJsonTimeoutMs(
  modelsJsonTimeoutRaw = process.env.CARLITO_LIVE_MODELS_JSON_TIMEOUT_MS,
  ...
  it(
    ...
    async () => {
      ...
      await withLiveStageTimeout(
        ensureCarlitoModelsJson(cfg),
        "[live-models] prepare models.json",
        LIVE_MODELS_JSON_TIMEOUT_MS,
      );
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set CARLITO_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.CARLITO_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.CARLITO_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.CARLITO_LIVE_MODEL_TIMEOUT_MS, 30_000);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via CARLITO_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
