...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-subagent-"));
  ...
  it("uses isolated temp state when CARLITO_STATE_DIR is unset in tests", async () => {
    delete process.env.CARLITO_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "carlito-test-state"));
  });
});
