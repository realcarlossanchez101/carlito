...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/carlito/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/carlito/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue("/tmp/carlito/agents/codex/sessions/sess-123.jsonl");

    const resolved = resolveAcpSpawnStreamLogPath({
      childSessionKey: "agent:codex:acp:child-1",
    });

    expect(resolved).toBe("/tmp/carlito/agents/codex/sessions/sess-123.acp-stream.jsonl");
