...
async function withOutsideHardlinkInCarlitoTmp<T>(
  params: {
    carlitoTmpDir: string;
  ...
  const hardlinkPath = path.join(params.carlitoTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.carlitoTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.carlitoTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.carlitoTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const carlitoTmpDir = resolvePreferredCarlitoTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Carlito tmp root",
      media: path.join(carlitoTmpDir, "image.png"),
      expected: path.join(carlitoTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Carlito tmp root",
      media: pathToFileURL(path.join(carlitoTmpDir, "photo.png")).href,
      expected: path.join(carlitoTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Carlito tmp root",
      media: path.join(carlitoTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(carlitoTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(carlitoTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside carlito tmp root",
      media: path.join(os.tmpdir(), "outside-carlito", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Carlito tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(carlitoTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(carlitoTmpDir, { recursive: true });
      const symlinkPath = path.join(carlitoTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Carlito tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCarlitoTmp(
      {
        carlitoTmpDir,
  ...
  it("rejects symlinked Carlito tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCarlitoTmp(
