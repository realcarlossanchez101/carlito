...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-test-"));

    try {
      await withEnvAsync({ CARLITO_STATE_DIR: stateDir, CARLITO_PROFILE: "isolated" }, async () => {
        const tool = requireGatewayTool();
        ...
        expect(parsed.payload?.doctorHint).toBe(
          "Run: carlito --profile isolated doctor --non-interactive",
        );
      });
    } finally {
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/carlito-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/carlito-bin"] } } }',
      }),
    ).rejects.toThrow(
