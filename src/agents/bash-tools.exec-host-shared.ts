...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.carlito/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "carlito doctor" and "carlito approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
