...
describe("FS tools with workspaceOnly=false", () => {
  ...
  const toolsFor = (workspaceOnly: boolean | undefined): AnyAgentTool[] => {
    const read = createCarlitoReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool);
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-test-"));
  ...
  it("restricts memory-triggered writes to append-only canonical memory files", async () => {
    ...
    const tools = [
      createCarlitoReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool),
