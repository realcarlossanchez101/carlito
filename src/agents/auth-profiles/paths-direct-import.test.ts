...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveCarlitoAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveCarlitoAgentDir()`
    // nullish branch. With CARLITO_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-carlito-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-path-ensure-"));
    process.env.CARLITO_STATE_DIR = stateDir;
  });
