...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- Always use `ref="cv_..."` for hosted documents. Do not construct explicit hosted URLs in `[embed ...]`; the gateway resolves `ref` to the correct URL internally.',
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Carlito handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Carlito docs: ${docsPath}`,
    "Mirror: https://github.com/realcarlossanchez101/carlito",
    "Source: https://github.com/realcarlossanchez101/carlito",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For Carlito behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `carlito status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Carlito process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Carlito agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Carlito agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Carlito.";
  }
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Carlito's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    ...safetySection,
    "## Carlito CLI Quick Reference",
    "Carlito is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- carlito gateway status",
    "- carlito gateway start",
    "- carlito gateway stop",
    "- carlito gateway restart",
    "If unsure, ask the user to run `carlito help` (or `carlito gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Carlito Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Carlito pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Carlito and included below in Project Context.",
    "",
