...
const CARLITO_CLAUDE_PLUGIN_NAME = "carlito-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredCarlitoTmpDir(), "carlito-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, CARLITO_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: CARLITO_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Carlito skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
