...
const CLI_BACKEND_PRESERVE_ENV = "CARLITO_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.CARLITO_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.CARLITO_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.CARLITO_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.CARLITO_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.CARLITO_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.CARLITO_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.CARLITO_MCP_TOKEN ||
            env.CARLITO_MCP_SESSION_KEY ||
            env.CARLITO_MCP_SENDER_IS_OWNER
          ) {
