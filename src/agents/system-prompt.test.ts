...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes embed guidance only for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.carlito-dev/canvas",
      },
    });
    ...
    expect(prompt).toContain(
      "The active hosted embed root for this session is: `/Users/example/.carlito-dev/canvas`.",
    );
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Carlito docs: /tmp/carlito/docs");
    expect(prompt).toContain(
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/carlito. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`carlito ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Carlito subagents");
