...
function gatewayTokenConfig(config: CarlitoConfig): CarlitoConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function buildSnapshot(config: CarlitoConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/carlito-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: CarlitoConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.CARLITO_SKIP_CHANNELS;
  const previousSkipProviders = process.env.CARLITO_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.CARLITO_SKIP_CHANNELS;
    } else {
      process.env.CARLITO_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.CARLITO_SKIP_PROVIDERS;
    } else {
      process.env.CARLITO_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
