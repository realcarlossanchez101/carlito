...
export function attachCarlitoTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__carlito && typeof record.__carlito === "object" && !Array.isArray(record.__carlito)
      ? (record.__carlito as Record<string, unknown>)
      : {};
  return {
    ...record,
    __carlito: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachCarlitoTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "carlito" && model === "delivery-mirror";
