...
export function resolveGatewayReloadSettings(cfg: CarlitoConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: CarlitoConfig;
  initialCompareConfig?: CarlitoConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: CarlitoConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: CarlitoConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: CarlitoConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: CarlitoConfig, nextCompareConfig: CarlitoConfig) => {
