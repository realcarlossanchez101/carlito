  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-carlito-account-id": "work",
        "x-carlito-message-channel": "telegram",
      },
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-carlito-message-channel": "matrix",
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
  ...
  it("ignores spoofed owner headers when the bearer token is non-owner scoped", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-session-key": "agent:main:matrix:dm:test",
        "x-carlito-message-channel": "matrix",
        "x-carlito-sender-is-owner": "true",
      },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
    });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.carlito?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.carlito?.headers?.Authorization).toBe("Bearer ${CARLITO_MCP_TOKEN}");
    expect(config.mcpServers?.carlito?.headers?.["x-carlito-message-channel"]).toBe(
      "${CARLITO_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.carlito?.headers?.["x-carlito-sender-is-owner"]).toBeUndefined();
  });
});
