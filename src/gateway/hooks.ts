...
export function resolveHooksConfig(cfg: CarlitoConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: CarlitoConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-carlito-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-carlito-idempotency-key"]) ||
