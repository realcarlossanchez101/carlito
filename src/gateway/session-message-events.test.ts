...
beforeAll(async () => {
  previousMinimalGateway = process.env.CARLITO_TEST_MINIMAL_GATEWAY;
...
afterAll(async () => {
  ...
  if (previousMinimalGateway === undefined) {
    ...
    process.env.CARLITO_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
  }
});
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-session-message-"));
