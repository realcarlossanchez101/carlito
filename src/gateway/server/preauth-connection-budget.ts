...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__carlito_unknown_client_ip__";

export function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.CARLITO_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.CARLITO_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
