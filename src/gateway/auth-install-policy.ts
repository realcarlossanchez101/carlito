...
type GatewayInstallAuthMode = NonNullable<NonNullable<CarlitoConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: CarlitoConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.CARLITO_GATEWAY_PASSWORD) ||
