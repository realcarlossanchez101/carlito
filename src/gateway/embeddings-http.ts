...
function resolveAutoExplicitProviders(cfg: CarlitoConfig): Set<string> {
  return new Set(
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== CARLITO_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-carlito-model")) ||
