...
import { ensureCarlitoModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, CarlitoConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.CARLITO_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.CARLITO_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.CARLITO_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.CARLITO_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_PULSECHECK_MS = Math.max(
  1_000,
  toInt(process.env.CARLITO_LIVE_GATEWAY_PULSECHECK_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.CARLITO_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["CARLITO_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.CARLITO_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.CARLITO_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.CARLITO_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.CARLITO_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.CARLITO_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.CARLITO_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.CARLITO_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.CARLITO_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("CARLITO_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("CARLITO_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("CARLITO_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.CARLITO_LIVE_GATEWAY_MODELS;
    delete process.env.CARLITO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.CARLITO_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.CARLITO_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.CARLITO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.CARLITO_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.CARLITO_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: CarlitoConfig;
  agentDir: string;
}): Promise<CarlitoConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<CarlitoConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.CARLITO_CONFIG_PATH,
    token: process.env.CARLITO_GATEWAY_TOKEN,
    skipChannels: process.env.CARLITO_SKIP_CHANNELS,
    skipGmail: process.env.CARLITO_SKIP_GMAIL_WATCHER,
    skipCron: process.env.CARLITO_SKIP_CRON,
    skipCanvas: process.env.CARLITO_SKIP_CANVAS_HOST,
    disableBonjour: process.env.CARLITO_DISABLE_BONJOUR,
    logLevel: process.env.CARLITO_LOG_LEVEL,
    agentDir: process.env.CARLITO_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.CARLITO_STATE_DIR,
  };
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.CARLITO_DISABLE_BONJOUR = "1";
    process.env.CARLITO_LOG_LEVEL = "silent";
  }
  ...
  const hostAgentDir = resolveCarlitoAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-live-state-"));
  ...
  process.env.CARLITO_AGENT_DIR = tempAgentDir;
  process.env.PI_CODING_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".carlito"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".carlito", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.carlito-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveCarlitoAgentDir();
  const sanitizedCfg: CarlitoConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-live-"));
  const tempConfigPath = path.join(tempDir, "carlito.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.CARLITO_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "Carlito live tool probe (local, safe): " +
    ...
    process.env.CARLITO_CONFIG_PATH = previous.configPath;
    process.env.CARLITO_GATEWAY_TOKEN = previous.token;
    process.env.CARLITO_SKIP_CHANNELS = previous.skipChannels;
    process.env.CARLITO_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.CARLITO_SKIP_CRON = previous.skipCron;
    process.env.CARLITO_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.CARLITO_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.CARLITO_LOG_LEVEL = previous.logLevel;
    process.env.CARLITO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CARLITO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await ensureCarlitoModelsJson(cfg);
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via CARLITO_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.carlito-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
