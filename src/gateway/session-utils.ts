...
function listConfiguredAgentIds(cfg: CarlitoConfig): string[] {
...
export function listAgentsForGateway(cfg: CarlitoConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
export function loadCombinedSessionStoreForGateway(cfg: CarlitoConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: CarlitoConfig): GatewaySessionsDefaults {
