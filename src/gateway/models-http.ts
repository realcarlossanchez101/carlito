...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([CARLITO_MODEL_ID, CARLITO_DEFAULT_MODEL_ID]);
  ids.add(`carlito/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`carlito/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== CARLITO_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
