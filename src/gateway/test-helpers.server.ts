...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.CARLITO_CONFIG_PATH) {
    configPaths.add(process.env.CARLITO_CONFIG_PATH);
  }
  if (process.env.CARLITO_STATE_DIR) {
    configPaths.add(path.join(process.env.CARLITO_STATE_DIR, "carlito.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.CARLITO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "carlito-test-no-bundled-extensions")
    : "carlito-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".carlito-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".carlito-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".carlito-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>carlito-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__carlitoTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__carlitoTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["CARLITO_GATEWAY_TOKEN"]);
  const prev = process.env.CARLITO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CARLITO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.CARLITO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
