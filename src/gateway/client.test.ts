...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("carlito doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with CARLITO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with CARLITO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.CARLITO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://carlito-gateway.ai:18789",
      onConnectError,
    });
