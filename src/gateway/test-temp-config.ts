...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "carlito-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.CARLITO_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as CarlitoConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "carlito-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.CARLITO_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
