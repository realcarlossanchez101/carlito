...
function loadGatewayConfig(): CarlitoConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: CarlitoConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.CARLITO_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as CarlitoConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `carlito doctor` for diagnostics.";
  }
  return message;
}
