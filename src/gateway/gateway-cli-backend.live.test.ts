...
import { clearRuntimeConfigSnapshot, type CarlitoConfig } from "../config/config.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.CARLITO_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.CARLITO_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.CARLITO_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.CARLITO_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.CARLITO_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "CARLITO_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.CARLITO_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.CARLITO_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      const parsed = parseModelRef(rawModel, "claude-cli");
      if (!parsed) {
        throw new Error(
          `CARLITO_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
        );
      }
      ...
      const cliCommand = process.env.CARLITO_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `CARLITO_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }
      ...
      const cliClearEnv =
        parseJsonStringArray(
          "CARLITO_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.CARLITO_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const cliImageArg =
        process.env.CARLITO_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.CARLITO_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;
      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "CARLITO_LIVE_CLI_BACKEND_IMAGE_MODE requires CARLITO_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }

      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-live-cli-"));
      ...
      const bootstrapWorkspace =
        backendResolved?.bundleMcpMode === "claude-config-file"
          ...
          : null;
      const disableMcpConfig = process.env.CARLITO_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      const cfgWithCliBackends = cfg as CarlitoConfig & {
        agents?: {
          defaults?: {
