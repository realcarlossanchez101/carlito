...
const { createCarlitoTools } = await import("../agents/carlito-tools.js");
...
type SessionSendTool = ReturnType<typeof createCarlitoTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createCarlitoTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["CARLITO_GATEWAY_PORT", "CARLITO_GATEWAY_TOKEN"]);
  ...
  process.env.CARLITO_GATEWAY_PORT = String(gatewayPort);
  process.env.CARLITO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("CARLITO_CONFIG_PATH missing in gateway test environment");
      }
