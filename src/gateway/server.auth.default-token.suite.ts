...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers CARLITO_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.CARLITO_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CARLITO_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.CARLITO_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CARLITO_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CARLITO_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CARLITO_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
    ...
    test("hello-ok reports persisted token scopes when reusing an existing device token", async () => {
      ...
      const deviceIdentityPath = path.join(
        os.tmpdir(),
        `carlito-shared-auth-scope-reuse-${randomUUID()}.json`,
      );
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `carlito-test-device-${randomUUID()}.json`),
        nonce,
      });
