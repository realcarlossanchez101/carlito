...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.CARLITO_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: CarlitoConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: CarlitoConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: CarlitoConfig, requestedAgentId: string) => {
