...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: CarlitoConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.CARLITO_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.CARLITO_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set CARLITO_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: carlito doctor --fix",
