...
function cfg(input: Partial<CarlitoConfig>): CarlitoConfig {
  return input as CarlitoConfig;
}
...
const DEFAULT_ENV = {
  ...
  CARLITO_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when CARLITO env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when CARLITO env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when CARLITO env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
