...
export function resolveAcpInstallCommandHint(cfg: CarlitoConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `carlito plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^carlito plugins install /u, "");
    ...
    return "carlito plugins install acpx";
  }
