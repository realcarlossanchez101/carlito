...
describe("initSessionState - pulsecheck should not trigger session reset", () => {
  let tempDir: string;
  ...
  it("should NOT reset session when Provider is 'pulsecheck'", async () => {
    ...
    const ctx = createBaseCtx({
      Provider: "pulsecheck", // Pulsecheck provider should NOT trigger reset
      Body: "PULSECHECK_OK",
    });
  ...
  it("should reset session when Provider is NOT 'pulsecheck' and session is stale", async () => {
    // Setup: Create a session entry that is "stale" (older than idle timeout)
    const now = Date.now();
    const staleTime = now - 10 * 60 * 1000; // 10 minutes ago (exceeds 5min idle timeout)
  ...
  it("should preserve session when Provider is 'pulsecheck' even with daily reset mode", async () => {
  ...
  it("should handle cron-event provider same as pulsecheck (no reset)", async () => {
  ...
  it("should handle exec-event provider same as pulsecheck (no reset)", async () => {
    // Setup: Create a stale session
    const now = Date.now();
    const staleTime = now - 10 * 60 * 1000;
