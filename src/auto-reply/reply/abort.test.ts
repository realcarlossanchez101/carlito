...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop carlito!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@carlito_bot", { botUsername: "carlito_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@carlito_bot", { botUsername: "carlito_bot" })).toBe(true);
