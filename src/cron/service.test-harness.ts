...
export function createCronStoreHarness(options?: { prefix?: string }) {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), options?.prefix ?? "carlito-cron-"));
  });
...
export function createStartedCronServiceWithFinishedBarrier(params: {
  ...
  return { cron, enqueueSystemEvent, requestPulsecheckNow, finished };
}
...
export async function withCronServiceForTest(
  ...
  await cron.start();
  try {
    await run({ cron, enqueueSystemEvent, requestPulsecheckNow });
  } finally {
    cron.stop();
    await store.cleanup();
  }
