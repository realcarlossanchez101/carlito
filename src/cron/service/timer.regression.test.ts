...
describe("cron service timer regressions", () => {
  ...
  it("respects abort signals while retrying one-shot main-session wake-now pulsecheck runs", async () => {
    const abortController = new AbortController();
    const runPulsecheckOnce = vi.fn(
      async (): Promise<PulsecheckRunResult> => ({
        status: "skipped",
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/carlito-cron-abort-test/jobs.json",
      log: noopLogger,
      nowMs: () => Date.now(),
      enqueueSystemEvent,
      requestPulsecheckNow,
      runPulsecheckOnce,
      wakeNowPulsecheckBusyMaxWaitMs: 30,
      wakeNowPulsecheckBusyRetryDelayMs: 5,
      runIsolatedAgentJob: createDefaultIsolatedRunner(),
    });
  ...
  it("finishes recurring wake-now main jobs quickly when the main lane is busy (#58833)", async () => {
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/carlito-cron-busy-main-test/jobs.json",
      log: noopLogger,
      nowMs,
      enqueueSystemEvent,
      requestPulsecheckNow,
      runPulsecheckOnce,
      wakeNowPulsecheckBusyMaxWaitMs: 120_000,
      wakeNowPulsecheckBusyRetryDelayMs: 250,
      runIsolatedAgentJob: createDefaultIsolatedRunner(),
    });
