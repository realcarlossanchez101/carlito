...
describe("canvas host state dir defaults", () => {
  ...
  it("uses CARLITO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("carlito-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Carlito Canvas");
      } finally {
        await handler.close();
      }
    });
