...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.carlitoCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.carlitoCanvasA2UIAction &&
        typeof window.carlitoCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.carlitoSendUserAction === "function";
  ...
  window.addEventListener("carlito:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Carlito node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.carlitoSendUserAction === "function"
        ? window.carlitoSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CARLITO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.CARLITO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Carlito Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
