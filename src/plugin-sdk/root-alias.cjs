...
const pluginSdkPackageNames = ["carlito/plugin-sdk", "@realcarlossanchez101/plugin-sdk"];
...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.CARLITO_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function onDiagnosticEvent(listener) {
  ...
  if (!diagnosticEvents || typeof diagnosticEvents.onDiagnosticEvent !== "function") {
    throw new Error("carlito/plugin-sdk root alias could not resolve onDiagnosticEvent");
  }
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.CARLITO_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.CARLITO_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function getJiti(tryNative) {
  ...
  const jitiLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled Carlito core graph.
