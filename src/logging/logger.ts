...
import { POSIX_CARLITO_TMP_DIR, resolvePreferredCarlitoTmpDir } from "../infra/tmp-carlito-dir.js";
...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredCarlitoTmpDir() : POSIX_CARLITO_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "carlito.log")
    : `${POSIX_CARLITO_TMP_DIR}/carlito.log`;
}

export const DEFAULT_LOG_DIR = resolveDefaultLogDir();
export const DEFAULT_LOG_FILE = resolveDefaultLogFile(DEFAULT_LOG_DIR); // legacy single-file path

const LOG_PREFIX = "carlito";
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: CarlitoConfig["logging"] | undefined =
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.CARLITO_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[carlito] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
