...
const logPathTracker = createSuiteLogPathTracker("carlito-test-env-log-level-");

describe("CARLITO_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.CARLITO_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.CARLITO_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.CARLITO_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.CARLITO_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("CARLITO_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid CARLITO_LOG_LEVEL="nope"');
  });
});
