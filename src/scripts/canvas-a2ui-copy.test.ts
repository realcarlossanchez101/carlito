...
const ORIGINAL_SKIP_MISSING = process.env.CARLITO_A2UI_SKIP_MISSING;
const ORIGINAL_SPARSE_PROFILE = process.env.CARLITO_SPARSE_PROFILE;

describe("canvas a2ui copy", () => {
  afterEach(() => {
    if (ORIGINAL_SKIP_MISSING === undefined) {
      delete process.env.CARLITO_A2UI_SKIP_MISSING;
    } else {
      process.env.CARLITO_A2UI_SKIP_MISSING = ORIGINAL_SKIP_MISSING;
    }

    if (ORIGINAL_SPARSE_PROFILE === undefined) {
      delete process.env.CARLITO_SPARSE_PROFILE;
    } else {
      process.env.CARLITO_SPARSE_PROFILE = ORIGINAL_SPARSE_PROFILE;
    }
  });

  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    await withTempDir("carlito-a2ui-", run);
  }
  ...
  it("skips missing assets when CARLITO_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.CARLITO_A2UI_SKIP_MISSING = "1";
      await expect(
  ...
  it("skips missing assets when CARLITO_SPARSE_PROFILE is set", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.CARLITO_SPARSE_PROFILE = "core";
      await expect(
        copyA2uiAssets({ srcDir: dir, outDir: path.join(dir, "out") }),
      ).resolves.toBeUndefined();
    });
