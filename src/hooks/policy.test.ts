...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "carlito-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "carlito-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "carlito-bundled");
      const plugin = makeHookEntry("shared", "carlito-plugin");
      const managed = makeHookEntry("shared", "carlito-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("carlito-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "carlito-managed");
      const workspace = makeHookEntry("shared", "carlito-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "carlito-workspace");
      const second = makeHookEntry("shared", "carlito-workspace");
      second.hook.handlerPath = "/tmp/carlito-workspace/shared/handler-2.js";
