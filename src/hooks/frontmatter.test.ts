...
`;
    ...
    expect(parsed.carlito.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.carlito.emoji).toBe("📝");
    expect(parsed.carlito.events).toEqual(["command"]);
    expect(parsed.carlito.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.carlito.install[0].kind).toBe("bundled");
  });
...
describe("resolveCarlitoMetadata", () => {
  it("extracts carlito metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveCarlitoMetadata(frontmatter);
  ...
  it("returns undefined when carlito key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        carlito: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Carlito" },
            { id: "npm", kind: "npm", package: "@realcarlossanchez101/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@realcarlossanchez101/hook");
  });
    ...
    "carlito":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Carlito" }],
      },
  }
---
...
`;
    ...
    const carlito = resolveCarlitoMetadata(frontmatter);
    expect(carlito).toBeDefined();
    expect(carlito?.emoji).toBe("💾");
    expect(carlito?.events).toEqual(["command:new", "command:reset"]);
    expect(carlito?.requires?.config).toEqual(["workspace.dir"]);
    expect(carlito?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const carlito = resolveCarlitoMetadata(frontmatter);
    expect(carlito?.emoji).toBe("disk");
    expect(carlito?.events).toEqual(["command:new"]);
  });
});
