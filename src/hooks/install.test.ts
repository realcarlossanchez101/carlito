...
const fixtureRoot = path.join(process.cwd(), ".tmp", `carlito-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@realcarlossanchez101/test-hooks",
      version: "0.0.1",
      carlito: { hooks: params.hooks },
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "carlito.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "carlito.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("does not expose dangerous force unsafe install through npm-spec archive params", async () => {
    ...
    try {
      const result = await installHooksFromNpmSpec({
        spec: "@realcarlossanchez101/test-hooks@0.0.1",
      });
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@realcarlossanchez101/test-hooks@0.0.1",
              name: "@realcarlossanchez101/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@realcarlossanchez101/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@realcarlossanchez101/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@realcarlossanchez101/test-hooks@0.0.2-beta.1",
      name: "@realcarlossanchez101/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@realcarlossanchez101/test-hooks@beta"');
    }
  });
});
