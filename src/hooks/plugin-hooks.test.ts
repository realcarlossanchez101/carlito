...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "carlito-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.CARLITO_BUNDLED_HOOKS_DIR;
    process.env.CARLITO_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.CARLITO_BUNDLED_HOOKS_DIR;
    } else {
      process.env.CARLITO_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".carlito", "extensions", "sample-bundle");
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"carlito":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): CarlitoConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("carlito-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Carlito hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".carlito", "extensions", "claude-bundle");
