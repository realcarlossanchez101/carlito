...
homepage: https://docs.carlito.ai/automation/hooks#command-logger
metadata:
  {
    "carlito":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.carlito/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.carlito/logs/commands.log ~/.carlito/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/carlito`:
   ```
   /home/username/.carlito/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
