...
This directory contains hooks that ship with Carlito. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.carlito/workspace`)
...
homepage: https://docs.carlito.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.carlito/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.carlito/carlito.json`:
...
2. Restart gateway: `pkill -9 -f 'carlito.*gateway' && pnpm carlito gateway`
3. Enable the hook: `carlito hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.carlito.ai/automation/hooks
