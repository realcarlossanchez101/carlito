...
function listAgents(cfg: CarlitoConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: CarlitoConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<CarlitoConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: CarlitoConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: CarlitoConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: CarlitoConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<CarlitoConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  CarlitoConfig,
  {
    bindingsRef: CarlitoConfig["bindings"];
    agentsRef: CarlitoConfig["agents"];
    sessionRef: CarlitoConfig["session"];
...
function resolveRouteCacheForConfig(cfg: CarlitoConfig): Map<string, ResolvedAgentRoute> {
