...
import type { CarlitoConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitPulsecheckDirectPolicyWarnings(cfg: CarlitoConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: CarlitoConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Carlito default (full)";
  const defaultRequestedAskSource = "Carlito default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("carlito approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: CarlitoConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: CarlitoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("carlito security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.carlito/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("carlito approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.carlito.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("carlito configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("carlito config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("carlito doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("carlito config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('carlito config set session.dmScope "per-channel-peer"') +
