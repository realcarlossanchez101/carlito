...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.CARLITO_STATE_DIR ?? "/tmp/carlito-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.CARLITO_AGENT_DIR ?? "/tmp/carlito-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under CARLITO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("carlito-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-oauth-sync-"));
    ...
    process.env.CARLITO_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-oauth-nosync-"));
    process.env.CARLITO_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside CARLITO_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-oauth-external-"));
    process.env.CARLITO_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* CARLITO_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("carlito-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("carlito-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to CARLITO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("carlito-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
