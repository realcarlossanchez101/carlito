...
async function runRepair(cfg: CarlitoConfig) {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      CARLITO_GATEWAY_TOKEN: "stale-token",
    },
  });
  mocks.auditGatewayServiceConfig.mockResolvedValue({
    ok: false,
    issues: [
      {
        ...
        message: "Gateway service CARLITO_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    fsMocks.realpath.mockImplementation(async (value: string) => value);
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: CarlitoConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.CARLITO_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses CARLITO_GATEWAY_TOKEN when config token is missing", async () => {
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/carlito/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/carlito@2026.3.12/node_modules/carlito/dist/index.js",
      realpath: async (value: string) => {
        if (value.includes("/global/5/node_modules/carlito/")) {
          return value.replace(
            "/global/5/node_modules/carlito/",
            "/global/5/node_modules/.pnpm/carlito@2026.3.12/node_modules/carlito/",
          );
        }
        return value;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/carlito/../carlito/dist/index.js",
      installEntrypoint: "/opt/carlito/dist/index.js",
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/carlito/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/carlito/dist/index.js",
    });
  ...
  it("repairs entrypoint mismatch in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/carlito/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/carlito/dist/index.js",
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            CARLITO_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Carlito gateway next.",
    );
  });
});
