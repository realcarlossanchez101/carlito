...
describe("noteMemorySearchHealth", () => {
  ...
  beforeEach(() => {
    ...
    resolveActiveMemoryBackendConfig.mockImplementation(({ cfg }: { cfg: CarlitoConfig }) =>
  ...
  it("does not warn when QMD backend is active", async () => {
    const qmdCfg = { memory: { backend: "qmd", qmd: { command: "qmd" } } } as CarlitoConfig;
    resolveMemorySearchConfig.mockReturnValue({
      provider: "auto",
      local: {},
      remote: {},
  ...
  it("warns when lmstudio gateway probe is unavailable", async () => {
    ...
    expect(message).toContain("carlito memory status --deep");
  });
  ...
  it("uses model configure hint when gateway probe is unavailable and API key is missing", async () => {
    ...
    expect(message).toContain("carlito configure --section model");
    expect(message).not.toContain("carlito auth add --provider");
  });
...
describe("memory recall doctor integration", () => {
  ...
  it("runs dreaming artifact repair during doctor --fix", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValueOnce({
      changed: true,
      archiveDir: "/tmp/agent-default/workspace/.carlito-repair/dreaming/2026-04-11T21-35-00-000Z",
      archivedDreamsDiary: false,
...
describe("detectLegacyWorkspaceDirs", () => {
  it("returns active workspace and no legacy dirs", () => {
    const workspaceDir = "/home/user/carlito";
