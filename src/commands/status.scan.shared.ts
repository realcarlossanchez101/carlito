...
export function hasExplicitMemorySearchConfig(cfg: CarlitoConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: CarlitoConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: CarlitoConfig, agentId: string) => { store: { path: string } } | null;
