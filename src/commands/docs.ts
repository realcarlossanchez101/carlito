...
const SEARCH_TOOL = "https://docs.carlito.ai/mcp.SearchCarlito";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.carlito.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('carlito docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.carlito.ai/");
      runtime.log(`Search: ${formatCliCommand('carlito docs "your query"')}`);
    }
    return;
  }
