...
export async function loadValidConfigOrThrow(): Promise<CarlitoConfig> {
...
export async function updateConfig(
  mutator: (cfg: CarlitoConfig) => CarlitoConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: CarlitoConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: CarlitoConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("carlito agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
