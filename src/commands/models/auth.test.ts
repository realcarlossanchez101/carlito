...
vi.mock("../provider-auth-helpers.js", () => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: CarlitoConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: CarlitoConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: CarlitoConfig;
  let lastUpdatedConfig: CarlitoConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/carlito/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/carlito/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/carlito/workspace");
    mocks.isRemoteEnvironment.mockReturnValue(false);
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: CarlitoConfig) => CarlitoConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/carlito/agents/main");
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai-codex:user@example.com",
      agentDir: "/tmp/carlito/agents/main",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with carlito configure --section web (recommended mode: cached). Docs: https://docs.carlito.ai/tools/web",
    );
  });
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/carlito/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/carlito/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `carlito plugins list --json`.',
    );
  });
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith("Anthropic setup-token auth is supported in Carlito.");
    expect(runtime.log).toHaveBeenCalledWith(
