...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("carlito plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("carlito plugins install")}\`.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in Carlito.");
    runtime.log("Carlito prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this Carlito path is allowed again.");
  }
}
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("carlito plugins install")}\`.`,
    );
  }
