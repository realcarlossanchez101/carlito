...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveCarlitoAgentDir: vi.fn().mockReturnValue("/tmp/carlito-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/carlito-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/carlito-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/carlito-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/carlito-dev/carlito.json",
    }),
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/carlito-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveCarlitoAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/carlito-dev/carlito.json");
    expect(payload.auth.storePath).toBe("/tmp/carlito-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/carlito-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/carlito-agent-custom");
