    ...
    findLegacyConfigIssues: (raw: unknown, sourceRaw?: unknown, extraRules: LegacyRule[] = []) => {
      ...
      if ("pulsecheck" in root) {
        addIssue(
          issues,
          ["pulsecheck"],
          'pulsecheck is legacy; use agents.defaults.pulsecheck and channels.defaults.pulsecheck. Run "carlito doctor --fix".',
        );
      }
      if ("memorySearch" in root) {
        addIssue(
          issues,
          ["memorySearch"],
          'memorySearch is legacy; use agents.defaults.memorySearch. Run "carlito doctor --fix".',
        );
      }
      ...
      if (gateway && "bind" in gateway) {
        addIssue(
          issues,
          ["gateway", "bind"],
          'gateway.bind host aliases are legacy; use the canonical bind mode. Run "carlito doctor --fix".',
        );
      }
      ...
      if (sessionThreadBindings && "ttlHours" in sessionThreadBindings) {
        addIssue(
          ...
          'session.threadBindings.ttlHours is legacy; use session.threadBindings.idleHours. Run "carlito doctor --fix".',
        );
      }
      ...
      if (xSearch && "apiKey" in xSearch) {
        addIssue(
          ...
          'tools.web.x_search.apiKey is legacy; use plugins.entries.xai.config.webSearch.apiKey. Run "carlito doctor --fix".',
        );
      }
      ...
      if (sandbox && "perSession" in sandbox) {
        addIssue(
          ...
          'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope. Run "carlito doctor --fix".',
        );
      }
      ...
      for (const [channelId, channelRaw] of Object.entries(channels ?? {})) {
        ...
        if (hasLegacyStreamingAlias(channel)) {
          addIssue(
            ...
            channelId === "googlechat"
              ? `channels.${channelId}.streamMode is legacy and no longer used. Run "carlito doctor --fix".`
              : `channels.${channelId}.streamMode, channels.${channelId}.streaming aliases are legacy. Run "carlito doctor --fix".`,
          );
        }
        ...
        if (threadBindings && "ttlHours" in threadBindings) {
          addIssue(
            ...
            'channels.<id>.threadBindings.ttlHours is legacy; use channels.<id>.threadBindings.idleHours. Run "carlito doctor --fix".',
          );
        }
        if (channelId === "slack") {
          for (const roomRaw of Object.values(asRecord(channel.channels) ?? {})) {
            if ("allow" in (asRecord(roomRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "slack"],
                'channels.slack.channels.<id>.allow is legacy; use enabled. Run "carlito doctor --fix".',
              );
            }
          }
        }
          ...
          for (const spaceRaw of Object.values(asRecord(channel.groups) ?? {})) {
            if ("allow" in (asRecord(spaceRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "googlechat"],
                'channels.googlechat.groups.<id>.allow is legacy; use enabled. Run "carlito doctor --fix".',
              );
            }
          }
        }
            ...
            for (const roomRaw of Object.values(asRecord(guild?.channels) ?? {})) {
              if ("allow" in (asRecord(roomRaw) ?? {})) {
                addIssue(
                  issues,
                  ["channels", "discord"],
                  'channels.discord.guilds.<id>.channels.<id>.allow is legacy; use enabled. Run "carlito doctor --fix".',
                );
              }
            }
          }
    ...
    listPluginDoctorLegacyConfigRules: () => [
      {
        ...
        message:
          'channels.telegram.groupMentionsOnly was removed; use channels.telegram.groups."*".requireMention instead. Run "carlito doctor --fix".',
      },
      {
        path: ["talk"],
        message:
          "talk.voiceId/talk.voiceAliases/talk.modelId/talk.outputFormat/talk.apiKey are legacy; use talk.providers.<provider> and run carlito doctor --fix.",
...
vi.mock("./doctor-config-preflight.js", async () => {
  ...
  function resolveConfigPath() {
    const stateDir =
      process.env.CARLITO_STATE_DIR ||
      (process.env.HOME ? path.join(process.env.HOME, ".carlito") : "");
    return process.env.CARLITO_CONFIG_PATH || path.join(stateDir, "carlito.json");
  }
  ...
  it("converts numeric discord ids to strings on repair", async () => {
    await withTempHome(
      async (home) => {
        ...
        await fs.mkdir(configDir, { recursive: true });
        await fs.writeFile(
          path.join(configDir, "carlito.json"),
          JSON.stringify(
            {
              channels: {
                discord: {
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            message.includes('Run "carlito doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockClear();
