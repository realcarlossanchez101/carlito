...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: CarlitoConfig }) =>
      Boolean(config.auth?.profiles?.["openai-codex:default"]),
    ),
    setupChannels: vi.fn(async (cfg: CarlitoConfig) => cfg),
  };
});
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.carlito/carlito.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.carlito/workspace",
  applyWizardMetadata: (cfg: CarlitoConfig) => cfg,
...
function setupBaseWizardState(config: CarlitoConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: CarlitoConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: CarlitoConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(retryCall.nextConfig).toMatchObject({
      agents: {
        defaults: {
          workspace: expect.stringContaining("/.carlito/workspace"),
        },
      },
      plugins: {
        entries: {
