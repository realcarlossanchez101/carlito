...
function usesClaudeCliModelSelection(cfg: CarlitoConfig): boolean {
...
function hasClaudeCliConfigSignals(cfg: CarlitoConfig): boolean {
...
function resolveClaudeCliCommand(cfg: CarlitoConfig): string {
...
function formatWorkspaceHealthLine(workspaceDir: string, health: ClaudeCliDirHealth): string {
  ...
  if (health === "missing") {
    return `- Workspace: ${display} (missing; Carlito will create it on first run).`;
  }
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(`- Carlito auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- Carlito auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- Carlito auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
