...
type EnvSnapshot = {
  HOME?: string;
  CARLITO_HOME?: string;
  CARLITO_STATE_DIR?: string;
  CARLITO_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    CARLITO_HOME: process.env.CARLITO_HOME,
    CARLITO_STATE_DIR: process.env.CARLITO_STATE_DIR,
    CARLITO_OAUTH_DIR: process.env.CARLITO_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: CarlitoConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: CarlitoConfig) {
...
async function runStateIntegrityText(cfg: CarlitoConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.CARLITO_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when CARLITO_OAUTH_DIR is explicitly configured", async () => {
    process.env.CARLITO_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: CarlitoConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.CARLITO_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it("prints carlito-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: CarlitoConfig = {};
    ...
    expect(text).toMatch(/carlito sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/carlito sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /carlito sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
