...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Carlito", ".carlito");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/carlito-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".carlito");
  ...
  it("anchors cloud detection to OS homedir when CARLITO_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".carlito");
    const originalCarlitoHome = process.env.CARLITO_HOME;
    process.env.CARLITO_HOME = "/tmp/carlito-home-override";
    ...
    try {
      ...
      if (originalCarlitoHome === undefined) {
