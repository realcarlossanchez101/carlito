...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/carlito/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\carlito\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/carlito/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { CARLITO_PORT: "3000" },
  } = params;
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.carlito/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ CARLITO_PORT: "3000" });
  ...
  it("merges safe config env while dropping unsafe values and keeping service precedence", async () => {
    ...
    expect(plan.environment.CARLITO_PORT).toBe("3000");
    expect(plan.environment.CARLITO_SERVICE_MANAGED_ENV_KEYS).toBe(
      "CUSTOM_VAR,GOOGLE_API_KEY,CARLITO_PORT,SAFE_KEY",
    );
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("merges .env vars with config and service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".carlito"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { CARLITO_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.CARLITO_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        CARLITO_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
    ...
    expect(plan.environment.BLOGWATCHER_HOME).toBe("/Users/test/.blogwatcher");
    expect(plan.environment.GOPATH).toBeUndefined();
    expect(plan.environment.CARLITO_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:carlito|carlito)( --profile isolated)? gateway install/,
    );
  });
});
