...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "carlito-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): CarlitoConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<CarlitoConfig["acp"]>>,
) {
...
describe("agentCommand ACP runtime routing", () => {
  ...
  it("blocks ACP turns when disabled by policy", async () => {
    ...
    ] satisfies Array<Partial<NonNullable<CarlitoConfig["acp"]>>>) {
