...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.CARLITO_GATEWAY_TOKEN;
    prevPassword = process.env.CARLITO_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.CARLITO_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".carlito"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".carlito", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as CarlitoConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.CARLITO_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as CarlitoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "CARLITO_GATEWAY_TOKEN" },
        },
      },
    } as CarlitoConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as CarlitoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as CarlitoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as CarlitoConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("carlito approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as CarlitoConfig);
    const message = lastMessage();
    expect(message).toContain("[secrets]");
    expect(message).toContain("failed to resolve account");
    expect(message).toContain("Run: carlito security audit --deep");
  });
