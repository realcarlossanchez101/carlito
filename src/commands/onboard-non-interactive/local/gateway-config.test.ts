...
const SAMPLE_SECRET_REF = {
  source: "env" as const,
  provider: "default",
  id: "CARLITO_GATEWAY_TOKEN_REF",
};

function createTokenConfig(token: unknown): CarlitoConfig {
  return {
...
describe("applyNonInteractiveGatewayConfig token resolution chain", () => {
  const originalEnvToken = process.env.CARLITO_GATEWAY_TOKEN;
  ...
  afterEach(() => {
    if (originalEnvToken === undefined) {
      ...
      process.env.CARLITO_GATEWAY_TOKEN = originalEnvToken;
    }
  ...
  it("prefers existing plaintext token over ambient CARLITO_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd CARLITO_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    process.env.CARLITO_GATEWAY_TOKEN = "stale-env-token";
    const nextConfig = createTokenConfig("existing-user-token");
  ...
  it("uses CARLITO_GATEWAY_TOKEN to fill an empty config on first-run", () => {
  ...
  it("preserves an existing SecretRef even when ambient CARLITO_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "CARLITO_GATEWAY_TOKEN_NEW_REF";
