...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.CARLITO_GATEWAY_TOKEN);
  ...
  // plaintext > ambient CARLITO_GATEWAY_TOKEN > randomToken(). Ambient env
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env (use env var name like CARLITO_GATEWAY_TOKEN).",
        );
        runtime.exit(1);
        return null;
      }
      ...
      // branch, an ambient CARLITO_GATEWAY_TOKEN (or randomToken() fallback)
