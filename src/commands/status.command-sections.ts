...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("carlito security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("carlito security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    ...
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("carlito status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("carlito logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("carlito node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("carlito status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("carlito gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`carlito devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("carlito devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("carlito devices list")}`),
  ];
}
