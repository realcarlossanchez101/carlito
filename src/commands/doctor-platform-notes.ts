...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".carlito", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: CarlitoConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["CARLITO_GATEWAY_TOKEN", await getenv("CARLITO_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["CARLITO_GATEWAY_PASSWORD", await getenv("CARLITO_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "CARLITO_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.CARLITO_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- CARLITO_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
