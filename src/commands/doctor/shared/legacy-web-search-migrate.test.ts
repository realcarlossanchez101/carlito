...
describe("legacy web search config", () => {
  it("migrates legacy provider config through bundled web search ownership metadata", () => {
    const res = migrateLegacyWebSearchConfig<CarlitoConfig>({
      tools: {
        web: {
          search: {
            provider: "grok",
    ...
    expect(findLegacyConfigIssues(rawConfig)).toEqual([
      {
        ...
        message:
          'tools.web.search provider-owned config moved to plugins.entries.<plugin>.config.webSearch. Run "carlito doctor --fix".',
      },
    ]);
