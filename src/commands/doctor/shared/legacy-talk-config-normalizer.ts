...
export function normalizeLegacyTalkConfig(cfg: CarlitoConfig, changes: string[]): CarlitoConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as CarlitoConfig["talk"]) ?? {};
