...
export function resolveConfigDir(
  ...
  const override = env.CARLITO_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, homedir);
  }
  const configPath = env.CARLITO_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".carlito");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CARLITO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$CARLITO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CARLITO_STATE_DIR.
