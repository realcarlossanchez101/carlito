...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      CARLITO_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when CARLITO_THEME=light", async () => {
    const mod = await importThemeWithEnv({ CARLITO_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when CARLITO_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ CARLITO_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats CARLITO_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ CARLITO_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("CARLITO_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      CARLITO_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ CARLITO_THEME: "light" });
    const darkMod = await importThemeWithEnv({ CARLITO_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.CARLITO_THEME = "light";
