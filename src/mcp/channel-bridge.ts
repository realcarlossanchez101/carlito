...
export class CarlitoChannelBridge {
  ...
  constructor(
    private readonly cfg: CarlitoConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`carlito mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `carlito mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
