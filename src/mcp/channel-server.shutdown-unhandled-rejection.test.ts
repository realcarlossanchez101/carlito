...
vi.mock("./channel-bridge.js", () => ({
  CarlitoChannelBridge: class MockCarlitoChannelBridge {
...
describe("serveCarlitoChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveCarlitoChannelMcp } = await import("./channel-server.js");

    const servePromise = serveCarlitoChannelMcp({ verbose: false });
