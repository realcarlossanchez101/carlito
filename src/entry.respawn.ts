...
export const CARLITO_NODE_OPTIONS_READY = "CARLITO_NODE_OPTIONS_READY";
export const CARLITO_NODE_EXTRA_CA_CERTS_READY = "CARLITO_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.CARLITO_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[CARLITO_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[CARLITO_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[CARLITO_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[CARLITO_NODE_OPTIONS_READY] = "1";
