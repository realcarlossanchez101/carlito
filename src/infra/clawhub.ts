...
// Keep aligned with @realcarlossanchez101/plugin-package-contract ExternalPluginCompatibility.
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.CARLITO_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.CARLITO_CLAWHUB_TOKEN) ||
...
export async function downloadClawHubPackageArchive(params: {
  ...
  const target = await createTempDownloadTarget({
    prefix: "carlito-clawhub-package",
