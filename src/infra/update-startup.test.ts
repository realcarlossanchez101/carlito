...
vi.mock("./carlito-root.js", async () => {
  const actual = await vi.importActual<typeof import("./carlito-root.js")>("./carlito-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "carlito-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["CARLITO_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.CARLITO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveCarlitoPackageRoot } = await import("./carlito-root.js"));
    ...
    vi.mocked(resolveCarlitoPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveCarlitoPackageRoot).mockResolvedValue("/opt/carlito");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/carlito/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
