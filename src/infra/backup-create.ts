...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Carlito config file was found to back up."
        : "No local Carlito state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-backup-"));
