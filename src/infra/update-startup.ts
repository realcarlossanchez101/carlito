...
import { resolveCarlitoPackageRoot } from "./carlito-root.js";
...
function resolveAutoUpdatePolicy(cfg: CarlitoConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: CarlitoConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveCarlitoPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("carlito update")}`,
      );
