...
function candidateBinDirs(opts: EnsureCarlitoPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Carlito process is on.
  try {
  ...
  // Bundled macOS app: `carlito` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "carlito");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CARLITO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "carlito"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `carlito`
...
/**
 * Best-effort PATH bootstrap so skills that require the `carlito` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureCarlitoCliOnPath(opts: EnsureCarlitoPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CARLITO_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.CARLITO_PATH_BOOTSTRAPPED = "1";
