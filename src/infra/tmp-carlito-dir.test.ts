...
import { POSIX_CARLITO_TMP_DIR, resolvePreferredCarlitoTmpDir } from "./tmp-carlito-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredCarlitoTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `carlito-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredCarlitoTmpDir({
    ...
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_CARLITO_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredCarlitoTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredCarlitoTmpDir", () => {
  it("prefers /tmp/carlito when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_CARLITO_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/carlito when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_CARLITO_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it.each([
    {
      name: "falls back to os.tmpdir()/carlito when /tmp/carlito is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/carlito when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/carlito exists but is not writable",
    ...
    {
      name: "falls back when /tmp/carlito is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/carlito is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/carlito is group/other writable",
  ...
  it("repairs existing /tmp/carlito permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_CARLITO_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_CARLITO_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_CARLITO_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/carlito after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_CARLITO_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_CARLITO_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_CARLITO_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_CARLITO_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Carlito temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "carlito");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredCarlitoTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_CARLITO_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Carlito temp dir/);
  });
});
