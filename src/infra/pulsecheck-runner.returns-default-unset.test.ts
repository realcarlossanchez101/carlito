...
describe("resolvePulsecheckIntervalMs", () => {
  it("returns default when unset", () => {
    expect(resolvePulsecheckIntervalMs({})).toBe(30 * 60_000);
  });

  it("returns null when invalid or zero", () => {
    expect(
      resolvePulsecheckIntervalMs({
        agents: { defaults: { pulsecheck: { every: "0m" } } },
      }),
    ).toBeNull();
    expect(
      resolvePulsecheckIntervalMs({
        agents: { defaults: { pulsecheck: { every: "oops" } } },
      }),
    ).toBeNull();
  });
  ...
  it("parses duration strings with minute defaults", () => {
    expect(
      resolvePulsecheckIntervalMs({
        agents: { defaults: { pulsecheck: { every: "5m" } } },
      }),
    ).toBe(5 * 60_000);
    expect(
      resolvePulsecheckIntervalMs({
        agents: { defaults: { pulsecheck: { every: "5" } } },
      }),
    ).toBe(5 * 60_000);
    expect(
      resolvePulsecheckIntervalMs({
        agents: { defaults: { pulsecheck: { every: "2h" } } },
      }),
    ).toBe(2 * 60 * 60_000);
  });

  it("uses explicit pulsecheck overrides when provided", () => {
    expect(
      resolvePulsecheckIntervalMs(
...
describe("resolvePulsecheckPrompt", () => {
  it.each([
    ...
    expect(resolvePulsecheckPrompt(cfg)).toBe(expected);
  });
});
...
describe("isPulsecheckEnabledForAgent", () => {
  it("enables only explicit pulsecheck agents when configured", () => {
    ...
    expect(isPulsecheckEnabledForAgent(cfg, "main")).toBe(false);
    expect(isPulsecheckEnabledForAgent(cfg, "ops")).toBe(true);
  });
  ...
  it("falls back to default agent when no explicit pulsecheck entries", () => {
    ...
    expect(isPulsecheckEnabledForAgent(cfg, "main")).toBe(true);
    expect(isPulsecheckEnabledForAgent(cfg, "ops")).toBe(false);
  });
});

describe("resolvePulsecheckDeliveryTarget", () => {
  const baseEntry = {
    sessionId: "sid",
    updatedAt: Date.now(),
  };
  ...
  it("resolves target variants across route and allowlist rules", () => {
    const cases: Array<{
      ...
      expected: ReturnType<typeof resolvePulsecheckDeliveryTarget>;
    }> = [
      {
        name: "target none",
        cfg: { agents: { defaults: { pulsecheck: { target: "none" } } } },
        entry: baseEntry,
        expected: {
          channel: "none",
          reason: "target-none",
      ...
      },
      {
        ...
        cfg: {
          agents: {
            defaults: {
              pulsecheck: { target: "whatsapp", to: "whatsapp:120363401234567890@G.US" },
            },
          },
      ...
      },
      {
        ...
        cfg: {
          agents: { defaults: { pulsecheck: { target: "whatsapp", to: "+1999" } } },
      ...
      },
      {
        ...
        cfg: {
          agents: { defaults: { pulsecheck: { target: "last" } } },
      ...
      },
      {
        ...
        cfg: { agents: { defaults: { pulsecheck: { target: "telegram", to: "-100123" } } } },
        entry: baseEntry,
        expected: {
          channel: "telegram",
          to: "-100123",
      ...
      },
      {
        ...
        cfg: { agents: { defaults: { pulsecheck: { target: "last" } } } },
      ...
      },
      {
        ...
        cfg: { agents: { defaults: { pulsecheck: { target: "last", directPolicy: "block" } } } },
        entry: { ...baseEntry, lastChannel: "telegram", lastTo: "5232990709" },
        expected: {
          channel: "none",
          reason: "dm-blocked",
    ...
    for (const { cfg, entry, name, expected } of cases) {
      expect(resolvePulsecheckDeliveryTarget({ cfg, entry }), name).toEqual(expected);
    }
  });
  ...
  it.each([
    ...
    ({ to, expectedTo, expectedThreadId }) => {
      ...
      const result = resolvePulsecheckDeliveryTarget({ cfg, entry: baseEntry });
  ...
  it.each([
    ...
    ({ accountId, expected }) => {
      ...
      expect(resolvePulsecheckDeliveryTarget({ cfg, entry: baseEntry })).toEqual(expected);
    },
  );
...
describe("resolvePulsecheckSenderContext", () => {
  it("prefers delivery accountId for allowFrom resolution", () => {
    ...
    const ctx = resolvePulsecheckSenderContext({ cfg, entry, delivery });
...
describe("runPulsecheckOnce", () => {
  const createPulsecheckDeps = (
    ...
    options?: {
      nowMs?: number;
      getReplyFromConfig?: PulsecheckDeps["getReplyFromConfig"];
    },
  ): PulsecheckDeps => ({
  ...
  it("skips when agent pulsecheck is not enabled", async () => {
    ...
    const res = await runPulsecheckOnce({ cfg, agentId: "main" });
    ...
    try {
      const cfg: OpenClawConfig = {
        agents: {
          defaults: {
            workspace: tmpDir,
            pulsecheck: { every: "5m", target: "whatsapp" },
          },
        },
        channels: { whatsapp: { allowFrom: ["*"] } },
      ...
      await runPulsecheckOnce({
        cfg,
        deps: createPulsecheckDeps(sendWhatsApp, { getReplyFromConfig: replySpy }),
      });
  ...
  it("uses per-agent pulsecheck overrides and session keys", async () => {
    ...
    try {
        ...
        agents: {
          ...
          list: [
            { id: "main", default: true },
            {
              id: "ops",
              workspace: tmpDir,
              pulsecheck: { every: "5m", target: "whatsapp", prompt: "Ops check" },
            },
          ],
        },
        channels: { whatsapp: { allowFrom: ["*"] } },
      ...
      expect(replySpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ isPulsecheck: true, suppressToolErrorWarnings: false }),
        cfg,
      );
    } finally {
      replySpy.mockReset();
  ...
  it.each([
    ...
    {
      name: "runPulsecheckOnce sessionKey arg",
  ...
  it.each([
    ...
    {
      name: "subagent key via pulsecheck.session config",
    ...
    try {
      ...
      if (injectVia === "config" && cfg.agents?.defaults?.pulsecheck) {
        cfg.agents.defaults.pulsecheck.session = subagentKey;
      }
      ...
      replySpy.mockClear();
      replySpy.mockResolvedValue([{ text: "Main session pulsecheck" }]);
      const sendWhatsApp = vi
        .fn<
          (
            to: string,
      ...
      // The pulsecheck must use the main session, not the subagent session.
      expect(replySpy).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it("suppresses duplicate pulsecheck payloads within 24h", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          [sessionKey]: {
            ...
            lastChannel: "whatsapp",
            lastTo: "120363401234567890@g.us",
            lastPulsecheckText: "Final alert",
            lastPulsecheckSentAt: 0,
          },
        }),
      );
      ...
      await runPulsecheckOnce({
        cfg,
        deps: createPulsecheckDeps(sendWhatsApp, {
          nowMs: 60_000,
  ...
  it.each(
    typedCases<{
      ...
      {
        name: "reasoning + PULSECHECK_OK",
        caseDir: "hb-reasoning-pulsecheck-ok",
        replies: [{ text: "Reasoning:\n_Because it helps_" }, { text: "PULSECHECK_OK" }],
        expectedTexts: ["Reasoning:\n_Because it helps_"],
      },
    ]),
  )(
  ...
  async function runPulsecheckFileScenario(params: {
    fileState: PulsecheckFileState;
    ...
    if (params.fileState === "empty") {
      await fs.writeFile(
        path.join(workspaceDir, "PULSECHECK.md"),
        "# PULSECHECK.md\n\n## Tasks\n\n",
        "utf-8",
      );
...
`,
  ...
  it("adds explicit workspace PULSECHECK.md path guidance to pulsecheck prompts", async () => {
    const { res, replySpy, sendWhatsApp, workspaceDir } = await runPulsecheckFileScenario({
    ...
    try {
      ...
      const expectedPath = path.join(workspaceDir, "PULSECHECK.md").replace(/\\/g, "/");
      expect(calledCtx.Body).toContain(`use workspace file ${expectedPath} (exact case)`);
      expect(calledCtx.Body).toContain("Do not read docs/pulsecheck.md.");
    } finally {
      replySpy.mockRestore();
    }
  });

  it("applies PULSECHECK.md gating rules across file states and triggers", async () => {
    const cases: Array<{
      name: string;
      fileState: PulsecheckFileState;
      reason?: "interval" | "wake";
      ...
      expectedSkipReason?: "empty-pulsecheck-file";
    ...
    for (const {
      ...
      const { res, replySpy, sendWhatsApp } = await runPulsecheckFileScenario(scenario);
      try {
  ...
  it("uses an internal-only cron prompt when pulsecheck delivery target is none", async () => {
    ...
    const cfg: OpenClawConfig = {
      agents: {
        defaults: {
          workspace: tmpDir,
          pulsecheck: { every: "5m", target: "none" },
        },
      },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: storePath },
