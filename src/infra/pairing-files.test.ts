...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/carlito-state", "devices")).toEqual({
      dir: path.join("/tmp/carlito-state", "devices"),
      pendingPath: path.join("/tmp/carlito-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/carlito-state", "devices", "paired.json"),
    });
  });
