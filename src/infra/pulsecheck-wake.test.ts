...
describe("pulsecheck-wake", () => {
  ...
  async function expectRetryAfterDefaultDelay(params: {
    ...
    setPulsecheckWakeHandler(
      params.handler as unknown as Parameters<typeof setPulsecheckWakeHandler>[0],
    );
    requestPulsecheckNow({ reason: params.initialReason, coalesceMs: 0 });
  ...
  it("coalesces multiple wake requests into one run", async () => {
    ...
    requestPulsecheckNow({ reason: "interval", coalesceMs: 200 });
    requestPulsecheckNow({ reason: "exec-event", coalesceMs: 200 });
    requestPulsecheckNow({ reason: "retry", coalesceMs: 200 });

    expect(hasPendingPulsecheckWake()).toBe(true);
    ...
    expect(hasPendingPulsecheckWake()).toBe(false);
  });
  ...
  it("keeps retry cooldown even when a sooner request arrives", async () => {
    vi.useFakeTimers();
    const handler = setRetryOncePulsecheckHandler();
    ...
    requestPulsecheckNow({ reason: "hook:wake", coalesceMs: 0 });
  ...
  it("stale disposer does not clear a newer handler", async () => {
    ...
    const disposeA = setPulsecheckWakeHandler(handlerA);

    // Runner B registers its handler (replaces A)
    const disposeB = setPulsecheckWakeHandler(handlerB);

    // Runner A's stale cleanup runs — should NOT clear handlerB
    disposeA();
    expect(hasPulsecheckWakeHandler()).toBe(true);
    ...
    disposeB();
    expect(hasPulsecheckWakeHandler()).toBe(false);
  });
  ...
  it("preempts existing timer when a sooner schedule is requested", async () => {
    ...
    requestPulsecheckNow({ reason: "slow", coalesceMs: 5000 });

    // Schedule for 100ms from now — should preempt the 5s timer
    requestPulsecheckNow({ reason: "fast", coalesceMs: 100 });
  ...
  it("does not downgrade a higher-priority pending reason", async () => {
    ...
    requestPulsecheckNow({ reason: "exec-event", coalesceMs: 100 });
    requestPulsecheckNow({ reason: "retry", coalesceMs: 100 });
  ...
  it("resets running/scheduled flags when new handler is registered", async () => {
    ...
    setPulsecheckWakeHandler(handlerA);
    ...
    const handlerB = vi.fn().mockResolvedValue({ status: "ran", durationMs: 1 });
    setPulsecheckWakeHandler(handlerB);
  ...
  it("clears stale retry cooldown when a new handler is registered", async () => {
    ...
    requestPulsecheckNow({ reason: "manual", coalesceMs: 0 });
    await vi.advanceTimersByTimeAsync(1);
    expect(handlerB).toHaveBeenCalledTimes(1);
  ...
  it("preserves pulsecheck override when same-target wakes coalesce", async () => {
    vi.useFakeTimers();
    const handler = vi.fn().mockResolvedValue({ status: "ran", durationMs: 1 });
    setPulsecheckWakeHandler(handler);
