...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\Carlito\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/carlito/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/carlito/carlito.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/carlito/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/carlito/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the carlito executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\carlito.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/carlito-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/carlito-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\carlito-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/carlito/custom.js", "gateway"])).toBe(false);
