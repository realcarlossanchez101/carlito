...
describe("resolveLocalVitestMaxWorkers", () => {
  ...
  it("lets CARLITO_VITEST_MAX_WORKERS override the inferred cap", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          CARLITO_VITEST_MAX_WORKERS: "2",
        },
        {
          cpuCount: 10,
          loadAverage1m: 0,
  ...
  it("respects the legacy CARLITO_TEST_WORKERS override too", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          CARLITO_TEST_WORKERS: "3",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0,
...
describe("resolveLocalVitestScheduling", () => {
  ...
  it("allows disabling the system throttle probe explicitly", () => {
    expect(
      resolveLocalVitestScheduling(
        {
          CARLITO_VITEST_DISABLE_SYSTEM_THROTTLE: "1",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0.5,
...
describe("test scripts", () => {
  it("keeps test scripts on the native thread-first configs", () => {
    ...
    expect(pkg.scripts?.["test:serial"]).toBe(
      "CARLITO_TEST_PROJECTS_SERIAL=1 CARLITO_VITEST_MAX_WORKERS=1 node scripts/test-projects.mjs",
    );
