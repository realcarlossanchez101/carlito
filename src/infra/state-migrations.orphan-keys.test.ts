...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".carlito");
...
function sharedMainOpsConfig(sharedStorePath: string): CarlitoConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: CarlitoConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { CARLITO_STATE_DIR: stateDir };
