...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`carlito-${label}-`);
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the carlito package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "carlito");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "carlito", version: "2026.3.10" }),
      "utf-8",
    );
