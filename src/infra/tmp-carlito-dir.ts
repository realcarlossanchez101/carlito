...
export const POSIX_CARLITO_TMP_DIR = "/tmp/carlito";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredCarlitoTmpDirOptions = {
...
export function resolvePreferredCarlitoTmpDir(
  options: ResolvePreferredCarlitoTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "carlito" : `carlito-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[carlito] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Carlito temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Carlito temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_CARLITO_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_CARLITO_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_CARLITO_TMP_DIR)) {
      return POSIX_CARLITO_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_CARLITO_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_CARLITO_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_CARLITO_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_CARLITO_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_CARLITO_TMP_DIR;
