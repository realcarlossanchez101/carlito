...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers CARLITO_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for CARLITO_HOME",
      env: { CARLITO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when CARLITO_HOME is tilde-only and no fallback home exists",
      env: { CARLITO_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores CARLITO_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          CARLITO_HOME: "/srv/carlito-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { CARLITO_HOME: "/srv/carlito-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/carlito-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/carlito-home " },
      expected: "/srv/carlito-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/carlito-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of CARLITO_HOME", () => {
    expect(
