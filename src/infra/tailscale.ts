...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.CARLITO_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Carlito. You can keep running the web gateway without it: \`${formatCliCommand("carlito gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
