...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__carlito_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "carlito-root");
...
function setPackageRoot(root: string, name = "carlito") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./carlito-root.js").resolveCarlitoPackageRootSync,
  asyncResolver: typeof import("./carlito-root.js").resolveCarlitoPackageRoot,
  opts: Parameters<typeof import("./carlito-root.js").resolveCarlitoPackageRootSync>[0],
...
vi.mock("./carlito-root.fs.runtime.js", () => ({
  carlitoRootFsSync: mockFsModule(),
  carlitoRootFs: mockFsPromisesModule(),
}));

describe("resolveCarlitoPackageRoot", () => {
  let resolveCarlitoPackageRoot: typeof import("./carlito-root.js").resolveCarlitoPackageRoot;
  let resolveCarlitoPackageRootSync: typeof import("./carlito-root.js").resolveCarlitoPackageRootSync;

  beforeAll(async () => {
    ({ resolveCarlitoPackageRoot, resolveCarlitoPackageRootSync } =
      await import("./carlito-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "carlito");
        const pkgRoot = path.join(project, "node_modules", "carlito");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "carlito");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "carlito.mjs")));
    ...
    {
      name: "falls through from a non-carlito moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-carlito");
    ...
    {
      name: "returns null for non-carlito package roots",
      setup: () => {
        const pkgRoot = fx("not-carlito");
        setPackageRoot(pkgRoot, "not-carlito");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "carlito.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "carlito");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
