...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", () => {
    ...
    withEnv(
      {
        ...
        CARLITO_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "CARLITO_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith("env: CARLITO_TEST_ENV=<redacted> (test option)");
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        CARLITO_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "CARLITO_BLANK_ENV",
