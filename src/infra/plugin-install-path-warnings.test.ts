...
const MATRIX_REPO_INSTALL_COMMAND = `carlito plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/carlito-matrix-missing",
        installPath: "/tmp/carlito-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/carlito-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/carlito-matrix-missing",
      'Reinstall with "carlito plugins install @realcarlossanchez101/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/carlito-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<carlito plugins install @realcarlossanchez101/matrix>" when you want to return to the standard Matrix plugin.',
