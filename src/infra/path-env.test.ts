...
import { ensureCarlitoCliOnPath } from "./path-env.js";
...
describe("ensureCarlitoCliOnPath", () => {
  const envKeys = [
    "PATH",
    "CARLITO_PATH_BOOTSTRAPPED",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/carlito-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "carlito");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureCarlitoCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.CARLITO_PATH_BOOTSTRAPPED;
    delete process.env.CARLITO_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling carlito exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/carlito-path/case-runtime-dir");
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "carlito");
      ...
      if (envValue === undefined) {
        ...
        process.env.CARLITO_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/carlito-path/case-linuxbrew");
