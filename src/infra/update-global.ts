...
const PRIMARY_PACKAGE_NAME = "carlito";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const CARLITO_MAIN_PACKAGE_SPEC = "github:carlito/carlito#main";
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "carlito.plugin.json")))
      ) {
        expectedFiles.add(relativePath);
      }
    }),
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Carlito", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.CARLITO_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.CARLITO_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return CARLITO_MAIN_PACKAGE_SPEC;
  }
