...
vi.mock("./install-source-utils.js", async () => {
  ...
  return {
    ...
    withTempDir: vi.fn(async (_prefix: string, fn: (tmpDir: string) => Promise<unknown>) => {
      return await fn("/tmp/carlito-npm-pack-install-test");
    }),
...
describe("installFromNpmSpecArchive", () => {
  const baseSpec = "@realcarlossanchez101/test@1.0.0";
  const baseArchivePath = "/tmp/carlito-test.tgz";
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "carlito-test-",
      spec: "@realcarlossanchez101/test@1.0.0",
      timeoutMs: 1000,
      installFromArchive,
    });
    ...
    expect(withTempDir).toHaveBeenCalledWith("carlito-test-", expect.any(Function));
  });
  ...
  it("returns resolution metadata and installer result on success", async () => {
    mockPackedSuccess({ name: "@realcarlossanchez101/test", version: "1.0.0" });
    ...
    expect(okResult.npmResolution.resolvedSpec).toBe("@realcarlossanchez101/test@1.0.0");
    expect(okResult.npmResolution.resolvedAt).toBeTruthy();
    expect(installFromArchive).toHaveBeenCalledWith({ archivePath: "/tmp/carlito-test.tgz" });
  });
  ...
  it("aborts when integrity drift callback rejects drift", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "aborted: npm package integrity drift detected for @realcarlossanchez101/test@1.0.0",
    });
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("warns and aborts on drift when no callback is configured", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @realcarlossanchez101/test@1.0.0: expected sha512-old, got sha512-new",
    );
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("rejects prerelease resolutions unless explicitly requested", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ...
      archivePath: baseArchivePath,
      metadata: {
        resolvedSpec: "@realcarlossanchez101/test@latest",
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "carlito-test-",
      spec: "@realcarlossanchez101/test@latest",
      timeoutMs: 1000,
      installFromArchive,
    });
  ...
  it("allows prerelease resolutions when explicitly requested by tag", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ...
      archivePath: baseArchivePath,
      metadata: {
        resolvedSpec: "@realcarlossanchez101/test@beta",
        integrity: "sha512-same",
        version: "1.1.0-beta.1",
      },
    });
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "carlito-test-",
      spec: "@realcarlossanchez101/test@beta",
      timeoutMs: 1000,
      installFromArchive,
    });
...
describe("installFromNpmSpecArchiveWithInstaller", () => {
  ...
  it("passes archive path and installer params to installFromArchive", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/carlito-plugin.tgz",
      metadata: {
        resolvedSpec: "@realcarlossanchez101/voice-call@1.0.0",
        integrity: "sha512-same",
      },
    });
    ...
    const result = await installFromNpmSpecArchiveWithInstaller({
      tempDirPrefix: "carlito-test-",
      spec: "@realcarlossanchez101/voice-call@1.0.0",
      timeoutMs: 1000,
      installFromArchive,
...
describe("finalizeNpmSpecArchiveInstall", () => {
  ...
  it("returns install errors unchanged", () => {
    const result = finalizeNpmSpecArchiveInstall<{ ok: true } | { ok: false; error: string }>({
      ...
      npmResolution: {
        resolvedSpec: "@realcarlossanchez101/test@1.0.0",
        integrity: "sha512-same",
