...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "carlito-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".carlito");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".carlito", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      CARLITO_STATE_DIR: path.join(home, ".carlito"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "carlito.json"),
