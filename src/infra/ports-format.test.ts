...
describe("ports-format", () => {
  it.each([
    ...
    [{ command: "ssh" }, "ssh"],
    [{ commandLine: "node /Users/me/Projects/carlito/dist/entry.js gateway" }, "gateway"],
  ...
  it("builds ordered hints for mixed listener kinds and multiplicity", () => {
    expect(
      buildPortHints(
        [
          { commandLine: "node dist/index.js carlito gateway" },
  ...
  it("treats single-process loopback dual-stack gateway listeners as benign", () => {
    const listeners = [
      { pid: 4242, commandLine: "carlito-gateway", address: "127.0.0.1:18789" },
      { pid: 4242, commandLine: "carlito-gateway", address: "[::1]:18789" },
    ];
