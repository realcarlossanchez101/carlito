...
import { carlitoRootFs, carlitoRootFsSync } from "./carlito-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["carlito"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  try {
    return parsePackageName(await carlitoRootFs.readFile(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function readPackageNameSync(dir: string): string | null {
  try {
    return parsePackageName(
      carlitoRootFsSync.readFileSync(path.join(dir, "package.json"), "utf-8"),
    );
  } catch {
    return null;
  }
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = carlitoRootFsSync.realpathSync(normalized);
...
export async function resolveCarlitoPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveCarlitoPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
