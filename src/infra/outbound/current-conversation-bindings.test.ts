...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.CARLITO_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-current-bindings-"));
    process.env.CARLITO_STATE_DIR = testStateDir;
