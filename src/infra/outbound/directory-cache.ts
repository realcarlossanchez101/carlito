...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CarlitoConfig | null = null;
  ...
  get(key: string, cfg: CarlitoConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CarlitoConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: CarlitoConfig): void {
