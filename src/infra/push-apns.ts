...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Carlito to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "carlito.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.CARLITO_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.CARLITO_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set CARLITO_APNS_TEAM_ID and CARLITO_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.CARLITO_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.CARLITO_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.CARLITO_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set CARLITO_APNS_PRIVATE_KEY_P8 or CARLITO_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading CARLITO_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
