...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.CARLITO_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("carlito-gw");
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._carlito-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._carlito-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("carlito (Carlito)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("carlito");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("carlito.local");
