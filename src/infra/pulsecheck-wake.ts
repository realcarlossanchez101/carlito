...
export type PulsecheckWakeRequest = {
  reason?: string;
  agentId?: string;
  sessionKey?: string;
...
export type PulsecheckWakeHandler = (opts: PulsecheckWakeRequest) => Promise<PulsecheckRunResult>;

let pulsechecksEnabled = true;

export function setPulsechecksEnabled(enabled: boolean) {
  pulsechecksEnabled = enabled;
}

export function arePulsechecksEnabled(): boolean {
  return pulsechecksEnabled;
}
...
let handler: PulsecheckWakeHandler | null = null;
...
function resolveReasonPriority(reason: string): number {
  ...
  if (isPulsecheckActionWakeReason(reason)) {
...
function normalizeWakeReason(reason?: string): string {
  return normalizePulsecheckWakeReason(reason);
}
...
function queuePendingWakeReason(params?: {
  ...
  const merged =
    (next.pulsecheck ?? previous.pulsecheck)
      ? { ...next, pulsecheck: next.pulsecheck ?? previous.pulsecheck }
      : next;
    ...
    try {
      for (const pendingWake of pendingBatch) {
        const wakeOpts = {
          ...
          ...(pendingWake.pulsecheck ? { pulsecheck: pendingWake.pulsecheck } : {}),
        };
        ...
        if (res.status === "skipped" && res.reason === "requests-in-flight") {
          ...
          queuePendingWakeReason({
            ...
            pulsecheck: pendingWake.pulsecheck,
          });
          schedule(DEFAULT_RETRY_MS, "retry");
        }
      }
    } catch {
      // Error is already logged by the pulsecheck runner; schedule a retry.
      for (const pendingWake of pendingBatch) {
        queuePendingWakeReason({
          reason: pendingWake.reason ?? "retry",
          agentId: pendingWake.agentId,
...
/**
 * Register (or clear) the pulsecheck wake handler.
 ...
 */
export function setPulsecheckWakeHandler(next: PulsecheckWakeHandler | null): () => void {
  handlerGeneration += 1;
  ...
  if (next) {
    ...
    // an interrupted pulsecheck blocks all future schedule() attempts, and
...
export function requestPulsecheckNow(opts?: {
  reason?: string;
  coalesceMs?: number;
  agentId?: string;
  sessionKey?: string;
...
export function hasPulsecheckWakeHandler() {
  return handler !== null;
}

export function hasPendingPulsecheckWake() {
  return pendingWakes.size > 0 || Boolean(timer) || scheduled;
}

export function resetPulsecheckWakeStateForTests() {
  if (timer) {
    clearTimeout(timer);
  }
  timer = null;
