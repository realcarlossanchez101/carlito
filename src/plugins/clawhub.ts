...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "carlito skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Carlito runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Carlito >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
    ...
    return {
      ...
      clawhub: {
        ...
        clawhubUrl:
          ...
          normalizeOptionalString(process.env.CARLITO_CLAWHUB_URL) ||
          "https://clawhub.ai",
