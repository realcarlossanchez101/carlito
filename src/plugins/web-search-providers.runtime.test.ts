...
let loadCarlitoPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/carlito.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/carlito.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadCarlitoPluginsMock).toHaveBeenCalledTimes(count);
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadCarlitoPluginsMock = vi
      .spyOn(loaderModule, "loadCarlitoPlugins")
  ...
  it("retains the snapshot cache when config contents change in place", () => {
    ...
    const env = createWebSearchEnv({ CARLITO_HOME: "/tmp/carlito-home-a" });
  ...
  it("invalidates the snapshot cache when env contents change in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.CARLITO_HOME = "/tmp/carlito-home-b";
      },
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        CARLITO_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadCarlitoPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      CARLITO_PLUGIN_DISCOVERY_CACHE_MS: "5",
      CARLITO_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadCarlitoPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      CARLITO_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.CARLITO_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
