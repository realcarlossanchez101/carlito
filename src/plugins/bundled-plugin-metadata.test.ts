...
describe("bundled plugin metadata", () => {
  ...
  it("prefers built generated paths when present and falls back to source paths", () => {
    const tempRoot = createGeneratedPluginTempRoot("carlito-bundled-plugin-metadata-");
  ...
  it("resolves plugin-local generated entry paths when the plugin dir is provided", () => {
    const tempRoot = createGeneratedPluginTempRoot("carlito-bundled-plugin-metadata-local-");
  ...
  it("scans direct plugin-tree overrides and resolves generated paths from that scan dir", () => {
    const tempRoot = createGeneratedPluginTempRoot("carlito-bundled-plugin-direct-tree-");
    ...
    writeJson(path.join(pluginRoot, "carlito.plugin.json"), {
      id: "alpha",
      channels: ["alpha"],
  ...
  it("prefers direct scan-dir overrides over nested dist artifacts within the same override root", () => {
    const pluginsDir = createGeneratedPluginTempRoot("carlito-bundled-plugin-direct-priority-");
    const pluginRoot = path.join(pluginsDir, "alpha");
  ...
  it("resolves bundled repo entry paths from dist before workspace source", () => {
    const tempRoot = createGeneratedPluginTempRoot("carlito-bundled-plugin-repo-entry-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
    const distPluginRoot = path.join(tempRoot, "dist", "extensions", "alpha");
  ...
  it("merges runtime channel schema metadata with manifest-owned channel config fields", () => {
    const tempRoot = createGeneratedPluginTempRoot("carlito-bundled-plugin-channel-configs-");
    ...
    writeJson(path.join(tempRoot, "extensions", "alpha", "carlito.plugin.json"), {
      id: "alpha",
      channels: ["alpha"],
      configSchema: { type: "object" },
      channelConfigs: {
  ...
  it("captures top-level public surface artifacts without duplicating the primary entrypoints", () => {
    const tempRoot = createGeneratedPluginTempRoot("carlito-bundled-plugin-public-artifacts-");
  ...
  it("loads channel config metadata from built public surfaces in dist-only roots", () => {
    const tempRoot = createGeneratedPluginTempRoot("carlito-bundled-plugin-dist-config-");
    ...
    writeJson(path.join(distRoot, "extensions", "alpha", "carlito.plugin.json"), {
      id: "alpha",
      configSchema: {
        type: "object",
        properties: {},
