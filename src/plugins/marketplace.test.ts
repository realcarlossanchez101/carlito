...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("carlito-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("carlito-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("carlito-marketplace-test-", async (homeDir) => {
      const carlitoHome = path.join(homeDir, "carlito-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, CARLITO_HOME: carlitoHome },
