...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-carlito",
    targetDir: params?.targetDir ?? "/tmp/opik-carlito",
...
function createInstalledPackageDir(params: { name?: string; version: string }): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@carlito/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["carlito-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/carlito-codex-app-server",
    version: params.version,
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-carlito",
        spec: "@opik/opik-carlito",
        integrity: "sha512-old",
        installPath: "/tmp/opik-carlito",
      }),
      pluginIds: ["opik-carlito"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-carlito",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        ...
        spec: "carlito-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/carlito-codex-app-server",
      }),
      ...
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "carlito-codex-app-server",
        targetDir: "/tmp/carlito-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "carlito-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-carlito@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-carlito@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-carlito" (@opik/opik-carlito@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-carlito: aborted: npm package integrity drift detected for @opik/opik-carlito@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @realcarlossanchez101/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@realcarlossanchez101/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage:
        "Failed to check missing: npm package not found for @realcarlossanchez101/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "carlito-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "carlito-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "carlito-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "carlito-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      expectedSpec: "carlito-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "carlito-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "carlito-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      targetDir: "/tmp/carlito-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@realcarlossanchez101/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@realcarlossanchez101/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@realcarlossanchez101/voice-call");
    expect(result.config.plugins?.entries?.["@realcarlossanchez101/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@realcarlossanchez101/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@realcarlossanchez101/voice-call",
      installPath: "/tmp/carlito-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it.each([
    {
      ...
      config: createBundledPathInstallConfig({
        ...
        spec: "@realcarlossanchez101/feishu",
      }),
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/carlito-home";
    mockBundledSources(
      createBundledSource({
