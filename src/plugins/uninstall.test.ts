...
type PluginConfig = NonNullable<CarlitoConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: CarlitoConfig["channels"];
}): CarlitoConfig {
...
function createSinglePluginWithEmptySlotsConfig(): CarlitoConfig {
...
function createSingleNpmInstallConfig(installPath: string): CarlitoConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "carlito-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-carlito-plugin-install/my-plugin",
      },
      extensionsDir,
    });
