...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CARLITO_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.CARLITO_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @realcarlossanchez101/voice-call -> voice-call).
...
export function discoverCarlitoPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
  ...
  const scopedResult = getCachedDiscoveryResult({
    ...
    load: () => {
      ...
      if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
        // Keep workspace auto-discovery constrained to the Carlito extensions root.
