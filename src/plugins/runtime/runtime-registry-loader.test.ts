...
const mocks = vi.hoisted(() => ({
  loadCarlitoPlugins: vi.fn<typeof import("../loader.js").loadCarlitoPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadCarlitoPlugins: (...args: Parameters<typeof mocks.loadCarlitoPlugins>) =>
    mocks.loadCarlitoPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadCarlitoPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadCarlitoPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadCarlitoPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
});
