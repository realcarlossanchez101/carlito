    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/firecrawl/carlito.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
describe("resolvePluginWebFetchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadCarlitoPluginsMock = vi.spyOn(loaderModule, "loadCarlitoPlugins").mockImplementation(() => {
  ...
  it("does not force a fresh snapshot load when the same web-provider load is already in flight", () => {
    ...
    loadCarlitoPluginsMock.mockImplementation(() => {
