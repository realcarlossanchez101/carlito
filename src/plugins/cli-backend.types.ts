...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Carlito consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Carlito should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Carlito's bundle
   ...
   *
   * Use this when the backend should consume a canonical Carlito auth profile
   ...
   * `profile-only` treats the selected Carlito auth profile as the sole auth
