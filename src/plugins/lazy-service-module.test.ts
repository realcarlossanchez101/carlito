...
describe("startLazyPluginServiceModule", () => {
  afterEach(() => {
    delete process.env.CARLITO_LAZY_SERVICE_SKIP;
    delete process.env.CARLITO_LAZY_SERVICE_OVERRIDE;
  });
  ...
  it("uses the override module when configured", async () => {
    process.env.CARLITO_LAZY_SERVICE_OVERRIDE = "virtual:service";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));

    await expectLifecycleStarted({
      overrideEnvVar: "CARLITO_LAZY_SERVICE_OVERRIDE",
  ...
  it("surfaces override validation failures", async () => {
    process.env.CARLITO_LAZY_SERVICE_OVERRIDE = "data:text/javascript,boom";
