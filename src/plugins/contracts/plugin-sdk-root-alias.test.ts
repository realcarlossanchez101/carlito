...
describe("plugin-sdk root alias", () => {
  ...
  it("builds scoped and unscoped plugin-sdk aliases for jiti loads", () => {
    ...
    expect(lazyModule.createJitiOptions.at(-1)?.alias).toMatchObject({
      "carlito/plugin-sdk": rootAliasPath,
      "@realcarlossanchez101/plugin-sdk": rootAliasPath,
      "carlito/plugin-sdk/group-access": expect.stringContaining(
        path.join("src", "plugin-sdk", "group-access.ts"),
      ),
      "@realcarlossanchez101/plugin-sdk/group-access": expect.stringContaining(
        path.join("src", "plugin-sdk", "group-access.ts"),
      ),
    });
  });
  ...
  it("keeps bootstrap plugin-sdk aliases deterministic and ignores unsafe subpaths", () => {
    ...
    expect(aliasKeys).toEqual([
      "carlito/plugin-sdk/alpha",
      "@realcarlossanchez101/plugin-sdk/alpha",
      "carlito/plugin-sdk/group-access",
      "@realcarlossanchez101/plugin-sdk/group-access",
      "carlito/plugin-sdk/zeta",
      "@realcarlossanchez101/plugin-sdk/zeta",
      "carlito/plugin-sdk",
  ...
  it("ignores unsafe private local-only plugin-sdk subpaths in the CJS root alias", () => {
    ...
    const lazyModule = loadRootAliasWithStubs({
      env: { CARLITO_ENABLE_PRIVATE_QA_CLI: "1" },
    ...
    expect(aliasMap["carlito/plugin-sdk/qa-lab"]).toBe(
      path.join(packageRoot, "src", "plugin-sdk", "qa-lab.ts"),
    );
    expect(aliasMap["@realcarlossanchez101/plugin-sdk/qa-lab"]).toBe(
      path.join(packageRoot, "src", "plugin-sdk", "qa-lab.ts"),
    );
    expect(aliasMap).not.toHaveProperty("carlito/plugin-sdk/../escape");
    expect(aliasMap).not.toHaveProperty("carlito/plugin-sdk/nested/path");
  });
  ...
  it("builds source plugin-sdk subpath aliases through the wider source extension family", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    expect(lazyModule.createJitiOptions.at(-1)?.alias).toMatchObject({
      "carlito/plugin-sdk/channel-runtime": path.join(
        packageRoot,
        "src",
        "plugin-sdk",
        "channel-runtime.mts",
      ),
      "@realcarlossanchez101/plugin-sdk/channel-runtime": path.join(
        packageRoot,
        "src",
        "plugin-sdk",
        "channel-runtime.mts",
