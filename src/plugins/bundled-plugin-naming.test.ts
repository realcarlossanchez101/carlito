...
function readBundledPluginRecords(): BundledPluginRecord[] {
  return fs
    ...
    .flatMap((dirName) => {
      ...
      const manifestPath = path.join(rootDir, "carlito.plugin.json");
      if (!fs.existsSync(packagePath) || !fs.existsSync(manifestPath)) {
        return [];
      }

      const manifest = readJsonFile<PluginManifestShape>(manifestPath);
      const pkg = readJsonFile<CarlitoPackageShape>(packagePath);
      ...
      return [
        {
          ...
          packageName,
          manifestId,
          installNpmSpec: normalizeText(pkg.carlito?.install?.npmSpec),
          channelId: normalizeText(pkg.carlito?.channel?.id),
        },
      ];
    });
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@carlito/${pluginId}${suffix}`);
}
  ...
  it.each([
    {
      ...
      message: `Bundled extension package names must stay anchored to the manifest id via @carlito/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.`,
    ...
    },
    {
      ...
      message:
        "Bundled extension directory names should match carlito.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    {
      name: "keeps bundled carlito.install.npmSpec aligned with the package name",
      message:
        "Bundled carlito.install.npmSpec values must match the package name so install/update paths stay deterministic.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Bundled carlito.channel.id values must match carlito.plugin.json:id for the owning plugin.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
