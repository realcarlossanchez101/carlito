...
const discoverCarlitoPluginsMock = vi.fn();
const loadPluginManifestMock = vi.fn();

vi.mock("./discovery.js", () => ({
  discoverCarlitoPlugins: (...args: unknown[]) => discoverCarlitoPluginsMock(...args),
}));
...
function setBundledDiscoveryCandidates(candidates: unknown[]) {
  discoverCarlitoPluginsMock.mockReturnValue({
    candidates,
    diagnostics: [],
  });
}
  ...
  loadPluginManifestMock.mockImplementation((rootDir: string) =>
    rootDir in manifestIds
      ...
      : {
          ...
          manifestPath: `${rootDir}/carlito.plugin.json`,
        },
  );
}
...
function createResolvedBundledSource(params: {
  ...
  return {
    ...
    npmSpec: params.npmSpec ?? `@carlito/${params.pluginId}`,
  };
}
...
describe("bundled plugin sources", () => {
  beforeEach(() => {
    discoverCarlitoPluginsMock.mockReset();
  ...
  it("resolves bundled sources keyed by plugin id", () => {
    setBundledDiscoveryCandidates([
      ...
      createBundledCandidate({
        ...
        packageName: "@realcarlossanchez101/msteams",
      }),
    ]);
    setBundledManifestIdsByRoot({
      [appBundledPluginRoot("feishu")]: "feishu",
  ...
  it.each([
    [
      ...
      { kind: "npmSpec", value: "@realcarlossanchez101/feishu" } as const,
      { pluginId: "feishu", localPath: appBundledPluginRoot("feishu") },
    ],
    [
      "returns undefined for missing npm spec",
      { kind: "npmSpec", value: "@realcarlossanchez101/not-found" } as const,
      undefined,
    ],
    [
  ...
  it("forwards an explicit env to bundled discovery helpers", () => {
    ...
    expect(discoverCarlitoPluginsMock).toHaveBeenNthCalledWith(1, {
      workspaceDir: "/workspace",
      env,
    });
    expect(discoverCarlitoPluginsMock).toHaveBeenNthCalledWith(2, {
      workspaceDir: "/workspace",
      env,
    });
  });
  ...
  it("reuses a pre-resolved bundled map for repeated lookups", () => {
    ...
    expect(
      findBundledPluginSourceInMap({
        bundled,
        lookup: { kind: "npmSpec", value: "@realcarlossanchez101/feishu" },
      })?.pluginId,
    ).toBe("feishu");
  });
});
