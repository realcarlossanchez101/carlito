...
const tempRoot = makeTrackedTempDir("carlito-plugin-binding", tempDirs);
...
vi.mock("../infra/home-dir.js", async () => {
  ...
  return {
    ...
    expandHomePrefix: (value: string) => {
      if (value === "~/.carlito/plugin-binding-approvals.json") {
  ...
  it.each([
    ...
    },
    {
      ...
      existingRecord: {
        ...
        targetSessionKey: "carlito-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
        targetKind: "session" as const,
        conversation: {
          channel: "telegram",
          accountId: "default",
