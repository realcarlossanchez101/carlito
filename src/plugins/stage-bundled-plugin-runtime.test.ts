...
describe("stageBundledPluginRuntime", () => {
  it("stages bundled dist plugins as runtime wrappers and links staged dist node_modules", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-");
    ...
    expect(
      fs
        .lstatSync(
          path.join(repoRoot, "dist", "extensions", "node_modules", "carlito", "plugin-sdk"),
        )
        .isSymbolicLink(),
    ).toBe(false);
    expect(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "node_modules", "carlito", "package.json"),
        "utf8",
      ),
    ...
    expect(fs.existsSync(path.join(runtimePluginDir, "node_modules", "carlito"))).toBe(false);
  });

  it("keeps extension-local plugin-sdk wrappers resolving canonical dist chunks", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-sdk-wrapper-");
  ...
  it("writes wrappers that forward plugin entry imports into canonical dist files", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-chunks-");
    createDistPluginDir(repoRoot, "diffs");
    setupRepoFiles(repoRoot, {
  ...
  it("stages root runtime sidecars that bundled plugin boundaries resolve directly", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-sidecars-");
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__carlitoTestPluginCommands ??= new Map();",
  ...
  it("copies package metadata files but symlinks other non-js plugin artifacts into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-assets-");
    createDistPluginDir(repoRoot, "diffs");
    setupRepoFiles(repoRoot, {
      [bundledDistPluginFile("diffs", "package.json")]: JSON.stringify(
        { name: "@realcarlossanchez101/diffs", carlito: { extensions: ["./index.js"] } },
        null,
        2,
      ),
      [bundledDistPluginFile("diffs", "carlito.plugin.json")]: "{}\n",
    ...
    expectRuntimeArtifactText({
      repoRoot,
      pluginId: "diffs",
      relativePath: "carlito.plugin.json",
      expectedText: "{}\n",
      symbolicLink: false,
    });
    expectRuntimeArtifactText({
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-discovery-");
    ...
    setupRepoFiles(repoRoot, {
      [bundledDistPluginFile("demo", "package.json")]: JSON.stringify(
        {
          name: "@realcarlossanchez101/demo",
          carlito: {
            extensions: ["./main.js"],
            setupEntry: "./setup.js",
            startup: {
      ...
      [bundledDistPluginFile("demo", "carlito.plugin.json")]: JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
    ...
    const env = {
      ...
      CARLITO_BUNDLED_PLUGINS_DIR: runtimeExtensionsDir,
    };
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-stale-");
  ...
  it("removes dist-runtime when the built bundled plugin tree is absent", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-missing-");
  ...
  it("tolerates EEXIST when an identical runtime symlink is materialized concurrently", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-eexist-");
