...
const mocks = vi.hoisted(() => ({
  ...
  withBundledPluginAllowlistCompat: vi.fn(
    ({ config, pluginIds }: { config?: CarlitoConfig; pluginIds: string[] }) =>
      ({
        ...config,
        plugins: {
          ...config?.plugins,
...
function createCompatChainConfig() {
  const cfg = { plugins: { allow: ["custom-plugin"] } } as CarlitoConfig;
...
describe("resolvePluginCapabilityProviders", () => {
  ...
  it("keeps active capability providers even when cfg is passed", () => {
    ...
    const providers = resolvePluginCapabilityProviders({
      key: "speechProviders",
      cfg: { messages: { tts: { provider: "edge" } } } as CarlitoConfig,
    });
