...
describe("stageBundledPluginRuntimeDeps", () => {
  it("drops Lark SDK type cargo while keeping runtime entrypoints", () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-deps-");
  ...
  it("strips non-runtime dependency sections before fallback runtime staging", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-manifest-");
    writeRepoFile(
      repoRoot,
      "dist/extensions/amazon-bedrock/package.json",
      JSON.stringify(
        {
          name: "@realcarlossanchez101/amazon-bedrock-provider",
          version: "2026.4.10",
          dependencies: {
  ...
  it("patches installed Baileys encryptedStream flush ordering for shipped runtime deps", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-");
  ...
  it("patches the Baileys dispatcher guard when the flush hotfix is already present", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-dispatcher-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
  ...
  it("patches the Baileys dispatcher guard even when the encryptedStream block changed", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-dispatcher-only-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
  ...
  it("fails when the dispatcher block drifts even if encryptedStream is patchable", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-dispatcher-drifted-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
  ...
  it("patches the Baileys dispatcher guard when sequential awaits include comments", async () => {
    const repoRoot = makeRepoRoot(
      "carlito-stage-bundled-runtime-hotfix-dispatcher-sequential-comments-",
    );
    const targetPath = path.join(
      repoRoot,
      "node_modules",
  ...
  it("patches the Baileys dispatcher guard when the flush hotfix uses sequential awaits", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-dispatcher-sequential-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
  ...
  it("preserves the original module read mode when replacing Baileys", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-mode-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
  ...
  it("refuses symlink targets for the Baileys hotfix", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-symlink-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
  ...
  it("downgrades Baileys hotfix write failures to a non-fatal result", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-write-failure-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
  ...
  it("refuses pre-created symlink temp paths instead of following them", async () => {
    const repoRoot = makeRepoRoot("carlito-stage-bundled-runtime-hotfix-temp-symlink-");
    const targetPath = path.join(
      repoRoot,
      "node_modules",
      "@whiskeysockets",
