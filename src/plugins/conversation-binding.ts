...
const APPROVALS_PATH = "~/.carlito/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "carlito-app-server:thread:",
  "carlito-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("carlito.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("carlito.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Carlito instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
