...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.carlito.ai/gateway/health",
            "https://docs.carlito.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("carlito gateway run")}`,
          `Or rerun with: ${formatCliCommand("carlito onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("carlito onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.carlito.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
    ...
    if (gatewayProbe.ok) {
      await prompter.note(
        [
          ...
          "Stored in: $CARLITO_CONFIG_PATH (default: ~/.carlito/carlito.json) under gateway.auth.token, or in CARLITO_GATEWAY_TOKEN.",
          `View token: ${formatCliCommand("carlito config get gateway.auth.token")}`,
          `Generate token: ${formatCliCommand("carlito doctor --generate-gateway-token")}`,
          "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
          `Open the dashboard anytime: ${formatCliCommand("carlito dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Carlito."
            : "Copy/paste this URL in a browser on this machine to control Carlito.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("carlito dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.carlito.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.carlito.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("carlito configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("carlito configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.carlito.ai/tools/web"}`,
          "Docs: https://docs.carlito.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("carlito configure --section web")}`,
          "",
          "Docs: https://docs.carlito.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://carlito.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Carlito."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Carlito.",
  );
