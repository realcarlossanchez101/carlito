...
export async function runSetupWizard(
  ...
  await prompter.intro("Carlito setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: CarlitoConfig = snapshot.valid
    ? snapshot.exists
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("carlito doctor")}`,
        `Inspect: ${formatCliCommand("carlito plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("carlito configure")}.`;
  ...
  let localGatewayToken = process.env.CARLITO_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.CARLITO_GATEWAY_PASSWORD;
  try {
