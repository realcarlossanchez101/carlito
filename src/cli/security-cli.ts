...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["carlito security audit", "Run a local security audit."],
          ["carlito security audit --deep", "Include best-effort live Gateway probe checks."],
          ["carlito security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "carlito security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["carlito security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["carlito security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.carlito.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Carlito security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("carlito security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("carlito security audit --fix")}`));
        if (!fixResult) {
