...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as CarlitoConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: CarlitoConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
export const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/carlito-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: CarlitoConfig) =>
    invokeMock<
      [CarlitoConfig],
...
vi.mock("../plugins/enable.js", () => ({
  enablePluginInConfig: ((cfg: CarlitoConfig, pluginId: string) =>
    invokeMock<[CarlitoConfig, string], unknown>(
...
export function resetPluginsCliTestState() {
  ...
  replaceConfigFile.mockImplementation(
    (async (params: { nextConfig: CarlitoConfig }) => await writeConfigFile(params.nextConfig)) as (
      ...args: unknown[]
    ) => Promise<unknown>,
  );
  resolveStateDir.mockReturnValue("/tmp/carlito-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation(((cfg: CarlitoConfig) => ({ config: cfg })) as (
    ...args: unknown[]
  ) => unknown);
  recordPluginInstall.mockImplementation(
    ((cfg: CarlitoConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: CarlitoConfig }) => ({
    config,
    warnings: [],
