...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.CARLITO_GATEWAY_STARTUP_TRACE);
...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or CARLITO_GATEWAY_PASSWORD.",
  );
}
...
function maybeLogPendingControlUiBuild(cfg: CarlitoConfig): void {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("carlito setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("carlito onboard --mode local")}\` or \`${formatCliCommand("carlito setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
}): Promise<{ cfg: CarlitoConfig; snapshot: ConfigFileSnapshot | null }> {
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = normalizeOptionalLowercaseString(process.env.CARLITO_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.CARLITO_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.CARLITO_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.CARLITO_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (process.env.CARLITO_SERVICE_MARKER?.trim()) {
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or CARLITO_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set CARLITO_GATEWAY_TOKEN or CARLITO_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or CARLITO_GATEWAY_TOKEN/CARLITO_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("carlito gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: CARLITO_GATEWAY_TOKEN env if set)",
    )
