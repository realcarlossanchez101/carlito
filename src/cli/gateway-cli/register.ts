      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["carlito gateway run", "Run the gateway in the foreground."],
            ["carlito gateway status", "Show service status plus connectivity/capability."],
            ["carlito gateway discover", "Find local and wide-area gateway beacons."],
            ["carlito gateway stability", "Show recent stability diagnostics."],
            ["carlito gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.carlito.ai/cli/gateway")}\n`,
      ),
  );
