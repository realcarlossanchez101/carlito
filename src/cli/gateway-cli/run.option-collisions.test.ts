...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/carlito-stability.json",
  path: "/tmp/carlito-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/carlito-control-ui" as string | null,
}));

const { runtimeErrors, defaultRuntime, resetRuntimeCapture } = createCliRuntimeCapture();

vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/carlito-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/carlito-control-ui";
  ...
  it.each([
    ...
    delete process.env.CARLITO_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.CARLITO_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `carlito onboard --mode local` or `carlito setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("restores last-known-good config before startup when the effective config is invalid", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/carlito-test-missing-config.json",
      config: {},
      parsed: null,
    ...
    expect(gatewayLogMessages).toContain(
      "gateway: restored invalid effective config from last-known-good backup: /tmp/carlito-test-missing-config.json",
    );
    expect(startGatewayServer).toHaveBeenCalledWith(
      19170,
      expect.objectContaining({
