  ...
  plugins: [
    {
      ...
      source: "bundled",
      manifestPath: "/tmp/memory-wiki/carlito.plugin.json",
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "carlito", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "carlito", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "carlito", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "carlito", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "carlito", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "carlito", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "carlito", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "carlito", "models", "status", "--json"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "carlito", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "carlito", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "carlito", "acp", "-v"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "carlito", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "carlito", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "carlito", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "carlito", "--help", "status"])).toBe(false);
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("carlito memory");
  });
