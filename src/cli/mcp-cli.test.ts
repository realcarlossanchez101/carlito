...
const serveCarlitoChannelMcp = mocks.serveCarlitoChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveCarlitoChannelMcp: mocks.serveCarlitoChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("carlito-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("carlito-cli-mcp-home-", async () => {
      ...
      expect(serveCarlitoChannelMcp).toHaveBeenCalledWith({
