...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.CARLITO_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.CARLITO_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.CARLITO_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.CARLITO_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.CARLITO_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "carlito", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "carlito"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "carlito", "nodes", "list"], "carlito");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "carlito", "infer"], "carlito");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "carlito", "acp", "--help"], "carlito");
