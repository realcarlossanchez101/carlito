...
${formatHelpExamples([
  ['carlito agent --to +15555550123 --message "status update"', "Start a new session."],
  ['carlito agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['carlito agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.carlito.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.carlito.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['carlito agents set-identity --agent main --name "Carlito" --emoji "🦞"', "Set name + emoji."],
  ["carlito agents set-identity --agent main --avatar avatars/carlito.png", "Set avatar path."],
  [
    "carlito agents set-identity --workspace ~/.carlito/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "carlito agents set-identity --identity-file ~/.carlito/workspace/IDENTITY.md --agent main",
