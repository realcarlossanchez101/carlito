...
describe("private-qa-cli", () => {
  ...
  it("loads the private QA CLI from a source checkout path", async () => {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-private-qa-source-"));
  ...
  it("rejects non-source package roots even when private QA is enabled", async () => {
    process.env.CARLITO_ENABLE_PRIVATE_QA_CLI = "1";
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-private-qa-"));
    tempDirs.push(root);
    fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "carlito" }), "utf8");
    ...
    ).toThrow("Private QA CLI is only available from an Carlito source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", async () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an Carlito source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
