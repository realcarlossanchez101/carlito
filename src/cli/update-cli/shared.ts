...
import { resolveCarlitoPackageRoot } from "../../infra/carlito-root.js";
...
const CARLITO_REPO_URL = "https://github.com/realcarlossanchez101/carlito.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "carlito";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["carlito", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.CARLITO_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "carlito");
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveCarlitoPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", CARLITO_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CARLITO_GIT_DIR points at a non-git directory: ${params.dir}. Set CARLITO_GIT_DIR to an empty folder or an carlito checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`CARLITO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "carlito.mjs");
