...
describe("parseCliContainerArgs", () => {
  it("extracts a root --container flag before the command", () => {
    expect(
      parseCliContainerArgs(["node", "carlito", "--container", "demo", "status", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "carlito", "status", "--deep"],
    });
  });

  it("accepts the equals form", () => {
    expect(parseCliContainerArgs(["node", "carlito", "--container=demo", "health"])).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "carlito", "health"],
    });
  });

  it("rejects a missing container value", () => {
    expect(parseCliContainerArgs(["node", "carlito", "--container"])).toEqual({
      ok: false,
  ...
  it("does not consume an adjacent flag as the container value", () => {
    expect(
      parseCliContainerArgs(["node", "carlito", "--container", "--no-color", "status"]),
    ).toEqual({
      ok: false,
      error: "--container requires a value",
    });
  ...
  it("leaves argv unchanged when the flag is absent", () => {
    expect(parseCliContainerArgs(["node", "carlito", "status"])).toEqual({
      ok: true,
      container: null,
  ...
  it("extracts --container after the command like other root options", () => {
    expect(
      parseCliContainerArgs(["node", "carlito", "status", "--container", "demo", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "carlito", "status", "--deep"],
...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to CARLITO_CONTAINER", () => {
    expect(
      resolveCliContainerTarget(["node", "carlito", "--container", "demo", "status"], {}),
    ).toBe("demo");
    expect(resolveCliContainerTarget(["node", "carlito", "status"], {})).toBeNull();
    expect(
      resolveCliContainerTarget(["node", "carlito", "status"], {
        CARLITO_CONTAINER: "demo",
...
describe("maybeRunCliInContainer", () => {
  it("passes through when no container target is provided", () => {
    expect(maybeRunCliInContainer(["node", "carlito", "status"], { env: {} })).toEqual({
      handled: false,
      argv: ["node", "carlito", "status"],
    });
  });

  it("uses CARLITO_CONTAINER when the flag is absent", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "carlito", "status"], {
        env: { CARLITO_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("clears inherited host routing and gateway env before execing into the child CLI", () => {
    ...
    maybeRunCliInContainer(["node", "carlito", "status"], {
      env: {
        ...
        CARLITO_GATEWAY_PASSWORD: "password",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("executes through podman when the named container is running", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "carlito", "--container", "demo", "status"], {
        env: {},
        spawnSync,
      }),
    ).toEqual({
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "carlito", "--container", "demo", "health"], {
        env: { USER: "carlito" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "carlito", CARLITO_CONTAINER: "" },
      },
    );
  });
  ...
  it("checks docker after podman and before failing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "somalley", CARLITO_CONTAINER: "" },
      },
    );
  ...
  it("allocates a tty for interactive terminal sessions", () => {
    ...
    maybeRunCliInContainer(["node", "carlito", "--container", "demo", "setup"], {
      env: {},
      spawnSync,
      stdinIsTTY: true,
      stdoutIsTTY: true,
  ...
  it("prefers --container over CARLITO_CONTAINER", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "carlito", "--container", "flag-demo", "health"], {
        env: { CARLITO_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "carlito", "--container", "demo", "status"], {
        env: { CARLITO_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
      argv: ["node", "carlito", "--container", "demo", "status"],
    });
  });
  ...
  it("blocks updater commands from running inside the container", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "carlito", "--container", "demo", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
  ...
  it("blocks update after interleaved root flags", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "carlito", "--container", "demo", "--no-color", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
