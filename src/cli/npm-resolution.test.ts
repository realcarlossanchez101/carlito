...
const CLI_STATE_ROOT = "/tmp/carlito";
const ALPHA_INSTALL_PATH = installedPluginRoot(CLI_STATE_ROOT, "alpha");

describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@realcarlossanchez101/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@realcarlossanchez101/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@realcarlossanchez101/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@realcarlossanchez101/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @realcarlossanchez101/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      mapNpmResolutionMetadata({
        name: "@realcarlossanchez101/plugin-alpha",
        version: "1.2.3",
        resolvedSpec: "@realcarlossanchez101/plugin-alpha@1.2.3",
      ...
      resolvedName: "@realcarlossanchez101/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@realcarlossanchez101/plugin-alpha@1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual([
      "Pinned npm install record to @realcarlossanchez101/plugin-alpha@1.2.3.",
    ]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@realcarlossanchez101/plugin-alpha@latest",
      true,
      ALPHA_INSTALL_PATH,
      "1.2.3",
      undefined,
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@realcarlossanchez101/plugin-alpha@latest",
      installPath: ALPHA_INSTALL_PATH,
      version: "1.2.3",
