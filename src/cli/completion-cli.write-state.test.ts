...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["carlito.bash", "carlito.fish", "carlito.ps1", "carlito.zsh"]),
    );
