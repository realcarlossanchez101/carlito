...
const CLI_STATE_ROOT = "/tmp/carlito-state";

function cliInstallPath(pluginId: string): string {
  return installedPluginRoot(CLI_STATE_ROOT, pluginId);
}

function createEnabledPluginConfig(pluginId: string): CarlitoConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): CarlitoConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createPathHookPackInstalledConfig(tmpRoot: string): CarlitoConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): CarlitoConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing carlito.plugin.json",
  });
...
function primeHookPackPathFallback(params: {
  ...
  loadConfig.mockReturnValue({} as CarlitoConfig);
...
describe("plugins cli install", () => {
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `carlito doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-hook-link-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/carlito/.carlito/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing carlito.hooks",
    });
  ...
  it("passes the install logger to the --link dry-run probe", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-link-plugin-"));
    const cfg = {
      plugins: {
        entries: {},
        load: {
  ...
  it("does not fall back to hook pack for local path when dangerous force unsafe install is set", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-local-plugin-"));
    const cfg = {} as CarlitoConfig;
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-local-hook-pack-"));
    const cfg = {} as CarlitoConfig;
    const installedCfg = {
      hooks: {
        internal: {
    ...
    installPluginFromPath.mockResolvedValue({
      ok: false,
      error: "package.json missing carlito.plugin.json",
      code: "missing_carlito_extensions",
    });
  ...
  it("does not fall back to npm when ClawHub rejects a real package", async () => {
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "carlito skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "carlito skills install demo" instead.');
  });
