...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "carlito", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "carlito", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "carlito", "--profile", "work", "gateway", "status"],
