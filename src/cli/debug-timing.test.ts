...
describe("cli debug timing", () => {
  it("does not emit timing lines unless CARLITO_DEBUG_TIMING enables a mode", () => {
    const writer = vi.fn();
  ...
  it("emits readable timing lines with CARLITO_DEBUG_TIMING=1", () => {
    const writer = vi.fn();
    const timing = createCliDebugTiming({
      command: "models list",
      env: { CARLITO_DEBUG_TIMING: "1" },
      writer,
    });
  ...
  it("emits parseable timing JSON lines with CARLITO_DEBUG_TIMING=json", async () => {
    const writer = vi.fn();
    const timing = createCliDebugTiming({
      command: "models list",
      env: { CARLITO_DEBUG_TIMING: "json" },
      writer,
    });
