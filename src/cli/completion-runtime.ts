...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Carlito Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# Carlito Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(carlito completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "carlito") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
