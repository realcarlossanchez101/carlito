    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "carlito node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["carlito node status", "Check node host service status."],
          ["carlito node install", "Install the node host service."],
          ["carlito node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.carlito.ai/cli/node")}\n`,
    );
