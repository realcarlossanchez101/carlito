...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    CARLITO_STATE_DIR: "/tmp/carlito-daemon",
    CARLITO_CONFIG_PATH: "/tmp/carlito-daemon/carlito.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.CARLITO_STATE_DIR ?? "/tmp/carlito-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.CARLITO_CONFIG_PATH ?? `${stateDir}/carlito.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/carlito-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CARLITO_STATE_DIR = "/tmp/carlito-cli";
    process.env.CARLITO_CONFIG_PATH = "/tmp/carlito-cli/carlito.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/carlito-daemon/carlito.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/carlito-cli/carlito.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.CARLITO_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.CARLITO_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.CARLITO_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
