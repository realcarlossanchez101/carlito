...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("carlito-daemon-install-int-");
    configPath = path.join(tempHome, "carlito.json");
    process.env.HOME = tempHome;
    process.env.CARLITO_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.CARLITO_GATEWAY_TOKEN).toBeUndefined();
  });
});
