...
const buildGatewayInstallPlanMock = vi.hoisted(() =>
  vi.fn(async () => ({
    programArguments: ["carlito", "gateway", "run"],
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/carlito.json" },
  })),
}));
...
function mockResolvedGatewayTokenSecretRef() {
  ...
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:CARLITO_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${CARLITO_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["carlito", "gateway", "run"],
      environment: {
        CARLITO_STATE_DIR: "/tmp/carlito-doctor-manual",
        CARLITO_CONFIG_PATH: "/tmp/carlito-doctor-manual/carlito.json",
        CARLITO_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.CARLITO_STATE_DIR).toBeUndefined();
      expect(env.CARLITO_CONFIG_PATH).toBeUndefined();
