...
function buildExplicitApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["carlito", "devices", "approve", requestId];
      ...
      .action(async (requestId: string | undefined, opts: DevicesRpcOpts) => {
        ...
        if (usingImplicitSelection) {
          ...
          switch (approval.kind) {
            ...
            case "re-approval":
              defaultRuntime.log(
                "  Note:   Already paired. Approval-bound device details changed, so Carlito created a fresh request instead of silently reusing the old approval.",
              );
              break;
            case "new-pairing":
