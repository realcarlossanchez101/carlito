...
declare const __CARLITO_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "carlito";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["CARLITO_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["CARLITO_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["CARLITO_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.CARLITO_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Carlito version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __CARLITO_VERSION__ === "string" ? __CARLITO_VERSION__ : undefined,
  bundledVersion: process.env.CARLITO_BUNDLED_VERSION,
});
