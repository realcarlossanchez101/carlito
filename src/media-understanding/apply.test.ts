...
const TEMP_MEDIA_PREFIX = "carlito-media-";
...
function createGroqAudioConfig(): CarlitoConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): CarlitoConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): CarlitoConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: CarlitoConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredCarlitoTmpDir();
