...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        CARLITO_BASE_ENV: "base",
        CARLITO_TO_REMOVE: undefined,
      },
      env: {
        CARLITO_TEST_ENV: "ok",
      },
    ...
    expect(resolved.CARLITO_BASE_ENV).toBe("base");
    expect(resolved.CARLITO_TEST_ENV).toBe("ok");
    expect(resolved.CARLITO_TO_REMOVE).toBeUndefined();
    expect(resolved.CARLITO_CLI).toBe(CARLITO_CLI_ENV_VALUE);
  });
