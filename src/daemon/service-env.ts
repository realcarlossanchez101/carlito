...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.CARLITO_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CARLITO_PROFILE: profile,
    CARLITO_GATEWAY_PORT: String(port),
    CARLITO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CARLITO_SYSTEMD_UNIT: systemdUnit,
    CARLITO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    CARLITO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CARLITO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    CARLITO_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.CARLITO_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CARLITO_GATEWAY_TOKEN: gatewayToken,
    CARLITO_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CARLITO_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CARLITO_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CARLITO_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CARLITO_LOG_PREFIX: "node",
    CARLITO_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CARLITO_SERVICE_KIND: NODE_SERVICE_KIND,
    CARLITO_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    CARLITO_STATE_DIR: sharedEnv.stateDir,
    CARLITO_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.CARLITO_STATE_DIR;
  const configPath = env.CARLITO_CONFIG_PATH;
