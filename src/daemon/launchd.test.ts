...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  const label = "ai.carlito.gateway";
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.carlito.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", CARLITO_PROFILE: "default" },
    });
...
describe("launchd install", () => {
  ...
  it("stops LaunchAgent by disabling relaunch before stopping the process", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.carlito.gateway`;
    expect(state.launchctlCalls).toContainEqual(["disable", serviceId]);
    expect(state.launchctlCalls).toContainEqual(["stop", "ai.carlito.gateway"]);
  ...
  it("treats already-unloaded services as successfully stopped without bootout fallback", async () => {
    ...
    expect(state.launchctlCalls).toContainEqual([
      "disable",
      `${typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501"}/ai.carlito.gateway`,
    ]);
    expect(state.launchctlCalls.some((call) => call[0] === "bootout")).toBe(false);
    expect(output).toContain("Stopped LaunchAgent");
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.carlito.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when CARLITO_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.carlito.gateway.plist",
    },
    {
      name: "uses profile-specific label when CARLITO_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", CARLITO_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.carlito.jbphoenix.plist",
    },
    {
      name: "prefers CARLITO_LAUNCHD_LABEL over CARLITO_PROFILE",
      env: {
        HOME: "/Users/test",
        CARLITO_PROFILE: "jbphoenix",
        CARLITO_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from CARLITO_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        CARLITO_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty CARLITO_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        CARLITO_PROFILE: "myprofile",
        CARLITO_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.carlito.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        CARLITO_LAUNCHD_LABEL: "../evil/label",
      }),
