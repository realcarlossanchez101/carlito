...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.CARLITO_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.CARLITO_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = env.CARLITO_GATEWAY_PORT?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.CARLITO_GATEWAY_PORT) ??
...
async function activateScheduledTask(params: {
  ...
  const taskDescription = params.description ?? "Carlito Gateway";
