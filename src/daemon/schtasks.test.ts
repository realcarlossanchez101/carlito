...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when CARLITO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".carlito", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when CARLITO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", CARLITO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".carlito-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers CARLITO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        CARLITO_PROFILE: "rescue",
        CARLITO_STATE_DIR: "C:\\State\\carlito",
      },
      expected: path.join("C:\\State\\carlito", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", CARLITO_PROFILE: "default" },
      expected: path.join("/home/test", ".carlito", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Carlito Gateway",
          "cd /d C:\\Projects\\carlito",
          "set NODE_ENV=production",
          "set CARLITO_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\carlito",
          environment: {
            NODE_ENV: "production",
            CARLITO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Carlito Share\\node.exe" "\\\\fileserver\\Carlito Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Carlito Share\\node.exe",
            "\\\\fileserver\\Carlito Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from CARLITO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ CARLITO_STATE_DIR: path.join(tmpDir, "custom-state") }),
