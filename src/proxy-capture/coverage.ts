...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[carlito proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[carlito proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`carlito proxy coverage\` for details.`,
  );
}
