...
type ConfiguredBinding = NonNullable<CarlitoConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: CarlitoConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/carlito" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
