...
const createTempDir = () => tempDirs.make("carlito-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets CARLITO_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "carlito",
    });

    expect(env.CARLITO_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("carlito");
  });

  it("overrides pre-existing CARLITO_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      CARLITO_SHELL: "wrong",
    });
    expect(env.CARLITO_SHELL).toBe("acp-client");
  ...
  it("preserves CARLITO_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        CARLITO_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["CARLITO_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Carlito bridge", () => {
    ...
    expect(env.CARLITO_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.CARLITO_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "carlito",
        serverArgs: ["acp"],
        defaultServerCommand: "carlito",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("CARLITO_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "carlito", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "carlito", "dist", "entry.js");
    const shimPath = path.join(dir, "carlito.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\carlito\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/carlito-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/carlito-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/carlito-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
