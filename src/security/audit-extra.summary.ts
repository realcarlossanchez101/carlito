...
function summarizeGroupPolicy(cfg: CarlitoConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: CarlitoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: CarlitoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: CarlitoConfig): boolean {
...
function isBrowserEnabled(cfg: CarlitoConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: CarlitoConfig): SecurityAuditFinding[] {
