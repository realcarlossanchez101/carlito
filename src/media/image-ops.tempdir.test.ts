...
describe("image-ops temp dir", () => {
  ...
  beforeEach(() => {
    process.env.CARLITO_IMAGE_BACKEND = "sips";
  ...
  afterEach(() => {
    delete process.env.CARLITO_IMAGE_BACKEND;
    vi.restoreAllMocks();
  });

  it("creates sips temp dirs under the secured Carlito tmp root", async () => {
    const secureRoot = resolvePreferredCarlitoTmpDir();

    await getImageMetadata(Buffer.from("image"));

    expect(fs.mkdtemp).toHaveBeenCalledTimes(1);
    expect(fs.mkdtemp).toHaveBeenCalledWith(path.join(secureRoot, "carlito-img-"));
    expect(createdTempDir.startsWith(path.join(secureRoot, "carlito-img-"))).toBe(true);
