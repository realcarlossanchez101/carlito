...
describe("Dockerfile", () => {
  it("uses shared multi-arch base image refs for all root Node stages", async () => {
    ...
    expect(dockerfile).toContain("FROM ${CARLITO_NODE_BOOKWORM_IMAGE} AS ext-deps");
    expect(dockerfile).toContain("FROM ${CARLITO_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${CARLITO_NODE_BOOKWORM_IMAGE} AS base-default");
    expect(dockerfile).toContain("FROM ${CARLITO_NODE_BOOKWORM_SLIM_IMAGE} AS base-slim");
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG CARLITO_INSTALL_BROWSER");
  ...
  it("prunes runtime dependencies after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG CARLITO_EXTENSIONS");
    expect(dockerfile).toContain("ARG CARLITO_BUNDLED_PLUGIN_DIR");
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    ...
    expect(dockerfile).toContain(`ARG CARLITO_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bCARLITO_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for CARLITO_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
