...
describe("shared/node-match", () => {
  ...
  it("prefers a unique current Carlito client over a legacy clawdbot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=carlito-macos.*node=third-mac.*client=carlito-macos/,
    );
  });
