...
export function resolveCarlitoManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type CarlitoManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveCarlitoManifestRequires(
  metadataObj: Record<string, unknown>,
): CarlitoManifestRequires | undefined {
  const requiresRaw =
...
export function resolveCarlitoManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveCarlitoManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedCarlitoManifestInstallBase = {
...
export function parseCarlitoManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedCarlitoManifestInstallBase | undefined {
  ...
  const spec: ParsedCarlitoManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyCarlitoManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedCarlitoManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
