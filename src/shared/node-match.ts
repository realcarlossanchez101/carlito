...
function isCurrentCarlitoClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("carlito-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentCarlitoClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentCarlitoClient(node.clientId)) {
    score += 10;
