...
function setConfiguredFetchProviderKey(configTarget: CarlitoConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: CarlitoConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: CarlitoConfig, provider: ProviderUnderTest): unknown {
