...
let compiledSecretTargetRegistryState: {
  ...
  carlitoCompiledSecretTargets: CompiledTargetRegistryEntry[];
  carlitoTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreCarlitoTargetState: {
  knownTargetIds: Set<string>;
  carlitoCompiledSecretTargets: CompiledTargetRegistryEntry[];
  carlitoTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledBundledChannelCarlitoTargets = new Map<
  string,
...
function getCompiledSecretTargetRegistryState() {
  ...
  const carlitoCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "carlito.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    carlitoCompiledSecretTargets,
    carlitoTargetsById: buildConfigTargetIdIndex(carlitoCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreCarlitoTargetState() {
  if (compiledCoreCarlitoTargetState) {
    return compiledCoreCarlitoTargetState;
  }
  const carlitoCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "carlito.json")
    .map(compileTargetRegistryEntry);
  compiledCoreCarlitoTargetState = {
    knownTargetIds: new Set(carlitoCompiledSecretTargets.map((entry) => entry.id)),
    carlitoCompiledSecretTargets,
    carlitoTargetsById: buildConfigTargetIdIndex(carlitoCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(carlitoCompiledSecretTargets),
  };
  return compiledCoreCarlitoTargetState;
}

function getCompiledBundledChannelCarlitoTargets(
  channelId: string,
): CompiledTargetRegistryEntry[] | null {
  ...
  if (compiledBundledChannelCarlitoTargets.has(normalizedChannelId)) {
    return compiledBundledChannelCarlitoTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadBundledChannelSecretContractApi(normalizedChannelId)
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "carlito.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledBundledChannelCarlitoTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreCarlitoTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreCarlitoTargetState().carlitoCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitBundledChannelEntries = explicitBundledChannelId
    ? getCompiledBundledChannelCarlitoTargets(explicitBundledChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().carlitoCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargets(config: CarlitoConfig): DiscoveredConfigSecretTarget[] {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreCarlitoTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreCarlitoTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.carlitoCompiledSecretTargets,
    entriesById: registryState.carlitoTargetsById,
  });
