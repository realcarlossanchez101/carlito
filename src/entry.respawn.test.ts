...
import {
  ...
  CARLITO_NODE_EXTRA_CA_CERTS_READY,
  CARLITO_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(plan?.env[CARLITO_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(plan?.env[CARLITO_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "carlito", "gateway", "run"],
        env: {
          [CARLITO_NODE_EXTRA_CA_CERTS_READY]: "1",
          [CARLITO_NODE_OPTIONS_READY]: "1",
        },
