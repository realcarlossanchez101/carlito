...
describe("logger helpers", () => {
  ...
  it("writes to configured log file at configured level", () => {
    withTempDirSync({ prefix: "carlito-log-test-" }, (dir) => {
      const logPath = path.join(dir, "carlito.log");
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `carlito-${today}.log`);

    // create an old file to be pruned
    const oldPath = path.join(DEFAULT_LOG_DIR, "carlito-2000-01-01.log");
