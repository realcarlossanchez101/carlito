...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("carlito-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "carlito-sandbox-browser-global" },
      agentBrowser: { network: "carlito-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("carlito-sandbox-browser-agent");
  });
