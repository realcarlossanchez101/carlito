...
import { buildWebSearchProviderConfig, withTempHome, writeCarlitoConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = CarlitoSchema.safeParse({
      $schema: "https://carlito.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://carlito.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = CarlitoSchema.safeParse({ $schema: 123 });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://carlito.ai/config.json");
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@carlito"]);
    }
  });
...
describe("config strict validation", () => {
  ...
  it("does not treat resolved-only gateway.bind aliases as source-literal legacy or invalid", async () => {
    await withTempHome(async (home) => {
      await writeCarlitoConfig(home, {
        gateway: { bind: "${CARLITO_BIND}" },
      });

      const prev = process.env.CARLITO_BIND;
      process.env.CARLITO_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(true);
        ...
        if (prev === undefined) {
          delete process.env.CARLITO_BIND;
        } else {
          process.env.CARLITO_BIND = prev;
        }
      }
    });
  });
