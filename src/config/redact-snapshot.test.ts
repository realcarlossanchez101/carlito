...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/carlito/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/carlito/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/carlito/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/carlito/nickserv-password.txt");
