...
export function parseCarlitoVersion(raw: string | null | undefined): CarlitoVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeCarlitoVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseCarlitoVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameCarlitoStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseCarlitoVersion(a);
  const parsedB = parseCarlitoVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareCarlitoVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseCarlitoVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseCarlitoVersion(current);
  const parsedTouched = parseCarlitoVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameCarlitoStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareCarlitoVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: CarlitoVersion): number {
