...
function collectConfigEnvVarsByTarget(cfg?: CarlitoConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: CarlitoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: CarlitoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: CarlitoConfig): Record<string, string> {
...
export function applyConfigEnvVars(cfg: CarlitoConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.CARLITO_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
