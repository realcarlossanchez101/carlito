...
// Keep integration tests deterministic: never read a real carlito.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "carlito-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.CARLITO_SESSION_CACHE_TTL_MS;
    process.env.CARLITO_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.CARLITO_SESSION_CACHE_TTL_MS;
    } else {
      process.env.CARLITO_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
