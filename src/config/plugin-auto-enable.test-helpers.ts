...
export function makeTempDir(): string {
  return makeTrackedTempDir("carlito-plugin-auto-enable", tempDirs);
}
...
export function writePluginManifestFixture(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "carlito.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/carlito.plugin.json`,
    })),
    diagnostics: [],
  };
}
