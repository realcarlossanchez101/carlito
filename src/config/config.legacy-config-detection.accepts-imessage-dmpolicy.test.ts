...
function expectCarlitoSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = CarlitoSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectCarlitoSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = CarlitoSchema.safeParse(config);
