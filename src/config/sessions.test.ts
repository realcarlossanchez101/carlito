...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carlito-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ CARLITO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-carlito",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-carlito#general");
  });
  ...
  it("derives session transcripts dir from CARLITO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CARLITO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when CARLITO_STATE_DIR differs from stored paths", () => {
