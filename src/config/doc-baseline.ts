...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.CARLITO_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveCarlitoPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    CARLITO_STATE_DIR: path.join(os.tmpdir(), "carlito-config-doc-baseline-state"),
    CARLITO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
