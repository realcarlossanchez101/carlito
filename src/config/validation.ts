...
import type { CarlitoConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.carlito.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: CarlitoConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: CarlitoConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: CarlitoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CarlitoSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as CarlitoConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: CarlitoConfig | null | undefined;
  ...
  const ensureCompatConfig = (): CarlitoConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Carlito config schema.
