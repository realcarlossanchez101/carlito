...
// resolveCarlitoPackageRootSync. Isolate from the real repo by mocking
// moduleUrl/argv1 resolution to null and deriving only from the tmp cwd.
vi.mock("../infra/carlito-root.js", () => ({
  resolveCarlitoPackageRootSync: (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
  resolveCarlitoPackageRoot: async (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
}));
...
function seedRoot(prefix: string): string {
  ...
  writeJsonFile(path.join(root, "package.json"), { name: "carlito" });
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // CARLITO_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
    ...
    writeJsonFile(path.join(root, "dist", "channel-catalog.json"), {
      entries: [
        {
          name: "@realcarlossanchez101/fallback",
          carlito: {
            channel: {
              id: "fallback-channel",
              label: "Fallback",
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an CARLITO_BUNDLED_PLUGINS_DIR override pointing at
