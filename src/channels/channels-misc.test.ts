...
describe("WA_WEB_AUTH_DIR", () => {
  ...
  it("resolves lazily and caches across the legacy and channels/web entrypoints", async () => {
    const resolveWebChannelAuthDir = vi.fn(() => "/tmp/carlito-whatsapp-auth");
    ...
    vi.doMock("../plugins/runtime/runtime-web-channel-plugin.js", () => ({
      ...
      resolvePulsecheckRecipients: vi.fn(),
      resolveWebChannelAuthDir,
      runWebPulsecheckOnce: vi.fn(),
    ...
    expect(readLazyString(channelWeb.WA_WEB_AUTH_DIR)).toBe("/tmp/carlito-whatsapp-auth");
    expect(readLazyString(webEntry.WA_WEB_AUTH_DIR)).toBe("/tmp/carlito-whatsapp-auth");
