...
describe("group policy warning builders", () => {
  ...
  it("projects cfg-only warning collector inputs", () => {
    const collect = projectConfigWarningCollector<{ cfg: CarlitoConfig; accountId: string }>(
      ({ cfg }) => [cfg.channels ? "configured" : "none"],
    );

    expect(
      collect({
        cfg: { channels: { slack: {} } } as CarlitoConfig,
        accountId: "acct-1",
      }),
    ).toEqual(["configured"]);
  });
  ...
  it("projects account+cfg warning collector inputs", () => {
    const collect = projectAccountConfigWarningCollector<
      ...
      { account: { accountId: string }; cfg: CarlitoConfig }
    >(
      (cfg: CarlitoConfig) => cfg.channels ?? {},
