...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: CarlitoConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: CarlitoConfig }>(
  collector: WarningCollector<{ cfg: CarlitoConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: CarlitoConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: CarlitoConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: CarlitoConfig },
>(
  projectCfg: (cfg: CarlitoConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: CarlitoConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: CarlitoConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: CarlitoConfig },
>(params: {
  providerConfigPresent: (cfg: CarlitoConfig) => boolean;
