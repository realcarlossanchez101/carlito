...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): CarlitoConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as CarlitoConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: CarlitoConfig) => string[];
  input: CarlitoConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: CarlitoConfig, expected: string) {
