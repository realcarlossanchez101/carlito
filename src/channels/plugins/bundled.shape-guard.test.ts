...
function restoreBundledPluginsDir(previousBundledPluginsDir: string | undefined) {
  if (previousBundledPluginsDir === undefined) {
    ...
    process.env.CARLITO_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
  }
}
...
describe("bundled channel entry shape guards", () => {
  ...
  it("fills sparse bundled channel plugin metadata from package metadata", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-metadata-"));
    const previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.CARLITO_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-override-"));
    const previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(tempRoot, "dist", "extensions", "alpha");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.CARLITO_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.CARLITO_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.CARLITO_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("loads setup-entry feature plugins without loading the main channel entry", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-setup-only-"));
    const previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.CARLITO_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("does not load bundled setup entries through external staged runtime deps during discovery", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-setup-runtime-deps-"));
    const stageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-stage-"));
    const previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    const previousPluginStageDir = process.env.CARLITO_PLUGIN_STAGE_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
    ...
    process.env.CARLITO_PLUGIN_STAGE_DIR = stageRoot;
    ...
    try {
      ...
      if (previousPluginStageDir === undefined) {
        delete process.env.CARLITO_PLUGIN_STAGE_DIR;
      } else {
        process.env.CARLITO_PLUGIN_STAGE_DIR = previousPluginStageDir;
      }
      fs.rmSync(root, { recursive: true, force: true });
  ...
  it("swallows and caches bundled plugin and setup load failures", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-load-failure-"));
    const previousBundledPluginsDir = process.env.CARLITO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    const offenders = collectBundledChannelEntrypointOffenders(
      bundledPluginRoots,
      (source) =>
        !source.includes('from "carlito/plugin-sdk/channel-entry-contract"') ||
        source.includes('from "carlito/plugin-sdk/core"') ||
        source.includes('from "carlito/plugin-sdk/channel-core"'),
    );
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.carlito?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "carlito/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("carlito/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes('from "carlito/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "carlito-bundled-reentrant-"));
