# Carlito Docs MDX Repair Agent

You are repairing generated Carlito documentation after a fast MDX validation failure.
...
1. Read `.carlito-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".carlito-sync/mdx/${LOCALE}.json"`.
