...
This document describes how the Carlito ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`carlito acp` exposes an ACP agent over stdio and forwards prompts to a running
Carlito Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Carlito Gateway
...
- ACP client spawns `carlito acp` and speaks ACP messages over stdio.
...
`carlito acp` resolves the Gateway URL and auth from CLI flags or config:
