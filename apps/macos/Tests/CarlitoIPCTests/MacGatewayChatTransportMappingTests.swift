...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: CarlitoProtocol.AnyCodable(["ok": CarlitoProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: CarlitoProtocol.AnyCodable(["ok": CarlitoProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = CarlitoProtocol.AnyCodable([
            "runId": CarlitoProtocol.AnyCodable("run-1"),
            "sessionKey": CarlitoProtocol.AnyCodable("main"),
            "state": CarlitoProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: CarlitoProtocol.AnyCodable(["a": CarlitoProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
