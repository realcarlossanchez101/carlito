...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["carlito"]
        let text = "hey carlito"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey carlito do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["carlito"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "carlito", start: 0.2, duration: 0.1, range: transcript.range(of: "carlito")),
