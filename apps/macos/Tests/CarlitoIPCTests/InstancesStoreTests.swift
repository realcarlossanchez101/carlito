...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: CarlitoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CarlitoProtocol.AnyCodable] = [
            "presence": .init([CarlitoProtocol.AnyCodable(entry)]),
        ]
        let payload = CarlitoProtocol.AnyCodable(payloadMap)
