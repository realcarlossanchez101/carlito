...
struct CarlitoMacCLI {
    static func main() async {
        ...
        default:
            fputs("carlito-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      carlito-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      carlito-mac discover [--timeout <ms>] [--json] [--include-local]
      carlito-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      carlito-mac connect
      carlito-mac connect --url ws://127.0.0.1:18789 --json
      carlito-mac discover --timeout 3000 --json
      carlito-mac wizard --mode local
    """)
}
