...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CARLITO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/carlito")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("carlito-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("carlito-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/carlito/ (rolling or stdout), or nil if none exist.
