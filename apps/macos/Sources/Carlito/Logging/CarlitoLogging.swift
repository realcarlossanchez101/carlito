...
enum CarlitoLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = CarlitoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CarlitoFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CarlitoLogging.bootstrapIfNeeded()
        let label = CarlitoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CarlitoOSLogHandler: AppLogLevelBackedHandler {
...
struct CarlitoFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CarlitoLogging.parseLabel(self.label)
