...
enum CommandResolver {
    private static let projectRootDefaultsKey = "carlito.gatewayProjectRootPath"
    private static let helperName = "carlito"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let carlitoEntry = root.appendingPathComponent("carlito.mjs").path
        if FileManager().isReadableFile(atPath: carlitoEntry) { return carlitoEntry }
        let binEntry = root.appendingPathComponent("bin/carlito.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/carlito")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let carlitoPaths = self.carlitoManagedPaths(home: home)
        if !carlitoPaths.isEmpty {
            extras.insert(contentsOf: carlitoPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + carlitoPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func carlitoManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".carlito"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func carlitoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectCarlitoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("carlito.mjs").path,
            root.appendingPathComponent("bin/carlito.js").path,
        ]
    ...
    static func hasAnyCarlitoInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.carlitoExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func carlitoNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let carlitoPath = self.projectCarlitoExecutable(projectRoot: root) {
            return [carlitoPath, subcommand] + extraArgs
        }
        if let carlitoPath = self.carlitoExecutable(searchPaths: searchPaths) {
            return [carlitoPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "carlito", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            carlito entrypoint missing (looked for dist/index.js or carlito.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real carlito CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/carlito"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/carlito.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/carlito.mjs"
            node "$PRJ/carlito.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/carlito.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/carlito.js"
            node "$PRJ/bin/carlito.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent carlito"
          pnpm --silent carlito \(quotedArgs);
        else
          echo "carlito CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? CarlitoConfigFile.loadDict()
