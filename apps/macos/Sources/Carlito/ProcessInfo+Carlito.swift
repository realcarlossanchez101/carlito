...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.carlito.mac`) even if the shipped
    ...
    {
        if environment["CARLITO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "carlito.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "carlito.nixMode") { return true }
