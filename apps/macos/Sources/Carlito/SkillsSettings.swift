...
private struct SkillRow: View {
    ...
    private var sourceLabel: String {
        ...
        case "carlito-bundled":
            "Bundled"
        case "carlito-managed":
            "Managed"
        case "carlito-workspace":
            "Workspace"
        case "carlito-extra":
            "Extra"
        case "carlito-plugin":
...
private struct EnvEditorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Saved to carlito.json under skills.entries.\(self.editor.skillKey)")
                .font(.caption2)
            ...
            do {
                if isPrimary {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        apiKey: value)
                    self.statusMessage = "Saved API key — stored in carlito.json (skills.entries.\(skillKey))"
                } else {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        env: [envKey: value])
                    self.statusMessage = "Saved \(envKey) — stored in carlito.json (skills.entries.\(skillKey).env)"
                }
            } catch {
                self.statusMessage = error.localizedDescription
            }
