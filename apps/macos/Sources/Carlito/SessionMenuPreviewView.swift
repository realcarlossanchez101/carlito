...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.carlito", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> CarlitoSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: CarlitoSessionPreviewEntry,
    ...
    private static func cache(payload: CarlitoSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: CarlitoChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [CarlitoKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [CarlitoKit.AnyCodable]) -> [CarlitoChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(CarlitoChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: CarlitoChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: CarlitoChatMessage) -> Bool {
    ...
    private static func toolNames(for message: CarlitoChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: CarlitoChatMessage) -> String? {
