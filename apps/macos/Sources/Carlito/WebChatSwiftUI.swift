...
private let webChatSwiftLogger = Logger(subsystem: "ai.carlito", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "carlito.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: CarlitoChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> CarlitoChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(CarlitoChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            CarlitoChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return CarlitoChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> CarlitoChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                CarlitoGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: CarlitoProtocol.ModelChoice) -> CarlitoChatModelChoice {
        CarlitoChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CarlitoChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CarlitoChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CarlitoChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: CarlitoChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CarlitoChatView>) -> NSViewController
    {
