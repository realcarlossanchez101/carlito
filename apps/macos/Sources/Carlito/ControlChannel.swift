...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CarlitoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.carlito", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CarlitoKit.AnyCodable]()) {
                $0[$1.key] = CarlitoKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: CarlitoProtocol.AnyCodable?) -> [String: CarlitoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CarlitoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CarlitoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CarlitoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlPulsecheck = Notification.Name("carlito.control.pulsecheck")
    static let controlAgentEvent = Notification.Name("carlito.control.agent")
}
