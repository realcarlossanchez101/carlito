...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.carlito", category: "portguard")
    #if DEBUG
    private var testingDescriptors: [Int: Descriptor] = [:]
    #endif
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("Carlito", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "carlito", "tsx", "pnpm", "bun"]
    ...
    private static func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("carlito-gateway")
                || cmd.contains("carlito-gateway")
            {
                return true
            }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("carlito"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
