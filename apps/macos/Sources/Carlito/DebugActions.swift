...
enum DebugActions {
    private static let verboseDefaultsKey = "carlito.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "carlito.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = CarlitoPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Carlito", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = CarlitoPaths.configURL
        guard
