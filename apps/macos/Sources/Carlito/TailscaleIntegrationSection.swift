...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.carlito/carlito.json. Prefer CARLITO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.carlito/carlito.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.carlito/carlito.json. Restart the gateway to apply."
        }
