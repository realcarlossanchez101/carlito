...
private let gatewayConnectionLogger = Logger(subsystem: "ai.carlito", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    enum Method: String {
        case agent
        case status
        case setPulsechecks = "set-pulsechecks"
    ...
    private func sessionDefaultString(_ defaults: [String: CarlitoProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func setPulsechecksEnabled(_ enabled: Bool) async -> Bool {
        do {
            try await self.requestVoid(method: .setPulsechecks, params: ["enabled": AnyCodable(enabled)])
            return true
        } catch {
            gatewayConnectionLogger.error("setPulsechecksEnabled failed \(error.localizedDescription, privacy: .public)")
            return false
        }
    }
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(CarlitoGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> CarlitoSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return CarlitoSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> CarlitoChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [CarlitoChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> CarlitoChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
