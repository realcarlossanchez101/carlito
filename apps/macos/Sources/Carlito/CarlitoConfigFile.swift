...
enum CarlitoConfigFile {
    private static let logger = Logger(subsystem: "ai.carlito", category: "config")
    private static let configAuditFileName = "config-audit.jsonl"
    private static let configHealthFileName = "config-health.json"

    static func url() -> URL {
        CarlitoPaths.configURL
    }

    static func stateDirURL() -> URL {
        CarlitoPaths.stateDirURL
    }

    static func defaultWorkspaceURL() -> URL {
        CarlitoPaths.workspaceURL
    }
    ...
    private static func appendConfigWriteAudit(_ fields: [String: Any]) {
        var record: [String: Any] = [
            ...
            "source": "macos-carlito-config-file",
            "event": "config.write",
