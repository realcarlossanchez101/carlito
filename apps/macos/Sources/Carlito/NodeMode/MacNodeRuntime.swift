...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case CarlitoBrowserCommand.proxy.rawValue:
            ...
            case CarlitoLocationCommand.get.rawValue:
            ...
            case CarlitoSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case CarlitoSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case CarlitoSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case CarlitoSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case CarlitoSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CarlitoCanvasA2UICommand.push.rawValue,
             CarlitoCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard CarlitoConfigFile.browserControlEnabled() else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CarlitoNodeError(
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__carlito__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(CarlitoSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(CarlitoSystemWhichParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> CarlitoLocationMode {
    ...
    private static func errorResponse(
        ...
        code: CarlitoNodeErrorCode,
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: CarlitoNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: CarlitoCanvasSnapshotFormat,
        maxWidth: Int?,
