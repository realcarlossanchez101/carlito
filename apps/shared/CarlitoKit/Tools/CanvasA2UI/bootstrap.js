...
import "@a2ui/lit/ui";
import { themeContext } from "@realcarlossanchez101/a2ui-theme-context";
...
class CarlitoA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.carlitoA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["carlito:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.carlitoCanvasA2UIAction ??
      globalThis.carlitoCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.carlitoCanvasA2UIAction) {
...
if (!customElements.get("carlito-a2ui-host")) {
  customElements.define("carlito-a2ui-host", CarlitoA2UIHost);
}
