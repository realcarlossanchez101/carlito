...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> CarlitoChatHistoryPayload
{
    CarlitoChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> CarlitoChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> CarlitoChatSessionEntry
{
    CarlitoChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> CarlitoChatModelChoice {
    CarlitoChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [CarlitoChatHistoryPayload],
    sessionsResponses: [CarlitoChatSessionsListResponse] = [],
    modelResponses: [[CarlitoChatModelChoice]] = [],
    ...
    -> (TestChatTransport, CarlitoChatViewModel)
{
...
private func sendUserMessage(_ vm: CarlitoChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, CarlitoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CarlitoChatHistoryPayload]
    private let sessionsResponses: [CarlitoChatSessionsListResponse]
    private let modelResponses: [[CarlitoChatModelChoice]]
    ...
    private let stream: AsyncStream<CarlitoChatTransportEvent>
    private let continuation: AsyncStream<CarlitoChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<CarlitoChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> CarlitoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CarlitoChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [CarlitoChatAttachmentPayload]) async throws -> CarlitoChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return CarlitoChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CarlitoChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CarlitoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: CarlitoChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = CarlitoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = CarlitoChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: CarlitoChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = CarlitoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CarlitoChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(CarlitoChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == CarlitoChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = CarlitoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CarlitoChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = CarlitoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = CarlitoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = CarlitoChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { CarlitoChatViewModel(sessionKey: "main", transport: transport) }
