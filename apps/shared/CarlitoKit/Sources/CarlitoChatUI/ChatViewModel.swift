...
private let chatUILogger = Logger(subsystem: "ai.carlito", category: "CarlitoChatUI")

@MainActor
@Observable
public final class CarlitoChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [CarlitoChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [CarlitoChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [CarlitoPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CarlitoChatPendingToolCall] = []
    public private(set) var sessions: [CarlitoChatSessionEntry] = []
    private let transport: any CarlitoChatTransport
    private var sessionDefaults: CarlitoChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: CarlitoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any CarlitoChatTransport,
    ...
    public var sessionChoices: [CarlitoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [CarlitoChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: CarlitoPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CarlitoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CarlitoChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: CarlitoChatMessage) -> CarlitoChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> CarlitoChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return CarlitoChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: CarlitoChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: CarlitoChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: CarlitoChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [CarlitoChatMessage],
        incoming: [CarlitoChatMessage]) -> [CarlitoChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [CarlitoChatMessage]) -> [CarlitoChatMessage] {
        var result: [CarlitoChatMessage] = []
    ...
    private static func dedupeKey(for message: CarlitoChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CarlitoChatMessageContent] = [
            CarlitoChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> CarlitoChatAttachmentPayload in
            CarlitoChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> CarlitoChatSessionEntry {
        CarlitoChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = CarlitoChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: CarlitoChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: CarlitoChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: CarlitoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CarlitoChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> CarlitoPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
