...
@MainActor
public struct CarlitoChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: CarlitoChatViewModel
    ...
    public init(
        viewModel: CarlitoChatViewModel,
    ...
    private var visibleMessages: [CarlitoChatMessage] {
        let base: [CarlitoChatMessage]
    ...
    private func mergeToolResults(in messages: [CarlitoChatMessage]) -> [CarlitoChatMessage] {
    ...
    private func isToolResultMessage(_ message: CarlitoChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: CarlitoChatMessage) -> Bool {
    ...
    private func primaryText(in message: CarlitoChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: CarlitoChatMessage) -> Bool {
    ...
    private func toolCalls(in message: CarlitoChatMessage) -> [CarlitoChatMessageContent] {
    ...
    private func inlineToolResults(in message: CarlitoChatMessage) -> [CarlitoChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: CarlitoChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: CarlitoChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(CarlitoChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
