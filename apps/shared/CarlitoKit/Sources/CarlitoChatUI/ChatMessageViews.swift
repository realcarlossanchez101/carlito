...
struct ChatMessageBubble: View {
    let message: CarlitoChatMessage
    let style: CarlitoChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? CarlitoChatTheme.userText : CarlitoChatTheme.assistantText
    ...
    private var inlineAttachments: [CarlitoChatMessageContent] {
    ...
    private var toolCalls: [CarlitoChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [CarlitoChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CarlitoChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CarlitoChatTheme.onboardingAssistantBubble
        }
        return CarlitoChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CarlitoChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CarlitoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? CarlitoChatTheme.userText : CarlitoChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: CarlitoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(CarlitoChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CarlitoChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: CarlitoChatTheme.assistantText)
            }
        }
    }
}
