...
public enum GatewayConnectionProblemMapper {
    ...
    private static func map(_ authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
        ...
        case .authTokenMissing:
            return self.problem(
                ...
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.carlito.ai/gateway/authentication"),
        ...
        case .authTokenNotConfigured:
            return self.problem(
                ...
                actionCommand: authError.actionCommand ?? "carlito config set gateway.auth.token <new-token>",
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.carlito.ai/gateway/authentication"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
        ...
        case .authPasswordNotConfigured:
            return self.problem(
                ...
                actionLabel: authError.actionLabel ?? "Fix on gateway",
                actionCommand: authError.actionCommand ?? "carlito config set gateway.auth.password <new-password>",
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.carlito.ai/gateway/authentication"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
        ...
        case .authBootstrapTokenInvalid:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.carlito.ai/platforms/ios"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        case .authDeviceTokenMismatch:
            return self.problem(
                ...
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.carlito.ai/gateway/pairing"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        ...
        case .deviceAuthSignatureExpired:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.carlito.ai/gateway/troubleshooting"),
                requestId: authError.requestId,
                retryable: true,
                pauseReconnect: true,
                authError: authError)
        ...
        case .authTailscaleIdentityMissing:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.carlito.ai/gateway/tailscale"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        ...
        if nsError.domain == URLError.errorDomain {
            ...
            case .timedOut:
                return GatewayConnectionProblem(
                    ...
                    actionLabel: "Retry",
                    actionCommand: nil,
                    docsURL: URL(string: "https://docs.carlito.ai/gateway/troubleshooting"),
                    retryable: true,
            ...
            case .cannotFindHost, .dnsLookupFailed, .notConnectedToInternet, .networkConnectionLost, .internationalRoamingOff, .callIsActive, .dataNotAllowed:
                return GatewayConnectionProblem(
                    ...
                    message: "Carlito could not reach the gateway over the current network.",
    ...
    private static func approvalCommand(requestId: String?) -> String {
        if let requestId = self.nonEmpty(requestId) {
            return "carlito devices approve \(requestId)"
        }
        return "carlito devices list"
    }
