...
final class RemindersService: RemindersServicing {
    func list(params: CarlitoRemindersListParams) async throws -> CarlitoRemindersListPayload {
        ...
        let payload: [CarlitoReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return CarlitoRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: CarlitoRemindersAddParams) async throws -> CarlitoRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = CarlitoReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return CarlitoRemindersAddPayload(reminder: payload)
    }
