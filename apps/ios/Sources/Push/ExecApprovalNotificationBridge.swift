...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "carlito.exec-approval"
    static let reviewActionIdentifier = "carlito.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.carlitoPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.carlitoPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func carlitoPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
