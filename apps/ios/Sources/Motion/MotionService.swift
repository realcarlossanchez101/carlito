...
final class MotionService: MotionServicing {
    func activities(params: CarlitoMotionActivityParams) async throws -> CarlitoMotionActivityPayload {
        ...
        let mapped: [CarlitoMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return CarlitoMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: CarlitoPedometerParams) async throws -> CarlitoPedometerPayload {
        ...
        let payload: CarlitoPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = CarlitoPedometerPayload(
