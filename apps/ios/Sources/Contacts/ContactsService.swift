...
final class ContactsService: ContactsServicing {
    ...
    func search(params: CarlitoContactsSearchParams) async throws -> CarlitoContactsSearchPayload {
        ...
        return CarlitoContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: CarlitoContactsAddParams) async throws -> CarlitoContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return CarlitoContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return CarlitoContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> CarlitoContactPayload {
        CarlitoContactPayload(
