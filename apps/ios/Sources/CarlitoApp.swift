...
@MainActor
enum CarlitoAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class CarlitoAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.carlito.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.carlito.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.carlito.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? CarlitoAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "carlito.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "carlito.watch.promptId"
    static let sessionKeyKey = "carlito.watch.sessionKey"
    static let actionPrimaryIDKey = "carlito.watch.action.primary.id"
    static let actionPrimaryLabelKey = "carlito.watch.action.primary.label"
    static let actionSecondaryIDKey = "carlito.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "carlito.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "carlito.watch.action.primary"
    static let actionSecondaryIdentifier = "carlito.watch.action.secondary"
    static let actionIdentifierPrefix = "carlito.watch.action."
    static let actionIDKeyPrefix = "carlito.watch.action.id."
    static let actionLabelKeyPrefix = "carlito.watch.action.label."
    static let categoryPrefix = "carlito.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: CarlitoWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> CarlitoWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CarlitoWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Carlito" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [CarlitoWatchAction]) -> [UNNotificationAction] {
...
@main
struct CarlitoApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(CarlitoAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        CarlitoAppModelRegistry.appModel = appModel
...
extension CarlitoApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Carlito: installing uncaught exception handler")
