# Carlito iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Carlito Gateway as a `role: node`.
...
- Beta release uses canonical `ai.carlito.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `CarlitoPushTransport=relay`, `CarlitoPushDistribution=official`, and `CarlitoPushAPNsEnvironment=production`.
...
- `CARLITO_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export CARLITO_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Carlito-<version>.ipa`
   - `apps/ios/build/beta/Carlito-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Carlito.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `CarlitoPushTransport=direct` and `CarlitoPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `CARLITO_APNS_TEAM_ID`, `CARLITO_APNS_KEY_ID`, and either `CARLITO_APNS_PRIVATE_KEY_P8` or `CARLITO_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.carlito/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `CARLITO_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `CarlitoPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `carlito.json`. `CARLITO_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Carlito official builds and to ensure a
