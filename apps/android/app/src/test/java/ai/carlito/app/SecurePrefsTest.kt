...
class SecurePrefsTest {
  ...
  fun loadLocationMode_migratesLegacyAlwaysValue() {
    ...
    val plainPrefs = context.getSharedPreferences("carlito.node", Context.MODE_PRIVATE)
  ...
  fun saveGatewayBootstrapToken_persistsSeparatelyFromSharedToken() {
    val context = RuntimeEnvironment.getApplication()
    val securePrefs = context.getSharedPreferences("carlito.node.secure.test", Context.MODE_PRIVATE)
  ...
  fun clearGatewaySetupAuth_removesStoredGatewayAuth() {
    val context = RuntimeEnvironment.getApplication()
    val securePrefs = context.getSharedPreferences("carlito.node.secure.test.clear", Context.MODE_PRIVATE)
    securePrefs.edit().clear().commit()
    val prefs = SecurePrefs(context, securePrefsOverride = securePrefs)
