...
private const val NOTIFICATION_CHANNEL_BASE_ID = "carlito.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Carlito Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Carlito Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Carlito Active")
      }
