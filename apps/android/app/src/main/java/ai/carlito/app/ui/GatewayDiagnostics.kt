...
internal fun carlitoAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Carlito Android gateway connection failure.
    ...
    - tell me whether `carlito devices list` should show a pending pairing request
    - if more signal is needed, ask for `carlito qr --json`, `carlito devices list`, and `carlito nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${carlitoAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Carlito gateway diagnostics", report))
