...
val androidStoreFile = providers.gradleProperty("CARLITO_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("CARLITO_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("CARLITO_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("CARLITO_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set CARLITO_ANDROID_STORE_FILE, " +
            "CARLITO_ANDROID_STORE_PASSWORD, CARLITO_ANDROID_KEY_ALIAS, and " +
            "CARLITO_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/CarlitoKit/Sources/CarlitoKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.carlito.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "CARLITO_ENABLE_SMS", "false")
            buildConfigField("boolean", "CARLITO_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "CARLITO_ENABLE_SMS", "true")
            buildConfigField("boolean", "CARLITO_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "carlito-$versionName-$buildType.apk"
                    } else {
                        "carlito-$versionName-$flavorName-$buildType.apk"
                    }
